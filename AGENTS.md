...
- Do not create fake fallbacks, placeholder behavior, synthetic demo paths, or
  quiet compatibility branches to skip real implementation work. If the
  canonical implementation or required asset is missing, assert, return a direct
  error, or fail compilation/tests at the boundary where the invariant is
  violated. Runtime ambiguity is worse than a clear failure before public
  release.
...
- Workspace owns tiled surface placement. Apps do not float over each other and do
...
- App crates expose one `UiSurfaceApp` and instantiate it through UI core host
  helpers such as `instantiate_sdl_app`, `instantiate_webgl_app`, or
  `UiSurfaceHost`. Do not wire pointer capture, text input, scroll, drag/drop,
  focus, transitions, scene packing, or renderer setup in app crates.
  App-level shortcuts may use normalized `UiEvent` values; raw SDL/WebGL/JS
  device events stay inside UI core adapters.
...
- `src/gpu/scene.rs`: scene buffer types, clipping, canonical text emission, and
...
- `src/gpu/painter.rs`, `src/gpu/paint.rs`: drawing facade and shared paint
  helpers.
- `src/gpu/workspace.rs`: tiled workspace model, workspace surfaces, tabs, focus, and
...
- `edgerun-ui-core` is a UI toolkit, not an app registry. Do not add product
  apps, app catalogs, app metadata registries, launcher inventories, or
  built-in app surfaces to this crate.
- Use `edgerun_ui_core::gpu::prelude::*` for app crate imports. Implement
  `UiSurfaceApp`; use the core instantiation helpers; keep lower-level SDL,
  WebGL, and direct draw APIs inside trusted framework adapters and diagnostics.
- App surfaces should receive projected state from real stores where available;
  label demo/preview data honestly.
- Shadcn base components are the canonical component system and the shadcn
  showcase is the canonical visual reference. Keep it in core as a component
  reference, not as an app registry or product app.
...
- Default features are `fontdue-text`, `tabler-svg-atlas`, and
  `lucide-svg-atlas`.
