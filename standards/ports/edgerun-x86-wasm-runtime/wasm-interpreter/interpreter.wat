(module
    ...
    (block $lp
      (loop $cont
        ...
        (block $op_handled
          ...
          (if (i32.and (i32.ge_u (local.get $op) (i32.const 0x20)) (i32.le_u (local.get $op) (i32.const 0x24))) ;; local.get/set/tee/global.get/set
            (then
              (if (call $leb_u32 (local.get $offset)) (then (return (global.get $ERR_PARSE))))
              (i32.store (i32.add (local.get $base) (i32.const 4)) (i32.load (global.get $OFF_SCRATCH0)))
              (local.set $offset (i32.add (local.get $offset) (i32.load (global.get $OFF_SCRATCH1))))
  ...
  (func $stack_push (param $val i32) (result i32)
    (local $len i32)
    (local.set $len (i32.load (global.get $OFF_EXEC_STACK_LEN)))
    (if (i32.ge_u (local.get $len) (global.get $MAX_STACK))
      (then (return (global.get $ERR_STK_OV)))
    )
    (i32.store
      (i32.add (global.get $OFF_EXEC_STACK) (i32.shl (local.get $len) (i32.const 2)))
      (local.get $val)
    )
    (i32.store (global.get $OFF_EXEC_STACK_LEN) (i32.add (local.get $len) (i32.const 1)))
    (return (global.get $OK))
  )
  ...
  (func $stack_pop (result i32)
    ...
    (i32.store (global.get $OFF_EXEC_STACK_LEN) (local.get $len))
    (i32.store (global.get $OFF_SCRATCH0)
      (i32.load (i32.add (global.get $OFF_EXEC_STACK) (i32.shl (local.get $len) (i32.const 2))))
    )
    (return (global.get $OK))
  )
  ...
  (func $stack_push_i64 (param $lo i32) (param $hi i32) (result i32)
    (local $len i32)
    (local.set $len (i32.load (global.get $OFF_EXEC_STACK_LEN)))
    (if (i32.ge_u (i32.add (local.get $len) (i32.const 2)) (global.get $MAX_STACK))
      (then (return (global.get $ERR_STK_OV)))
    )
    (i32.store
      (i32.add (global.get $OFF_EXEC_STACK) (i32.shl (local.get $len) (i32.const 2)))
    ...
    (i32.store
      (i32.add (global.get $OFF_EXEC_STACK) (i32.shl (i32.add (local.get $len) (i32.const 1)) (i32.const 2)))
      (local.get $hi)
    )
    (i32.store (global.get $OFF_EXEC_STACK_LEN) (i32.add (local.get $len) (i32.const 2)))
    (return (global.get $OK))
  )
  ...
  (func $stack_pop_i64 (result i32)
    ...
    (i32.store (global.get $OFF_SCRATCH1)  ;; hi
      (i32.load (i32.add (global.get $OFF_EXEC_STACK) (i32.shl (i32.add (local.get $len) (i32.const 1)) (i32.const 2))))
    )
  ...
  (func $exec_fn (param $func_idx i32) (param $args_ptr i32) (param $args_len i32) (result i32)
    ...
    ;; Find code index for this function
    (local.set $code_idx (local.get $func_idx))
    ;; In the simple case, code_idx == func_idx (no imports)
    ...
    (block $dispatch_end
      (loop $dispatch_loop
        ...
        (block $op_done
          ...
          ;; ═════════════════════════════════════════════
          ;; end (0x0B) - structured control flow / function return
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x0B))
            (then
              ...
              (if (i32.or (i32.eqz (local.get $ctrl_len))
                ...
                (then
                  ;; function-level end: pop result and return
                  (local.set $err (call $stack_pop))
                  (if (local.get $err) (then (return (local.get $err))))
                  (i64.store (global.get $OFF_EXEC_RESULT)
                    (i64.extend_i32_s (i32.load (global.get $OFF_SCRATCH0)))
                  )
                  (i32.store (global.get $OFF_EXEC_RES_COUNT) (i32.const 1))
          ...
          ;; ═════════════════════════════════════════════
          ;; block (0x02) - push block control frame
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; loop (0x03) - push loop control frame
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; else (0x05) - skip past matching end
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x05))
            (then
          ...
          ;; ═════════════════════════════════════════════
          ;; return (0x0F) - early return from function
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x0F))
            (then
              ;; Pop result, unwind all control frames, store result and return
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (i64.store (global.get $OFF_EXEC_RESULT)
          ...
          ;; ═════════════════════════════════════════════
          ;; call (0x10) - call another function
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x10))
            (then
              ;; imm0 = function index
              ;; Look up type to get param count
              ...
              ;; Push result from exec_fn onto value stack
              (local.set $err (call $stack_push (i32.wrap_i64 (i64.load (global.get $OFF_EXEC_RESULT)))))
              (if (local.get $err) (then (return (local.get $err))))
              (br $dispatch_loop)
            )
          )
          ...
          ;; i32.load (0x28)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x28))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.store (0x36)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x36))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
          ...
          ;; memory.size (0x3F)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x3F))
            (then
          ...
          ;; ═════════════════════════════════════════════
          ;; memory.grow (0x40) - grow within preallocated region
          ;; ═════════════════════════════════════════════
          ...
          ;; i32.eq (0x46)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x46))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.ne (0x47)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x47))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.lt_u (0x49)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x49))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.gt_s (0x4A)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x4A))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.gt_u (0x4B)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x4B))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.le_s (0x4C)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x4C))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.ge_u (0x4F)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x4F))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.sub (0x6B)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x6B))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.mul (0x6C)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x6C))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x6D))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x6E))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x6F))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.and (0x71)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x71))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.or (0x72)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x72))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.xor (0x73)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x73))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.shl (0x74)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x74))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.shr_s (0x75)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x75))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.shr_u (0x76)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x76))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          ;; br_table (0x0E) - jump to one of several labels
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x0E))
            (then
              ;; imm0 = number of labels
              ;; Pop selector value
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))  ;; selector
              ;; Target label index is stored at op_base+8 (imm1 slot) after br_table parsing
              ;; The decoder stores targets in a temp area after the decoded op
              ;; For MVP: just use the default (last label)
              (local.set $err (call $ctrl_peek (i32.const 0)))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $target (i32.load (global.get $OFF_SCRATCH1)))
              (local.set $dec_idx (local.get $target))
              (br $dispatch_loop)
          ...
          ;; ═════════════════════════════════════════════
          ;; select (0x1B) - conditional value selection
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x1B))
            (then
          ...
          ;; select_t (0x1C) - typed select (same semantics)
          (if (i32.eq (local.get $op) (i32.const 0x1C))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
          ...
          ;; ═════════════════════════════════════════════
          ;; global.get (0x23) - push global value
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; global.set (0x24) - set global value
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; Memory load variants (0x29-0x35)
          ;; ═════════════════════════════════════════════
          ;; i64.load (0x29)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x2E))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          (if (i32.eq (local.get $op) (i32.const 0x2F))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i64.load16_s (0x32)
          (if (i32.eq (local.get $op) (i32.const 0x32))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i64.load16_u (0x33)
          (if (i32.eq (local.get $op) (i32.const 0x33))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i64.load32_s (0x34)
          (if (i32.eq (local.get $op) (i32.const 0x34))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i64.load32_u (0x35)
          (if (i32.eq (local.get $op) (i32.const 0x35))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          ;; Memory store variants (0x37-0x3E)
          ;; ═════════════════════════════════════════════
          ;; i64.store (0x37)
          (if (i32.eq (local.get $op) (i32.const 0x37))
            (then
          ...
          ;; i32.store8 (0x3A)
          (if (i32.eq (local.get $op) (i32.const 0x3A))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i32.store16 (0x3B)
          (if (i32.eq (local.get $op) (i32.const 0x3B))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; i64.store32 (0x3E)
          (if (i32.eq (local.get $op) (i32.const 0x3E))
            (then
              (local.set $err (call $stack_pop_i64))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm2 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ═════════════════════════════════════════════
          ;; f32.const (0x43) - push f32 bit pattern as i32
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; f64.const (0x44) - push f64 bit pattern as two i32 slots
          ;; ═════════════════════════════════════════════
          ...
          (if (i32.eq (local.get $op) (i32.const 0x52))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.lt_s (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x53))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.lt_u (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x54))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.gt_s (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x55))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.gt_u (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          ;; i64.le_s (0x56)
          (if (i32.eq (local.get $op) (i32.const 0x56))
            (then
              (local.set $err (call $stack_pop_i64))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.le_s (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x57))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.le_u (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x58))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.ge_s (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x59))
            (then
              ...
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $err (call $stack_push
                (if (result i32) (i64.ge_u (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          ;; ═════════════════════════════════════════════
          ;; i32.wrap_i64 (0x5A) - wrap i64 to i32
          ;; ═════════════════════════════════════════════
          ...
          ;; ═════════════════════════════════════════════
          ;; i64.extend_i32_s (0x5B) - sign-extend i32 to i64
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0x5B))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
          ...
          ;; ═════════════════════════════════════════════
          ;; i64.extend_i32_u (0x5C) - zero-extend i32 to i64
          ;; ═════════════════════════════════════════════
          ...
          (if (i32.eq (local.get $op) (i32.const 0x78))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm1 (i32.load (global.get $OFF_SCRATCH0)))
          ...
          (if (i32.eq (local.get $op) (i32.const 0x7D))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $tmp64 (i64.sub (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push_i64
                (i32.wrap_i64 (local.get $tmp64))
                (i32.wrap_i64 (i64.shr_u (local.get $tmp64) (i64.const 32)))
              ))
          ...
          ;; i64.mul (0x7E)
          (if (i32.eq (local.get $op) (i32.const 0x7E))
            (then
              (local.set $err (call $stack_pop_i64))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
          ...
          ;; i64.div_s (0x7F)
          (if (i32.eq (local.get $op) (i32.const 0x7F))
            (then
              (local.set $err (call $stack_pop_i64))
              (if (local.get $err) (then (return (local.get $err))))
          ...
          (if (i32.eq (local.get $op) (i32.const 0x86))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $imm2 (i32.and (i32.load (global.get $OFF_SCRATCH0)) (i32.const 63)))  ;; shift amount
              (local.set $err (call $stack_pop_i64))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
          ...
          ;; ═════════════════════════════════════════════
          ;; f32 ops (0x8B-0x9E) - bit-preserving via i32 reinterpret
          ;; ═════════════════════════════════════════════
          ;; f32.add (0x8B)
          (if (i32.eq (local.get $op) (i32.const 0x8B))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.add (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
            )
          ...
          (if (i32.eq (local.get $op) (i32.const 0x8C))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.sub (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x8D))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.mul (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x8E))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.div (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x8F))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.min (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          ;; f32.max (0x90)
          (if (i32.eq (local.get $op) (i32.const 0x90))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.max (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x91))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.abs (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x92))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.neg (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x93))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $tmp64 (f32.copysign (local.get $tmp64) (local.get $tmp64b)))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x94))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.ceil (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x95))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.floor (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x96))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.trunc (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x97))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.nearest (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x98))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (f32.sqrt (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0)))))
              (local.set $err (call $stack_push (i32.reinterpret_f32 (local.get $tmp64))))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x99))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.eq (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x9A))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.ne (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x9B))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.lt (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x9C))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.gt (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x9D))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.le (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          (if (i32.eq (local.get $op) (i32.const 0x9E))
            (then
              ...
              (local.set $tmp64b (f32.reinterpret_i32 (i32.load (global.get $OFF_SCRATCH0))))
              (local.set $err (call $stack_push
                (if (result i32) (f32.ge (local.get $tmp64) (local.get $tmp64b)) (then (i32.const 1)) (else (i32.const 0)))
              ))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
          ...
          ;; f64 ops (0x9F-0xBB) - bit-preserving via i64 reinterpret (two i32 slots)
          ;; Helper macro: pop f64 from two i32 slots into tmp64
          ;; f64.add (0x9F)  (actually 0xA0 per spec, check actual)
          (if (i32.eq (local.get $op) (i32.const 0xA0))
            (then
              ...
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $tmp64 (i64.or (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH0))) (i64.shl (i64.extend_i32_u (i32.load (global.get $OFF_SCRATCH1))) (i64.const 32))))
              (local.set $tmp64 (f64.add (f64.reinterpret_i64 (local.get $tmp64)) (f64.reinterpret_i64 (local.get $tmp64b))))
              (local.set $tmp64 (i64.reinterpret_f64 (local.get $tmp64)))
              (local.set $err (call $stack_push_i64
                (i32.wrap_i64 (local.get $tmp64))
                (i32.wrap_i64 (i64.shr_u (local.get $tmp64) (i64.const 32)))
              ))
          ...
          ;; ═════════════════════════════════════════════
          ;; i32.extend8_s (0xC0) - sign-extend low byte
          ;; ═════════════════════════════════════════════
          ...
          ;; i32.extend16_s (0xC1) - sign-extend low 16 bits
          ...
          ;; ═════════════════════════════════════════════
          ;; ref.null (0xD0) - push null reference (0)
          ;; ═════════════════════════════════════════════
          (if (i32.eq (local.get $op) (i32.const 0xD0))
            (then
              (local.set $err (call $stack_push (i32.const 0)))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $dec_idx (i32.add (local.get $dec_idx) (i32.const 1)))
              (br $dispatch_loop)
            )
          )
          ;; ref.is_null (0xD1) - check if reference is null
          (if (i32.eq (local.get $op) (i32.const 0xD1))
            (then
              (local.set $err (call $stack_pop))
              (if (local.get $err) (then (return (local.get $err))))
              (local.set $val (i32.load (global.get $OFF_SCRATCH0)))
          ...
          ;; ref.func (0xD2) - push function reference
          (if (i32.eq (local.get $op) (i32.const 0xD2))
            (then
  ...
  (func (export "load") (param $wasm_ptr i32) (param $wasm_len i32) (result i32)
    (local $offset i32) (local $err i32)
    ...
    (block $sec_loop
      (loop $sec_cont
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_TYPE))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_IMPORT))
          (then
            ;; TODO: parse imports
          )
        )
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_FUNCTION))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_TABLE))
          (then
            ;; skip table section
          )
        )
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_MEMORY))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_GLOBAL))
          (then
            ;; skip global section
          )
        )
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_EXPORT))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_START))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_ELEMENT))
          (then
            ;; skip element section
          )
        )
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_CODE))
          (then
        ...
        (if (i32.eq (i32.load (global.get $OFF_SCRATCH0)) (global.get $SEC_DATA))
          (then
