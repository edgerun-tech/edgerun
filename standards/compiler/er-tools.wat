...
(module
  ...
  (import "host" "memory" (memory $mem 1024))  ;; 1024 pages = 64 MB
  ...
  ;; ── Module state ───────────────────────────────────────────────────────────
  (global $wasm_ptr     (mut i32) (i32.const 0))  ;; host-provided ptr to .wasm
  (global $wasm_len     (mut i32) (i32.const 0))
  ...
  ;; ── init(wasm_ptr, wasm_len) → file_count or -1
  (func (export "init") (param $wp i32) (param $wl i32) (result i32)
    (global.set $wasm_ptr (local.get $wp))
    (global.set $wasm_len (local.get $wl))
    (global.set $file_count (i32.const 0))
    (global.set $arena_ptr (global.get $ARENA))

    ;; Validate WASM magic \0asm
    (if (i32.ne (i32.load (local.get $wp)) (i32.const 0x6d736100))
      (then (return (i32.const -1))))

    (call $parse_source_section)
  ...
  (func (export "list_files") (param $out i32) (param $max i32) (result i32)
    ...
    (block $done
      (loop $loop
        ...
        (i32.store8 (local.get $pos) (i32.const 0x20))
        (local.set $pos (i32.add (local.get $pos) (i32.const 1)))
        (call $memcpy (local.get $pos) (local.get $np) (local.get $nl))
    ...
    (if (i32.eq (global.get $file_count) (i32.const 1))
      (then (call $memcpy_str (local.get $pos) (i32.const 0x66696c65))  ;; "file"
            (local.set $pos (i32.add (local.get $pos) (i32.const 4))))
      (else (call $memcpy_str (local.get $pos) (i32.const 0x66696c65))  ;; "file"
  ...
  (func (export "edit_file") (param $np i32) (param $nl i32)
    ...
    (call $memcpy (local.get $new_arena) (local.get $cp) (local.get $cl))
  ...
  (func $parse_source_section
    (local $pos i32) (local $section_id i32) (local $payload_len i32)
    (local $ls i32) (local $name_len i32)
    (local $content_ptr i32) (local $content_len i32) (local $fc i32)

    (local.set $pos (i32.const 8))  ;; skip magic + version

    (block $done
      (loop $loop
        (br_if $done (i32.ge_u (local.get $pos) (global.get $wasm_len)))

        (local.set $section_id
          (i32.load8_u (i32.add (global.get $wasm_ptr) (local.get $pos))))
        (local.set $pos (i32.add (local.get $pos) (i32.const 1)))

        (call $leb_u32_read
          (i32.add (global.get $wasm_ptr) (local.get $pos)))
        (local.set $payload_len (global.get $R0))
        (local.set $ls (global.get $R1))
        (local.set $pos (i32.add (local.get $pos) (local.get $ls)))

        (if (i32.eqz (local.get $section_id))
          (then
            (if (call $is_source_section
                  (local.get $section_id)
                  (i32.add (global.get $wasm_ptr) (local.get $pos))
                  (local.get $payload_len))
              (then
                (local.set $name_len (call $cstr_len
                  (i32.add (global.get $wasm_ptr) (local.get $pos))
                  (local.get $payload_len)))
                (local.set $content_ptr
                  (i32.add (i32.add (i32.add (global.get $wasm_ptr)
                    (local.get $pos)) (local.get $name_len)) (i32.const 1)))

                (call $leb_u32_read (local.get $content_ptr))
                (local.set $fc (global.get $R0))
                (local.set $content_ptr
                  (i32.add (local.get $content_ptr) (global.get $R1)))

                (call $parse_file_entries (local.get $content_ptr) (local.get $fc))
                (return)))))

        (local.set $pos (i32.add (local.get $pos) (local.get $payload_len)))
        (br $loop)))
  )

  ;; ── $is_source_section(section_id, payload_ptr, payload_len) → bool
  (func $is_source_section (param $id i32) (param $pp i32) (param $pl i32) (result i32)
    (if (i32.ne (local.get $id) (i32.const 0)) (then (return (i32.const 0))))
    (if (i32.lt_u (local.get $pl) (i32.const 7)) (then (return (i32.const 0))))
    ;; Check "source\0" — inline byte comparison, no data section needed
    (if (i32.ne (i32.load8_u (local.get $pp))    (i32.const 0x73)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 1))) (i32.const 0x6f)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 2))) (i32.const 0x75)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 3))) (i32.const 0x72)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 4))) (i32.const 0x63)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 5))) (i32.const 0x65)) (then (return (i32.const 0))))
    (if (i32.ne (i32.load8_u (i32.add (local.get $pp) (i32.const 6))) (i32.const 0x00)) (then (return (i32.const 0))))
    (i32.const 1)
  )

  ;; ── $cstr_len(ptr, max) → len (without NUL)
  (func $cstr_len (param $ptr i32) (param $max i32) (result i32)
    (local $i i32)
    (block $done
      (loop $loop
        (if (i32.eqz (i32.load8_u (i32.add (local.get $ptr) (local.get $i))))
          (then (br $done)))
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (if (i32.lt_u (local.get $i) (local.get $max))
          (then (br $loop)))))
    (local.get $i))
  ...
  ;; ── $memcpy_str(dst, packed32) — write 4 bytes of a string literal
  (func $memcpy_str (param $dst i32) (param $val i32)
  ...
  (func $copy_to_out (param $sp i32) (param $sl i32) (param $op i32) (param $ml i32) (result i32)
    ...
    (call $memcpy (local.get $op) (local.get $sp) (local.get $copy_len))
    (local.get $copy_len)
  )
)
