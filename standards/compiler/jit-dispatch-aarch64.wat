;; ═════════════════════════════════════════════════════════════════════
  ...
  ;; Use aarch64 placeholder for arch suffix (x86_64 / arm32 / aarch64)
  ;; ═════════════════════════════════════════════════════════════════════
        ...
        (block $dispatch_done
          ...
          (if (i32.eq (local.get $opcode) (i32.const 0xFD))
            (then
              ...
              (block $fd_done
                (if (i32.eq (local.get $imm0) (i32.const 0x00))
                  (then (call $template_v128_load_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x1B))
                  (then (call $template_v128_store_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x0C))
                  (then (call $template_v128_const_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x2D))
                  (then (call $template_i8x16_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x31))
                  (then (call $template_i16x8_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x35))
                  (then (call $template_i32x4_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x39))
                  (then (call $template_i64x2_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3A))
                  (then (call $template_f32x4_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3D))
                  (then (call $template_f64x2_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x2E))
                  (then (call $template_i8x16_extract_lane_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x2F))
                  (then (call $template_i8x16_extract_lane_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x32))
                  (then (call $template_i16x8_extract_lane_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x33))
                  (then (call $template_i16x8_extract_lane_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x36))
                  (then (call $template_i32x4_extract_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x38))
                  (then (call $template_i64x2_extract_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3B))
                  (then (call $template_f32x4_extract_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3E))
                  (then (call $template_f64x2_extract_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x30))
                  (then (call $template_i8x16_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x34))
                  (then (call $template_i16x8_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x37))
                  (then (call $template_i32x4_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3C))
                  (then (call $template_f32x4_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x3F))
                  (then (call $template_f64x2_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x40))
                  (then (call $template_i8x16_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x51))
                  (then (call $template_i16x8_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x62))
                  (then (call $template_i32x4_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x73))
                  (then (call $template_i64x2_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x84))
                  (then (call $template_f32x4_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x95))
                  (then (call $template_f64x2_add_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x41))
                  (then (call $template_i8x16_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x52))
                  (then (call $template_i16x8_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x63))
                  (then (call $template_i32x4_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x74))
                  (then (call $template_i64x2_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x85))
                  (then (call $template_f32x4_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x96))
                  (then (call $template_f64x2_sub_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x50))
                  (then (call $template_i8x16_mul_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x61))
                  (then (call $template_i16x8_mul_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x72))
                  (then (call $template_i32x4_mul_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x87))
                  (then (call $template_f32x4_mul_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x98))
                  (then (call $template_f64x2_mul_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x46))
                  (then (call $template_i8x16_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x57))
                  (then (call $template_i16x8_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x68))
                  (then (call $template_i32x4_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x79))
                  (then (call $template_i64x2_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x8A))
                  (then (call $template_f32x4_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x9B))
                  (then (call $template_f64x2_neg_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x47))
                  (then (call $template_i8x16_eq_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x58))
                  (then (call $template_i16x8_eq_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x69))
                  (then (call $template_i32x4_eq_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x8C))
                  (then (call $template_f32x4_eq_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x9D))
                  (then (call $template_f64x2_eq_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x48))
                  (then (call $template_i8x16_ne_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x59))
                  (then (call $template_i16x8_ne_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6A))
                  (then (call $template_i32x4_ne_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x8B))
                  (then (call $template_f32x4_ne_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x9C))
                  (then (call $template_f64x2_ne_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4B))
                  (then (call $template_i8x16_lt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4A))
                  (then (call $template_i8x16_lt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5C))
                  (then (call $template_i16x8_lt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5B))
                  (then (call $template_i16x8_lt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6D))
                  (then (call $template_i32x4_lt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6C))
                  (then (call $template_i32x4_lt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x8E))
                  (then (call $template_f32x4_lt_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x9F))
                  (then (call $template_f64x2_lt_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4D))
                  (then (call $template_i8x16_gt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4C))
                  (then (call $template_i8x16_gt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5E))
                  (then (call $template_i16x8_gt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5D))
                  (then (call $template_i16x8_gt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6F))
                  (then (call $template_i32x4_gt_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6E))
                  (then (call $template_i32x4_gt_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x90))
                  (then (call $template_f32x4_gt_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA1))
                  (then (call $template_f64x2_gt_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4F))
                  (then (call $template_i8x16_le_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x4E))
                  (then (call $template_i8x16_le_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x60))
                  (then (call $template_i16x8_le_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5F))
                  (then (call $template_i16x8_le_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x71))
                  (then (call $template_i32x4_le_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x70))
                  (then (call $template_i32x4_le_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x92))
                  (then (call $template_f32x4_le_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA3))
                  (then (call $template_f64x2_le_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x49))
                  (then (call $template_i8x16_ge_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x5A))
                  (then (call $template_i16x8_ge_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x6B))
                  (then (call $template_i32x4_ge_s_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x94))
                  (then (call $template_f32x4_ge_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA5))
                  (then (call $template_f64x2_ge_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x43))
                  (then (call $template_i8x16_and_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x44))
                  (then (call $template_i8x16_or_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0x45))
                  (then (call $template_i8x16_xor_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                ;; ── New canonical ops (0xA6+) ──
                (if (i32.eq (local.get $imm0) (i32.const 0xA6))
                  (then (call $template_v128_load8_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA7))
                  (then (call $template_v128_load16_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA8))
                  (then (call $template_v128_load32_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xA9))
                  (then (call $template_v128_load64_splat_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAA))
                  (then (call $template_i8x16_swizzle_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAB))
                  (then (call $template_i64x2_replace_lane_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAC))
                  (then (call $template_v128_not_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAD))
                  (then (call $template_v128_bitselect_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAE))
                  (then (call $template_i8x16_ge_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xAF))
                  (then (call $template_i16x8_ge_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xB0))
                  (then (call $template_i32x4_ge_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xB8))
                  (then (call $template_i8x16_min_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xBA))
                  (then (call $template_i8x16_max_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xBB))
                  (then (call $template_i8x16_avgr_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xBC))
                  (then (call $template_i16x8_min_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xBE))
                  (then (call $template_i16x8_max_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xBF))
                  (then (call $template_i16x8_avgr_u_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xE1))
                  (then (call $template_f32x4_min_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xE2))
                  (then (call $template_f32x4_max_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xE3))
                  (then (call $template_f64x2_min_aarch64 (local.get $dec_ptr)) (br $fd_done)))
                (if (i32.eq (local.get $imm0) (i32.const 0xE4))
                  (then (call $template_f64x2_max_aarch64 (local.get $dec_ptr)) (br $fd_done)))
