...
use lifegraph_mesh::{EventType, MeshFrame};
use lifegraph_core::protocol::CommandEnvelope;
...
impl MeshNetwork {
    ...
    pub fn send_command(
        ...
        from: &NodeID,
        to: &NodeID,
        command: &lifegraph_core::protocol::CommandEnvelope,
    ) -> Result<(), String> {
        let node = self
            .get_node_mut(from)
