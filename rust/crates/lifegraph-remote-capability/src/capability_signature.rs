...
mod tests {
    ...
    fn raw_ecdsa_sign_verify_roundtrip() {
        ...
        let sig2 = p256::ecdsa::Signature::from_slice(&sig_bytes).unwrap();
    ...
    fn sign_and_verify_invocation() {
        ...
        impl MeshSigner for KeySigner {
            ...
            fn sign_digest(&self, digest: &[u8; 32]) -> Result<[u8; 64], lifegraph_hardware_signing::HardwareSigningError> {
                use p256::ecdsa::signature::hazmat::RandomizedPrehashSigner;
                let sig: p256::ecdsa::Signature = self.key.sign_prehash_with_rng(&mut OsRng, digest).unwrap();
                let mut bytes = [0u8; 64];
                bytes.copy_from_slice(&sig.to_bytes());
                Ok(bytes)
            }
