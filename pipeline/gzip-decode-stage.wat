;; Gzip Decode Pipeline Stage — slot 29
  ;; Stage type: batch (state=0)
  ;; Input:  gzip-compressed bytes via input pipe
  ;; Output: decompressed raw bytes via output pipe
  ;; Uses $gzip_member_scan + $deflate_inflate_raw from codec/compress.wat

  (func $process_gzip_decode (export "process_gzip_decode")
    ...
    (local.set $status (call $gzip_member_scan (global.get $SCRATCH_BUF) (local.get $read) (local.get $record)))
