...
export function initCodeAnalyzerConnection(_url?: string): void {
  xrayState.set({ ...xrayState.get(), loading: false, error: null })
}

export function ensureCodeAnalyzerConnection(): void {
}

export function requestAnalysis(_path: string): void {
  xrayState.set({ ...xrayState.get(), loading: false, error: null })
}
