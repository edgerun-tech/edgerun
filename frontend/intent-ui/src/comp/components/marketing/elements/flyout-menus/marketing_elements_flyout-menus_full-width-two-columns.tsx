...
const engagement: Array<{ name: string; href: string; icon: any }> = [
...
const resources: Array<{ name: string; href: string; icon: any }> = [
...
export function Example() {
  return (
    <Popover class="relative isolate z-50 shadow-sm">
      ...
      <PopoverPanel
        transition
        class="absolute inset-x-0 top-16 bg-white transition data-closed:-translate-y-1 data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-150 data-leave:ease-in dark:bg-gray-900"
      >
        {/* Presentational element used to render the bottom shadow, if we put the shadow on the actual panel it pokes out the top, so we use this shorter element to hide the top of the shadow */}
        <div
          aria-hidden="true"
          class="absolute inset-0 top-1/2 bg-white shadow-lg ring-1 ring-gray-900/5 dark:bg-gray-900 dark:shadow-none dark:ring-white/10"
        />
                ...
                <div class="mt-6 flow-root">
                  <div class="-my-2">
                    {engagement.map((item) => (
                      <a
                        href={item.href}
                        class="flex gap-x-4 py-2 text-sm/6 font-semibold text-gray-900 dark:text-white"
                      >
                        <item.icon aria-hidden="true" class="size-6 flex-none text-gray-400 dark:text-gray-500" />
                        {item.name}
                      </a>
                    ))}
                  </div>
                ...
                >
                  <div class="relative flex-none">
                    <img
                      alt=""
                      src={post.imageUrl}
                      class="aspect-2/1 w-full rounded-lg bg-gray-100 object-cover sm:aspect-video sm:h-32 lg:h-auto dark:bg-gray-800"
                    />
                    <div class="absolute inset-0 rounded-lg ring-1 ring-gray-900/10 ring-inset dark:ring-white/10" />
                  </div>
                  <div>
                    <div class="flex items-center gap-x-4">
                      <time dateTime={post.datetime} class="text-sm/6 text-gray-600 dark:text-gray-400">
                        {post.date}
                      </time>
                      <a
                        href={post.category.href}
                        class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
                      >
                        {post.category.title}
                      </a>
                    </div>
