...
function DataTable(props) {
  ...
  const [sortConfig, setSortConfig] = createSignal(null);
  ...
  >
                    <div class="flex items-center gap-1">
                      ...
                      <Show when={sortConfig()?.key === col}>
                        {sortConfig()?.direction === "asc" ? <TbOutlineSortAscending size={14} /> : <TbOutlineSortDescending size={14} />}
                      </Show>
                    </div>
                  </th>}
              </For>
          ...
          <tbody class="divide-y divide-neutral-800">
            <For each={processedRows()}>
              {(row, index) => <tr class={cn(
    "hover:bg-neutral-800/30 transition-colors",
    index() % 2 === 0 ? "bg-transparent" : "bg-neutral-900/20"
  )}>
                  <For each={columns()}>
                    {(col) => <td class="px-4 py-3 text-neutral-300">
                        <span class={getStatusColor(row[col])}>
                          {formatValue(row[col])}
                        </span>
                      </td>}
                  </For>
