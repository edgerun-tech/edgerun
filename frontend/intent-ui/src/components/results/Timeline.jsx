...
import {
  ...
  TbOutlineCalendar,
  TbOutlineFilter,
  TbOutlineSortAscending,
  TbOutlineSortDescending
} from "solid-icons/tb";
function cn(...classes) {
...
function Timeline(props) {
  ...
  const [typeFilter, setTypeFilter] = createSignal(null);
  ...
  >
              {sortOrder() === "asc" ? <TbOutlineSortAscending size={16} /> : <TbOutlineSortDescending size={16} />}
            </button>
          </div>
        </div>
    ...
    class={cn(
      ...
      !typeFilter() ? "bg-neutral-600 text-white" : "text-neutral-500 hover:text-neutral-300 hover:bg-neutral-700"
    )}
  >
              All
            </button>
    ...
    class={cn(
      "px-2 py-1 rounded text-xs font-medium transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-neutral-800",
      typeFilter() === type ? getTypeColor(type) + " text-white" : "text-neutral-500 hover:text-neutral-300 hover:bg-neutral-700"
    )}
    ...
    /* Events */
          ...
          <div class="space-y-4">
            <For each={filteredEvents()}>
              {(event) => <div class="relative flex gap-4">
                  {
    /* Icon */
  }
                  <div class={cn(
                    ...
                    <Show when={event.metadata}>
                      <div class="mt-2 flex flex-wrap gap-2">
                        <For each={Object.entries(event.metadata || {})}>
                          {([key, value]) => <span class="text-xs px-2 py-1 bg-neutral-900 rounded text-neutral-400">
                              {key}: {String(value)}
                            </span>}
                        </For>
                      </div>
