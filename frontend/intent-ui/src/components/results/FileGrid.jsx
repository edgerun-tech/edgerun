...
function getFileIcon(file, size = 24) {
  if (file.type === "folder") {
...
function FileGrid(props) {
  ...
  const [selectedFiles, setSelectedFiles] = createSignal(/* @__PURE__ */ new Set());
    ...
    fallback={
      ...
      <div class="space-y-1">
        ...
        class={cn(
          ...
          selectedFiles().has(file.id || file.name) ? "bg-blue-900/20 border border-blue-600/30" : "hover:bg-neutral-800/50 border border-transparent"
        )}
  ...
  >
    ...
    class={cn(
      ...
      selectedFiles().has(file.id || file.name) ? "bg-blue-900/20 border-blue-600/30" : "bg-neutral-900/30 border-neutral-700 hover:border-neutral-600 hover:bg-neutral-800/50"
    )}
    onClick={() => toggleSelect(file.id || file.name)}
  >
                  {
  ...
  >
    ...
    /* Footer */
      ...
      <Show when={ui()?.metadata}>
        <div data-result-meta-footer class="px-4 py-3 bg-neutral-800/30 border-t border-neutral-700 text-xs text-neutral-500">
