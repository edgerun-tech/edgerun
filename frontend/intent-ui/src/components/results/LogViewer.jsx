...
function LogViewer(props) {
  ...
  const [levelFilter, setLevelFilter] = createSignal(null);
  ...
  const downloadLogs = () => {
    const content = logs().map(
      (l) => `[${l.timestamp || "NOW"}] ${l.level?.toUpperCase() || "INFO"}: ${l.message}`
    ).join("\n");
  ...
  >
    ...
    /* Level filters */
            ...
            <div class="flex items-center gap-1" role="group" aria-label="Filter by log level">
              {["info", "warn", "error", "debug"].map((level) => <button
    type="button"
    ...
    class={cn(
      ...
      levelFilter() === level ? getLevelColor(level) : "text-neutral-500 hover:text-neutral-300 hover:bg-neutral-700"
    )}
  ...
  >
        <For each={filteredLogs()}>
          {(log) => <div class="flex items-start gap-3 py-0.5 hover:bg-neutral-800/50 rounded px-2">
