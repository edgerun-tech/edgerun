...
import { BrowserIframeAppHost } from "@/platform/runtime/browser-iframe-app-host"
...
function declaredRuntime(app: AppDefinition): AppRuntimeKind | undefined {
  ...
  return runtime === "builtin" || runtime === "wasm" || runtime === "sandboxed-worker" || runtime === "browser-iframe" ? runtime : undefined
}
...
export function createAppLaunchPlan(app: AppDefinition, context: AppHostContext = {}): AppLaunchPlan {
  ...
  if (runtime === "browser-iframe") {
    return {
      runtime: "browser-iframe",
      reason: "Packaged browser apps run in a sandboxed iframe after their eapp package is installed.",
      component: <BrowserIframeAppHost app={app} />,
    }
  }
  ...
  if (runtime === "builtin" || app.kind === "builtin" || app.source === "builtin") {
    return {
      runtime: "builtin",
