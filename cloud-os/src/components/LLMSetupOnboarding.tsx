...
export default function LLMSetupOnboarding() {
  const [dismissed, setDismissed] = createSignal(false);
  const [selectedProvider, setSelectedProvider] = createSignal<ProviderPreset | null>(null);
  ...
  const initiateOAuth = async () => {
    ...
    try {
      ...
      // The /api/qwen endpoint redirects to Qwen auth
      // We need to extract the redirect URL to get user_code
      if (deviceResponse.type === 'opaqueredirect') {
        // Can't read redirect URL, so open it directly
      ...
      console.error('[LLMSetup] Device flow error:', error);
                ...
                <div class="p-4 bg-neutral-800/50 rounded-xl border border-neutral-700">
                  <div class="flex items-center gap-3 mb-4">
                    {(() => {
                      ...
                      if (!Icon) return null;
                      return (
                        <div class={`p-2 rounded-lg ${selectedProvider()!.bg}`}>
