...
export default function Onboarding() {
  ...
  const [showFeature, setShowFeature] = createSignal<string | null>(null);
  ...
  onMount(() => {
    ...
    // Show LLM setup if no provider is configured
  ...
  const openIntegrations = () => {
    import('../stores/windows').then(({ openWindow }) => {
      openWindow('integrations');
    });
  };
  ...
  const features = [
    ...
    },
    {
      ...
      icon: 'sparkles',
      title: 'AI Commands',
      description: 'Press Ctrl+Space to open the command bar. Type what you want to do.',
  ...
  return (
    <div class="fixed bottom-20 left-4 z-[9999] w-80">
      <div class="bg-[#1a1a1a] rounded-2xl border border-neutral-800 shadow-2xl overflow-hidden">
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    ...
                    <Show when={feature.connected !== undefined}>
                      <Show when={feature.connected} fallback={
                        <icons.chevronRight size={14} class="text-neutral-600" />
                      }>
                        {(() => {
              ...
              <Show when={showFeature() === feature.id}>
                <div class="mt-3 pt-3 border-t border-neutral-700">
                  <Show when={feature.examples}>
                    ...
                    <div class="space-y-1">
                      {feature.examples!.map((ex) => (
                        <div class="text-xs text-neutral-300 bg-neutral-900 px-2 py-1.5 rounded font-mono">
                          {ex}
                        </div>
                      ))}
                    </div>
                  ...
                  <Show when={feature.id === 'github' && !feature.connected}>
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); openIntegrations(); }}
        ...
        <div class="p-3 border-t border-neutral-800">
          <div class="flex items-center justify-between text-xs text-neutral-500">
            <span>Press <kbd class="px-1.5 py-0.5 bg-neutral-800 rounded text-neutral-400">Ctrl+Space</kbd> for commands</span>
