...
import { 
  TbOutlinePhoto, TbOutlineX, TbOutlineZoomIn, TbOutlineZoomOut,
  TbOutlineChevronLeft, TbOutlineChevronRight, TbOutlineDownload 
} from 'solid-icons/tb';
...
function parseMediaData(data: any): MediaItem[] {
  ...
  // If it's an object with media/images property
  if (data?.media && Array.isArray(data.media)) {
    return data.media;
  }
  if (data?.images && Array.isArray(data.images)) {
    return data.images.map((img: any) => ({ ...img, type: 'image' as const }));
  }
...
export function MediaGallery(props: MediaGalleryProps) {
  ...
  const [selectedMedia, setSelectedMedia] = createSignal<MediaItem | null>(null);
  const [zoom, setZoom] = createSignal(1);
  const [media, setMedia] = createSignal<MediaItem[]>([]);

  // Parse media on mount
  onMount(() => {
        ...
        <Motion.div
          ...
          >
            ...
            <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50 rounded-full px-4 py-2" role="group" aria-label="Zoom controls">
              ...
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); downloadMedia(selectedMedia()!); }}
