...
import {
  TbOutlineGitCommit, TbOutlineArrowsExchange,
  TbOutlineColumns, TbOutlineLayoutGrid
} from 'solid-icons/tb';
...
function parseDiffData(data: any): FileDiff[] {
...
function parseUnifiedDiff(diffText: string): FileDiff[] {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('@@')) {
      ...
      currentHunk = {
        oldStart: match ? parseInt(match[1]) : undefined,
        oldLines: match && match[2] ? parseInt(match[2]) : undefined,
        newStart: match ? parseInt(match[3]) : undefined,
        newLines: match && match[4] ? parseInt(match[4]) : undefined,
        lines: [],
      };
    }
    ...
    if (currentHunk) {
      if (line.startsWith('+') && !line.startsWith('+++')) {
        currentHunk.lines.push({
          ...
          newNumber: currentHunk.newLines ? (currentHunk.newStart || 0) + currentHunk.newLines : undefined,
        });
        ...
        currentHunk.lines.push({
          type: 'remove',
          content: line.slice(1),
          oldNumber: currentHunk.oldLines ? (currentHunk.oldStart || 0) + currentHunk.oldLines : undefined,
        });
  ...
  return (
    ...
    >
      ...
      <div class="overflow-auto max-h-[600px]">
        <For each={diffs()}>
          {(fileDiff) => (
            <Show when={expandedFiles().has(fileDiff.path)}>
                        ...
                        <For each={hunk.lines}>
                          {(line) => (
                            <div class={cn(
                              ...
                              line.type === 'remove' ? 'bg-red-900/20' : 'bg-transparent'
                            )}>
