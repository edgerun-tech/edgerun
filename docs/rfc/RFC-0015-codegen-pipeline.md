...
|-----------|-------|--------|
| `gen_tokenizer.go` | 752 | `tokenizer.rs` (4,551 lines) |
...
**Binary:** `scripts/generate_renderer.py`
...
**Binary:** `scripts/generate_wgsl.py` (planned, see PHASE1_WGSL_DESIGN.md)
**Status:** `shaders/render.wgsl` exists (1,810 lines), `shaders/layout.wgsl` exists
...
| 4c | `generate_renderer.py` | CSS proto data | Rust layout source | ~400 |
| 5 | `generate_wgsl.py` | CSS proto + LUTs | WGSL shaders | 1,810+ |

---

## Known Issues

1. **`generate_wgsl.py` may not exist** — PHASE1_WGSL_DESIGN.md describes it but the actual shader files may have been hand-written or generated by a different mechanism
