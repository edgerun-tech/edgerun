...
Captured at: 2026-04-30T05:06:00Z
...
- Edgerun rootless Podman: 100 local-only SMTP loopback deliveries and
  50 IMAP login/list/select/logout sessions, concurrency 4.
- SMTP-only stacks: 100 local-only loopback deliveries, concurrency
  4, recipient bench@example.test, no external delivery.
- Dovecot IMAP: 50 login/list/select/logout sessions, concurrency
  4, benchmark user bench.
...
- Native Edgerun snapshot: disabled by default. Set
  ...
  fallback.
- Stack selection: `edgerun postfix opensmtpd dovecot maddy mox stalwart`. Override with EDGERUN_EMAIL_STORY_STACKS to run
