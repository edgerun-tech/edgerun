...
- Admitted work carries network authority.
- Event logs carry append-only audit evidence and local projections.
- Legacy commands are local or migration records unless they are bound to
  admitted work.
...
The current model is identity-first and admission-first:

- user intent becomes network work only after signed admission;
- routes, capabilities, proofs, receipts, and settlement bind back to admitted
  work;
- single-writer streams remain append-only, contiguous, hash-linked, and signed
  audit evidence;
