# Current Architecture and Convergence Map

EdgeRun converges around admitted work. `edgerun-work` is the network authority
surface: signed user intent, admission, route commitments, relay/channel proofs,
capability packets, receipts, and settlement evidence. `edgerun-wire` is the
...
```text
UI intent / app intent / node intent
  -> signed WorkRequest
  -> signed WorkAdmission
  -> admitted route / relay channel
  -> capability or workload packets
  -> delivery, transit, storage, execution, and settlement proofs
  -> local audit log and UI projections
```

Only the admitted work chain is authoritative for network work. Append-only
event logs, immutable objects, indexes, query results, replay caches,
dashboards, health endpoints, route views, and local materialized state are
evidence or projections. They must be rebuildable or rejectable from work,
wire, object, and proof records.
...
User experience is modeled as a projection over runtime and work records, not
as a separate permission system. A UI action such as "allow this app to use this
storage location" becomes local runtime state and, when network authority is
needed, admitted work:
...
UI intent
  ...
  -> proofs, receipts, and RuntimeEvent audit log
```
...
|---|---|---|
| Run or cache verified app | `RuntimeAppInstall` | Runtime projection of package identity, declared routes, namespaces, provided and required capabilities. |
...
configuration, service worker memory, and append-only local logs are only
caches or evidence. The admitted work chain is the authority when work crosses a
node, relay, storage, capability, or settlement boundary.
...
rewriting verified app runtime/cache state.
...
| Work authority | `edgerun-work` | Signed requests, admissions, route commitments, ordered channels, capability packets, delivery/transit proofs, receipts, settlement, and native/std adapters. |
| Legacy protocol migration source | `edgerun-core` | Older command/stream/app records and validators. Do not add new authority semantics here; migrate useful records toward wire/work/storage/domain crates. |
| Audit and object storage | `edgerun-storage` | Event logs, objects, EdgeFS, indexes, snapshots, replay cache, and rebuild/integrity logic. These are evidence/projection unless bound to admitted work. |
| Node runtime | `edgerun-node` | Runtime adapter: validates user/app intent, requests admission when work crosses boundaries, binds local resources, records audit events, and realizes host/browser/remote execution. |
| Capabilities and policy | `edgerun-capabilities`, `edgerun-remote-capability` | Capability descriptor builders, grant validation, policy decisions, and adapter envelopes that must converge on admitted work routing. |
| Mesh and sessions | `edgerun-mesh` | Transitional identity-addressed routing and signed frames. New cross-node authority should converge on `edgerun-work` routes/channels. |
...
1. Do not add a second authority model. If work crosses a node, relay,
   capability, storage, execution, or settlement boundary, it must be expressible
   as admitted work or as evidence derived from admitted work.
2. Do not add a second wire model. If data crosses an Edgerun internal wire
   boundary, archive the concrete rkyv type.
3. Keep portable authority semantics in `edgerun-work`; keep host resources in
   node/runtime adapters.
4. Keep local event logs append-only, but treat them as audit/evidence and UI
   projection input unless a record is bound to admitted work.
...
|---|---|---|
| Signing adapters | Storage, node, mesh, and hardware crates adapt several signer shapes. | Keep signer adapters at the edge. Work signatures and verifier preimages should be canonical in `edgerun-work`; local stream signing remains audit support. |
| Stream validation | Storage validates chain integrity and event replay. | Keep sequence/hash/signature rules centralized in storage, but do not treat a local stream append as network authority without admitted work evidence. |
| Command outcomes | Command status appears in core validators, node dispatch, storage replay, and exchange projections. | Freeze as legacy/migration behavior. New cross-node actions should be admitted work; service-specific projections stay derived views. |
| Capability transport | `edgerun-remote-capability`, mesh paths, node runtime sessions, and work capability packets all carry invocation/session semantics. | Converge invocation authority on `edgerun-work` capability packets; keep transport-specific queue/session code as adapters. |
...
- Derived service projections and append-only audit logs. Projections are views;
  logs are evidence.
...
3. Move audit/storage crates next: `storage/`, then update node/storage
   dependents.
