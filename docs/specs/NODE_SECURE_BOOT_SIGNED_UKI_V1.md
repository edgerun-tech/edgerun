...
- Ensure `edgerun-node-manager` creates and manages a local `agave-validator` instance and uses its RPC endpoint for acceptance, registration metadata, and submission lifecycle operations.
...
- Node manager starts/restarts `agave-validator` locally (with local ledger init and identity key generation) and waits for JSON-RPC health before bootstrap/heartbeat calls.
