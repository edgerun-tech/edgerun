...
nodes. The first release should sell and run/cache wasm apps, templates, and
user store packages while Exchange handles non-custodial crypto settlement. EdgeRun
...
User stores can come next once the same listing, run/cache, payment, and receipt
machinery is proven.
...
- run once or verify and cache wasm packages on a node after payment receipt
  verification
...
- Buyer: chooses an app/package, approves payment, and runs or caches it on a
  node.
...
- Node: validates app intent, capability grants, local runtime projections, and
  audit evidence while cross-node authority comes from admitted work.
...
provider route.
The node records settlement audit events and the marketplace marks the sale paid
from admitted-work receipts, provider observations, and projection facts.
...
Marketplace browse data is advisory. Run/cache and payment decisions are protocol
facts:
...
- checkout creates a signed `AppIntent` over the exact listing and settlement
  terms
- settlement must converge on admitted work carrying the exact identity,
  package, payment, route, and policy terms
- node stream events are audit evidence for paid/runnable/cacheable state
...
MarketplaceRunReceipt
  listing_id
  package_id
  receipt_id
  run_or_cache_admission
  runtime_event
```
...
Marketplace run/cache is useful because buyers can see what an app wants before
paying, running, or caching:
...
capabilities. A paid app does not automatically get capability access; payment
unlocks run/cache eligibility, then admission and node policy grant the minimum
...
10. Buyer runs once or verifies and caches the package on the selected node.
11. Node validates package signature, capability policy, and admitted work, then
    records local runtime/audit events.
...
- receipt-to-run/cache eligibility checks
- rkyv archive helpers for marketplace objects

Keep HTTP/browser API as ingress only. The SDK module should not own admission,
settlement authority, or custody money. Node/store integration should append
marketplace audit events and derive projections from admitted work, receipts,
and local streams.
...
MarketplaceRunAuthorized
MarketplacePackageCached
```
...
2. Connect checkout to `edgerun-exchange` settlement intents and receipt
   verification.
3. Add package run/cache eligibility: paid receipt plus package signature plus
   capability policy.
4. Add HTTP/browser ingress for browse, checkout, run, and verify/cache routed
   into admitted work.
5. Build a compact marketplace UI: browse, detail, checkout, receipt, run/cache.
...
- every paid run/cache can be proven from admitted work, receipts, and audit
  events
- every checkout discloses and signs the commission split
- no route requires EdgeRun custody
- package run/cache fails closed on signature or capability mismatch
- marketplace indexes can rebuild from local stream events and admitted-work receipts
