...
- `codex-cli` container with `/home/ken` mounted for fast local Codex iteration
- `caddy` ingress service on `127.0.0.1:4175` serving generated output from `out/frontend/osdev` + proxying `/v1/local/*` and `/api/*`
