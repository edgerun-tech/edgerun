...
function compileWat(watPath: string, wasmPath: string): boolean {
  ...
  const r = Bun.spawnSync(['wat2wasm', watPath, '-o', wasmPath]);
  if (r.exitCode === 0) {
    const size = Bun.file(wasmPath).size;
    console.log(`  ✓ ${wasmPath} (${(size / 1024).toFixed(0)} KB)`);
    return true;
  }
  console.error(`  ✗ wat2wasm failed: ${r.stderr.toString().slice(0, 500)}`);
  return false;
}

function stripWasm(wasmPath: string): string | null {
  try {
    const strippedPath = wasmPath.replace(/\.wasm$/, '-stripped.wasm');
    const r = Bun.spawnSync(['wasm-tools', 'strip', '--all', wasmPath, '-o', strippedPath]);
    if (r.exitCode === 0) {
      const wSize = Bun.file(wasmPath).size;
      const sSize = Bun.file(strippedPath).size;
...
function optimizeWasm(wasmPath: string): string | null {
  try {
    const optPath = wasmPath.replace(/\.wasm$/, '-opt.wasm');
    const r = Bun.spawnSync(['wasm-opt', '-Oz', wasmPath, '-o', optPath]);
    if (r.exitCode === 0) {
      const wSize = Bun.file(wasmPath).size;
      const oSize = Bun.file(optPath).size;
...
function cmdBuild(args: string[]): void {
  ...
  const moduleWat = wrapModule(imports + '\n' + body);
...
function cmdBuildBuildTool(): void {
  const ROOT = resolveRoot();
  const FRAGMENTS = [
    resolve(ROOT, 'compiler', 'er-tools-fragment.wat'),
    resolve(ROOT, 'tools', 'build-tool.wat'),
  ];
  const OUT_DIR = resolve(ROOT, 'out', 'tools');
  ensureDir(OUT_DIR);

  const moduleProlog = `(module
  (import "host" "memory" (memory $mem 1024))
`;
  const moduleEpilog = '\n)';

  let body = '';
  for (const fp of FRAGMENTS) {
    if (!fileExists(fp)) { console.error(`Fragment not found: ${fp}`); process.exit(1); }
    body += stripModuleWrapper(readText(fp)).trimEnd() + '\n\n';
  }

  const fullWat = moduleProlog + body + moduleEpilog;
  const watPath = resolve(OUT_DIR, 'build-tool.wat');
  const wasmPath = resolve(OUT_DIR, 'build-tool.wasm');
  writeText(watPath, fullWat);
  console.log(`✓ ${watPath} (${(fullWat.length / 1024).toFixed(0)} KB)`);

  if (!compileWat(watPath, wasmPath)) process.exit(1);
}
...
Example:
  ...
  Bun.spawnSync(['rm', '-f', tmpWat, tmpWasm]);
...
Commands:
  ...
  build-tool              Build er-tools WASM module (out/tools/build-tool.wasm)
  help                    Show this help
`);
  }
}
