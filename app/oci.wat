;; LX64 syscall globals provided by out/gen/config.wat (linux_syscall_x64 group)
...
;; Status values: 0 ok, 1 invalid, 2 too large.
  ...
  (func (export "oci_media_layer_compression") (param $ptr i32) (param $len i32) (result i32)
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28672) (i32.const 38)) (then (return (i32.const 0))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28712) (i32.const 55)) (then (return (i32.const 0))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28768) (i32.const 44)) (then (return (i32.const 0))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28812) (i32.const 43)) (then (return (i32.const 1))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28856) (i32.const 60)) (then (return (i32.const 1))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28916) (i32.const 49)) (then (return (i32.const 1))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 28968) (i32.const 43)) (then (return (i32.const 2))))
    (if (call $string_eq (local.get $ptr) (local.get $len) (i32.const 29012) (i32.const 60)) (then (return (i32.const 2))))
    i32.const 3)
  ...
  (func $x86_syscall_class (param $nr i32) (result i32)
    (if (i32.or (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_UNSHARE)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETNS)))
                (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETHOSTNAME)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETDOMAINNAME))))
      (then (return (i32.const 1))))
    (if (i32.or
          (i32.or
            (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_MOUNT)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_PIVOT_ROOT)))
            (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_UMOUNT2)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_OPEN_TREE))))
          (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_MOVE_MOUNT)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_MOUNT_SETATTR))))
      (then (return (i32.const 2))))
    (if (i32.or
          (i32.or
            (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SECCOMP)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_CAPSET)))
            (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_PRCTL)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETUID))))
          (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETGID)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SETGROUPS))))
      (then (return (i32.const 3))))
    (if (i32.or (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_PRLIMIT64)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_SCHED_SETATTR)))
                (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_IOPRIO_SET)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_UMASK))))
      (then (return (i32.const 4))))
    (if (i32.or
          (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_KILL)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_WAIT4)))
          (i32.or (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_EXECVE)) (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_CLONE))))
      (then (return (i32.const 5))))
    (if (i32.eq (local.get $nr) (global.get $LINUX_SYS_X64_BPF)) (then (return (i32.const 6))))
    i32.const 0)
...
;; Status: 0 ok, 1 unsupported kind, 2 short, 3 invalid checksum/path/octal, 4 zero block.
  ...
  (data (i32.const 28672) "application/vnd.oci.image.layer.v1.tar")
  (data (i32.const 28712) "application/vnd.oci.image.layer.nondistributable.v1.tar")
  (data (i32.const 28768) "application/vnd.docker.image.rootfs.diff.tar")
  (data (i32.const 28812) "application/vnd.oci.image.layer.v1.tar+gzip")
  (data (i32.const 28856) "application/vnd.oci.image.layer.nondistributable.v1.tar+gzip")
  (data (i32.const 28916) "application/vnd.docker.image.rootfs.diff.tar.gzip")
  (data (i32.const 28968) "application/vnd.oci.image.layer.v1.tar+zstd")
  (data (i32.const 29012) "application/vnd.oci.image.layer.nondistributable.v1.tar+zstd")
