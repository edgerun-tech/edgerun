//! Codec between exchange domain events and the node-local audit stream.
//!
//! This module does not introduce a new authority model, event sink, store, or
//! persistence path.
...
//! returned event type through the existing local `EventEnvelope` stream writer.
//! Settlement authority must still bind to admitted work and verifiable receipts.
...
use edgerun_protocols::core_protocol::protocol::edgerun_wallet_v0::{
    WalletDepositObservedPayload, WalletExchangeEventPayload, WalletManualReviewRequiredPayload,
    ...
    WalletQuoteCreatedPayload, wallet_exchange_event_payload,
};
