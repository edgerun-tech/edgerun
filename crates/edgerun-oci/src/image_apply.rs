...
use crate::layer_pipeline::{format_digest, sha256_layer_digest, LayerDigest};
use crate::prelude::*;
use crate::registry::manifest::LayerDescriptor;
use crate::tar_layer::{
    apply_uncompressed_tar_layer, validate_and_decode_tar_layer, TarLayerApplyError,
    TarLayerApplyReport, TarLayerSink,
};
use core::fmt;
    ...
    S: TarLayerSink,
{
    ...
    for (index, (descriptor, blob)) in plan.layers.iter().zip(blobs.into_iter()).enumerate() {
        ...
        let decoded =
            validate_and_decode_tar_layer(descriptor, [blob], digest_for_layer(descriptor))
        ...
        layer_reports.push(TarLayerApplyReport {
            layer: decoded.layer,
