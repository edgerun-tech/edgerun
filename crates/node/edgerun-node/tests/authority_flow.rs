...
#[test]
fn sdk_authored_app_records_runtime_projection_and_is_archived_to_node_stream() {
    ...
    runtime
        .record_app_graph_wire_runtime_projection(&package.app_graph_bytes, 1)
        .expect("runtime records sdk app graph");
    ...
    let runtime_projection_payload =
        from_bytes::<SdkWireRecord, edgerun_protocols::wire::WireError>(&archived_event.payload)
            .expect("decode runtime projection payload");
    let runtime_projection = match runtime_projection_payload {
        SdkWireRecord::RuntimeAppInstall(projection) => projection,
        _ => panic!("expected runtime app projection payload"),
    };
    assert_eq!(runtime_projection.app_id, package.app_id);
    assert_eq!(runtime_projection.release_id, package.release_id);
    assert_eq!(runtime_projection.declared_routes.len(), 1);
    ...
    .expect("append sdk app runtime projection event to node stream");
    ...
    assert_eq!(
        store
            .validate_stream_chain_with_writer(&node.node_id, &node.node_id)
            .expect("validate sdk app runtime projection stream"),
        2
    );
