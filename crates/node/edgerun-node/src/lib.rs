//! edgerun Node — IPC router and resource adapter.
//!
//! A node moves data between apps, protocol state machines, sockets,
//! capabilities, storage, work admissions, and mesh links. It is the local
//! enforcement boundary for host resources. Apps can declare routes and ask for
//! capabilities, but they do not bind ports, share memory, open files, or own
//! sockets directly.
//!
//! Cross-node authority comes from admitted work. Signed stream events are local
//! audit and replay evidence unless they are bound to an admitted work chain.
//! Text configuration is not an authority boundary.
...
/// A edgerun node.
///
/// The node owns a local audit stream, manages capability grants, and processes
/// legacy incoming commands through validation, authorization, and event
/// recording. Cross-node work authority belongs to admitted work.
#[cfg(feature = "node-core")]
pub struct Node<L = MemEventLog, S = MeshProtocolSigner> {
    ...
    config: NodeConfig,
    /// The node's signed durable local audit stream writer.
...
impl<L: EventLog, S: ProtocolSigner> Node<L, S> {
    ...
    /// would let callers mutate local state before storage can record audit
    /// evidence. Cross-node authority belongs to admitted work.
