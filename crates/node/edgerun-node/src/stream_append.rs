//! Node helpers for appending signed local audit stream events.
//!
//! These helpers load the prior durable head, ask `edgerun-storage::stream` to build the
//! next signed event, then append that exact envelope to storage. The resulting stream is
//! local replay/audit evidence; network authority still comes from admitted work bindings.
...
pub fn append_command_stream_event(
    ...
    .map_err(|e| {
        crate::node_warn!("failed to append local audit stream event: {e}");
        e
    })
    .ok()?;
    Some(event.seq)
