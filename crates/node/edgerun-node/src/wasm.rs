...
struct BrowserNodeState {
    ...
    runtime_apps: BTreeMap<[u8; 32], RuntimeAppProjection>,
    node_id: [u8; 32],
    stream_id: [u8; 32],
    started_at_ms: u64,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn edgerun_node_verify_cache_eapp(
    ptr: *const u8,
    len: usize,
    time_ms: u64,
) -> u64 {
    ...
    let runtime_projection = graph.runtime_install.clone();
    let app_id = runtime_projection.app_id;
    let release_id = runtime_projection.release_id;
    let manifest_sha256 = runtime_projection.manifest_sha256;
    let developer_id = runtime_projection.developer_id;
    ...
    if let Err(error) = node
        .runtime
        .record_app_graph_runtime_projection(graph, time_ms)
    {
        return write_result(400, error.to_string().into_bytes());
    }
    node.runtime_apps.insert(
        app_id,
        RuntimeAppProjection {
            app_id,
            release_id,
            manifest_sha256,
            developer_id,
...
#[unsafe(no_mangle)]
pub extern "C" fn edgerun_node_cached_app_count() -> usize {
...
fn nodes_json(node: &BrowserNodeState) -> String {
    ...
    nodes.push(format!(
        r#"{{"nodeId":"{}","identity":"{}","target":"wasm","runtime":"wasm","relationship":"self","controlled":true,"reachable":true,"health":"healthy","cachedAppCount":{}}}"#,
        hex32(&node.node_id),
        hex32(&node.node_id),
        node.runtime_apps.len(),
    ));
...
fn apps_json(node: &BrowserNodeState) -> String {
    let mut apps = Vec::with_capacity(node.runtime_apps.len());
    for app in node.runtime_apps.values() {
        apps.push(format!(
