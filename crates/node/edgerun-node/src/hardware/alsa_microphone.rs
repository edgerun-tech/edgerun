...
const SNDRV_PCM_IOCTL_HW_PARAMS: c_ulong = c_iowr(b'A', 0x11, SNDRV_PCM_HW_PARAMS_SIZE);
const SNDRV_PCM_IOCTL_PREPARE: c_ulong = c_io(b'A', 0x40);
const SNDRV_PCM_IOCTL_READI_FRAMES: c_ulong = c_iowr(b'A', 0x51, std::mem::size_of::<SndXferi>());

const fn c_iowr(ty: u8, nr: u8, size: usize) -> c_ulong {
    // _IOC(_IOC_READ | _IOC_WRITE, ty, nr, size)
    // direction: READ=2, WRITE=1 => READ|WRITE = 3
    ((3u32 << 30) | ((ty as u32) << 8) | (nr as u32) | ((size as u32) << 16)) as c_ulong
}
const fn c_io(ty: u8, nr: u8) -> c_ulong {
    (((ty as u32) << 8) | (nr as u32)) as c_ulong
}
