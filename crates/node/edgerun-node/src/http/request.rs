...
///
/// Used by [`crate::http::Handler`] across HTTP/1.1, HTTP/2, and HTTP/3 servers.
#[derive(Clone)]
pub struct Request {
    method: Method,
    uri: Uri,
...
impl RequestBuilder {
    ...
    pub fn build(self) -> Result<Request> {
        let uri_str = self
            .uri
            .ok_or_else(|| crate::http::Error::InvalidRequest("No URI specified".to_string()))?;
        let uri = Uri::parse(&uri_str).map_err(crate::http::Error::InvalidUri)?;
