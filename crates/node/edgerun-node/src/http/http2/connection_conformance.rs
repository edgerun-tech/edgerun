...
use crate::http::http2::frame::{Frame, FrameType};
use crate::http::http2::settings::Settings;
use crate::http::http2::stream::{Stream, StreamManager, StreamState};
...
fn settings_ack_with_payload_rejected() {
    ...
    assert_eq!(frame.validate_semantics(), Err(crate::http::http2::ErrorCode::FRAME_SIZE_ERROR.to_u32()));
}
...
fn settings_on_nonzero_stream_rejected() {
    ...
    let (frame, _) = Frame::from_bytes(wire, Frame::DEFAULT_MAX_FRAME_SIZE).unwrap();
    assert_eq!(frame.validate_semantics(), Err(crate::http::http2::ErrorCode::PROTOCOL_ERROR.to_u32()));
}
