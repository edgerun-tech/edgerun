...
use crate::http::runtime::mpsc::{self, Receiver, Sender};
...
impl Body {
    ...
    pub async fn collect(self) -> crate::http::runtime::io::Result<Vec<u8>> {
        match self.inner {
...
impl Future for ReadChunkFut<'_> {
    type Output = crate::http::runtime::io::Result<Option<Vec<u8>>>;
        ...
        match &mut this.reader.inner {
            ...
            BodyReaderInner::Stream { rx } => {
                ...
                match rx.try_recv() {
                    ...
                    Err(crate::http::runtime::mpsc::TryRecvError::Empty) => {
...
impl BodySender {
    ...
    pub fn send(&mut self, data: Vec<u8>) -> crate::http::runtime::io::Result<()> {
        match self.tx.try_send(data) {
            Ok(()) => Ok(()),
            Err(_) => Err(crate::http::runtime::io::Error::new(
...
use crate::http::runtime::AsyncRead;
...
impl<R: AsyncRead + Unpin> Future for ReadBodyFut<'_, '_, R> {
    type Output = crate::http::runtime::io::Result<usize>;
...
impl<R: AsyncRead + Unpin> ReadBodyFut<'_, '_, R> {
    fn poll_chunked_read(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<crate::http::runtime::io::Result<usize>> {
        loop {
            match self.reader.state {
                ChunkState::ChunkSize => {
                    ...
                    loop {
                        ...
                        if n == 0 {
                            return Poll::Ready(Err(crate::http::runtime::io::Error::new(
                                crate::http::runtime::io::ErrorKind::UnexpectedEof,
...
impl<R: AsyncRead + Unpin> Future for CollectBodyFut<R> {
    type Output = crate::http::runtime::io::Result<Vec<u8>>;
