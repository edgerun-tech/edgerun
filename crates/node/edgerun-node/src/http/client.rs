...
//! `crate::tls::AsyncTlsStream`.

use crate::http::header::HeaderMap;
use crate::http::http1::pool::ConnectionPool;
#[cfg(feature = "tls")]
...
impl HttpClient {
    ...
    /// Configure HTTP/3 QUIC trust roots from admitted runtime policy/projection state.
        ...
        F: FnMut(&[u8]) -> Result<()>,
    {
        ...
        let mut h1_req = crate::http::http1::Request::builder()
    ...
    async fn execute_http3(&self, request: &Request) -> Result<Response> {
        ...
        use crate::http::http3::quic::QuicConnectOptions;
        ...
        loop {
            ...
            .await
            .map_err(|e| {
                Error::Network(crate::http::runtime::io::Error::new(
                    crate::http::runtime::io::ErrorKind::ConnectionRefused,
                    e,
                ))
            })?;
    ...
    async fn execute_http3(&self, _request: &Request) -> Result<Response> {
        Err(Error::ProtocolError(
            "HTTP/3 requires edgerun-node http http3 feature".into(),
        ))
    }
    ...
    async fn execute_http2(&self, _request: &Request) -> Result<Response> {
        Err(Error::ProtocolError(
            "HTTP/2 client requires edgerun-node http tls feature".into(),
        ))
    }
}
...
fn timing_log_http2_fallback(request: &Request, err: &Error) {
    ...
    if std::env::var_os("EDGERUN_TIMING").is_some() {
        std::eprintln!("http2.fallback url={} error={}", request.uri(), err);
    }
}
...
fn timing_log_http3_fallback(request: &Request, err: &Error) {
    #[cfg(feature = "std")]
    if std::env::var_os("EDGERUN_TIMING").is_some() {
        std::eprintln!("http3.fallback url={} error={}", request.uri(), err);
    }
}
...
fn timing_log_best_winner(request: &Request, protocol: &str) {
    #[cfg(feature = "std")]
    if std::env::var_os("EDGERUN_TIMING").is_some() {
        std::eprintln!("best.winner protocol={} url={}", protocol, request.uri());
    }
}
