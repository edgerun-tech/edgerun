...
#[cfg(feature = "tls")]
pub type TlsCertificate = crate::tls::certificate_gen::CertificateAndKey;
...
impl HttpServer {
    ...
    pub async fn bind(
        self,
        addr: impl crate::http::runtime::net::ToSocketAddrs,
    ) -> crate::http::runtime::io::Result<BoundHttpServer> {
...
pub struct BoundHttpServer {
    ...
    http2_idle_timeout: Duration,
    #[cfg(feature = "http3")]
    http3_server: Option<Arc<crate::http::http3::Http3Server>>,
}
...
impl BoundHttpServer {
    ...
    pub async fn serve(&self) -> crate::http::runtime::io::Result<()> {
    ...
    pub async fn accept_one(&self) -> crate::http::runtime::io::Result<()> {
        let (stream, _) = self
            .listener
            .accept()
            .await
        ...
        if negotiated_h2 {
            ...
            match reader.get_mut().read_exact(&mut preface_bytes).await {
                Ok(()) => {
                    ...
                    if preface_bytes != H2_PREFACE {
                        ...
                        return Err(crate::http::runtime::io::Error::new(
                            crate::http::runtime::io::ErrorKind::InvalidInput,
                ...
                Err(e) => {
                    ...
                    sleep(Duration::from_millis(50)).await;
                    let _ = reader.get_mut().shutdown().await;
                    return Err(crate::http::runtime::bare_io(e));
                }
            }
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    loop {
        ...
        let method: Method = match parts[0]
            .parse()
            .map_err(|e: String| crate::http::Error::InvalidRequest(e))
        {
            Ok(m) => m,
            Err(_) => break,
        };
        ...
        let connection_keep_alive = headers.iter().any(|(name, value)| {
            name.as_str().eq_ignore_ascii_case("connection")
                && value
                    .as_str()
                    .split(',')
        ...
        let close_after_response = connection_close
            || (http_version.eq_ignore_ascii_case("HTTP/1.0") && !connection_keep_alive);
        ...
        let mut response = maybe_compress_response(&request, response);
        if !response.headers().contains_key("Connection") {
            let value = if close_after_response {
                "close"
            } else {
                "keep-alive"
            };
            let _ = response.headers_mut().insert("Connection", value);
        }
        if !close_after_response
            && http_version.eq_ignore_ascii_case("HTTP/1.0")
            && !response.headers().contains_key("Keep-Alive")
        {
            let _ = response.headers_mut().insert("Keep-Alive", "timeout=5");
        }
        write_http1_response(&mut reader, &response, is_head).await?;

        if close_after_response || keep_alive.is_none() {
            break;
        }
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let settings_frame = crate::http::http2::frame::SettingsFrame::new(entries.clone());
        ...
        let action = match frame.frame_type {
            ...
            FrameType::Priority => {
                let pf = if frame.payload.len() >= 5 {
                    ...
                    crate::http::http2::frame::PriorityFrame::new(
                        frame.stream_id,
                        (dep_raw >> 31) != 0,
                        dep_raw & 0x7FFFFFFF,
                    ...
                    crate::http::http2::frame::PriorityFrame::new(frame.stream_id, false, 0, 16)
                };
            ...
            FrameType::Headers => {
                let hf = match crate::http::http2::frame::HeadersFrame::from_frame(&frame) {
                    Ok(hf) => hf,
                    Err(_) => {
                        write_goaway(
                            &mut rdwr,
...
async fn process_request_with_body(
    ...
    if !response.body().is_empty() {
        resp_frames.push(
            crate::http::http2::frame::DataFrame::new(stream_id, response.body().to_vec(), true)
                .to_frame(),
        );
    }
    if let Some(s) = server.stream_manager.get_stream_mut(stream_id) {
...
async fn read_frame<S>(
    stream: &mut S,
    max_frame_size: u32,
) -> crate::http::runtime::io::Result<Result<Frame, (u32, u32)>>
where
    S: AsyncRead + Unpin,
{
    let mut hdr = [0u8; 9];
...
async fn write_frame<S>(stream: &mut S, frame: &Frame) -> crate::http::runtime::io::Result<()>
where
    S: AsyncWrite + Unpin,
{
    stream
    ...
    S: AsyncWrite + Unpin,
{
    let _ = write_frame(
        stream,
        &crate::http::http2::frame::GoawayFrame::new(last_stream_id, error_code, debug.to_vec())
            .to_frame(),
    )
    .await;
}
    ...
    S: AsyncWrite + Unpin,
{
    let _ = write_frame(
        stream,
        &crate::http::http2::frame::RstStreamFrame::new(stream_id, error_code).to_frame(),
    )
    .await;
}
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let content_length = if skip_body { 0 } else { response.body().len() };
    let mut bytes = response_header_bytes(response, content_length);
...
where
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    let bytes = response_header_bytes(response, content_length);
    reader
        .write_all(&bytes)
        .await
        .map_err(crate::http::runtime::bare_io)?;
...
fn response_header_bytes(response: &Response, content_length: usize) -> Vec<u8> {
...
fn maybe_compress_response(request: &Request, response: Response) -> Response {
    ...
    let encoding = crate::http::http1::preferred_response_encoding(accept_encoding);
    if encoding == crate::http::http1::ContentEncoding::Identity {
        return response;
    }
    let Some(compressed) = crate::http::http1::compress_body(response.body(), encoding) else {
        return response;
    };
