...
pub struct QuicConnectOptions {
    ...
    ///
    /// In Edgerun nodes this should be derived from admitted bootstrap/policy
    /// state and local audit projections.
...
impl QuicConnection {
    ...
    pub async fn connect_with_options(
        ...
        // Bind UDP socket
        let socket = Arc::new(
            crate::http::runtime::bind_udp_socket("0.0.0.0:0")
    ...
    pub fn dummy() -> Self {
        ...
        let socket =
            Arc::new(crate::http::runtime::wrap_udp_socket(udp).expect("Cannot wrap test socket"));
        let local_cid = ConnectionId::random();
        let remote_cid = ConnectionId::random();
        let transport = QuicTransport::new(local_cid.clone(), remote_cid.clone());
    ...
    fn update_activity(&mut self) {
        self.runtime_last_activity = crate::http::runtime::time::Instant::now();
    ...
    pub fn from_server(
        socket: crate::http::runtime::net::UdpSocket,
        client_addr: String,
        client_dcid: ConnectionId,
        client_scid: ConnectionId,
        handshake_result: crate::http::http3::quic::server_handshake::ServerHandshakeResult,
    ) -> Result<Self, String> {
        let socket = Arc::new(
            crate::http::runtime::wrap_udp_socket(socket)
    ...
    pub fn set_active_path(
        &mut self,
        local: crate::http::runtime::net::SocketAddr,
        remote: crate::http::runtime::net::SocketAddr,
    ) {
    ...
    pub fn check_expired_path_challenges(&mut self) -> Vec<[u8; 8]> {
        let now = crate::http::runtime::time::Instant::now();
    ...
    pub fn is_idle_timeout(&self) -> bool {
        let timeout = crate::http::runtime::time::Duration::from_millis(
    ...
    pub fn time_until_idle_timeout(&self) -> crate::http::runtime::time::Duration {
        let timeout = crate::http::runtime::time::Duration::from_millis(
            self.transport.params.max_idle_timeout.max(30000),
        );
        let elapsed = self.runtime_last_activity.elapsed();
        if elapsed >= timeout {
            crate::http::runtime::time::Duration::ZERO
        } else {
            timeout - elapsed
        }
    }
