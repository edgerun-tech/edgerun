...
static TASK_QUEUE: crate::rt::sync::Mutex<VecDeque<TaskFuture>> =
    crate::rt::sync::Mutex::new(VecDeque::new());
    ...
    fn new(worker_threads: usize) -> Self {
        ...
        #[cfg(feature = "std")]
        {
            ...
            for index in 0..worker_count {
                ...
                let name = alloc::format!("edgerun-rt-{index}");
                let worker = thread::Builder::new()
                    ...
                    .expect("spawn edgerun runtime worker");
                workers.push(worker);
            }
            return Self {
                worker_count,
...
crate::rt::error::impl_error!(JoinError, |_this, f| { f.write_str("join error") });

struct JoinState<T> {
    result: crate::rt::sync::Mutex<Option<Result<T, JoinError>>>,
    aborted: AtomicBool,
    waiters: crate::rt::sync::Mutex<Vec<Waker>>,
}
...
impl<T> JoinHandle<T> {
    fn pending() -> Self {
        Self {
            state: Arc::new(JoinState {
                result: crate::rt::sync::Mutex::new(None),
                aborted: AtomicBool::new(false),
                waiters: crate::rt::sync::Mutex::new(Vec::new()),
            }),
            waker: None,
        }
    }
    ...
    fn ready(result: Result<T, JoinError>) -> Self {
        Self {
            state: Arc::new(JoinState {
                result: crate::rt::sync::Mutex::new(Some(result)),
                aborted: AtomicBool::new(false),
                waiters: crate::rt::sync::Mutex::new(Vec::new()),
            }),
            waker: None,
...
pub struct JoinSet<F> {
    tasks: crate::rt::sync::Mutex<alloc::collections::VecDeque<JoinHandle<F>>>,
    closed: AtomicBool,
}
...
impl<F> JoinSet<F> {
    pub fn new() -> Self {
        Self {
            tasks: crate::rt::sync::Mutex::new(alloc::collections::VecDeque::new()),
