...
fn run_bind_check(_standard_ports: bool) -> Result<String, String> {
    Err("bind-check requires features: display,dns,dhcp,http,https,acme,quic,proxy,sqlite,smtp,imap".into())
}
...
async fn run_bind_check_async(standard_ports: bool) -> Result<String, String> {
    ...
    let sqlite_path = sqlite_probe_path();
    open_sqlite_probe(&sqlite_path)?;

    let tls_cert = edgerun_tls::generate_self_signed(&["localhost", "127.0.0.1"])
        .map_err(|e| format!("tls certificate: {e}"))?;

    let dns_config = edgerun_server::DnsConfig {
        bind_addr: ports.dns.to_string(),
        bind_addr_ipv6: None,
        default_ttl: 60,
        rate_limit_qps: 100,
    };
    let dhcp_config = edgerun_dhcp::DhcpServerConfig {
    ...
    let dhcp_server = edgerun_dhcp::DhcpServer::new_bound(
        dhcp_config,
    ...
    let _server_dhcp_config = edgerun_server::DhcpConfig::new(
    ...
    let mut smtp_config = edgerun_server::SmtpConfig::default();
    ...
    let mut imap_config = edgerun_server::ImapConfig::default();
    ...
    let mut proxy_config = edgerun_proxy::ProxyConfig::default();
    proxy_config.bind_addr = ports.proxy.to_string();

    let mut bound = edgerun_server::Server::new()
        .with_dns(dns_config)
    ...
    let dhcp_task = edgerun_rt::spawn(async move {
        ...
        Ok::<(), edgerun_dhcp::message::io::Error>(())
    });
    let proxy_server = edgerun_proxy::ProxyServer::new(proxy_config);
    let proxy_shutdown = shutdown.clone();
    let proxy_task = edgerun_rt::spawn(async move { proxy_server.run(proxy_shutdown).await });
    ...
    let report = format!(
        ...
        ports.imap,
        ports.proxy,
        escape_json(&sqlite_path.display().to_string()),
    );
...
fn open_sqlite_probe(path: &PathBuf) -> Result<(), String> {
    let conn = rusqlite::Connection::open(path).map_err(|e| format!("sqlite open: {e}"))?;
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS bind_check (
            key TEXT PRIMARY KEY NOT NULL,
            value TEXT NOT NULL
        );
        INSERT INTO bind_check(key, value)
        VALUES ('status', 'ok')
        ON CONFLICT(key) DO UPDATE SET value = excluded.value;",
    )
    .map_err(|e| format!("sqlite initialize: {e}"))?;
    Ok(())
}

fn sqlite_probe_path() -> PathBuf {
    ...
    let now = SystemTime::now()
        ...
        .unwrap_or(0);
    path.push(format!(
        "edgerund-bind-check-{}-{now}.sqlite3",
        std::process::id()
    ));
    path
}
