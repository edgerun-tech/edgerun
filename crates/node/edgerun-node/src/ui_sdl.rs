...
use edgerun_protocols::seal::{
    generate_seal_key, seal_node_signing_key, unseal_node_signing_key, SealKey,
};
use edgerun_ui_core::gpu::sdl::{instantiate_sdl_app, SdlGlWindowOptions};
use edgerun_ui_core::gpu::{
    build_yubikey_grant_ceremony_surface, ButtonStyle, Color4, HitKind, InitialSetupUiIntent,
    InitialSetupUiState, UiAction, UiAppControl, UiEvent, UiIcon, UiKey, UiNode, UiRect,
    UiSurfaceApp, YubiKeyGrantCeremonyIntent, YubiKeyGrantCeremonyState,
    INITIAL_SETUP_CONFIRM_FIELD_ID, INITIAL_SETUP_CREATE_BUTTON_ID,
    INITIAL_SETUP_PASSWORD_FIELD_ID, YUBIKEY_GRANT_PIN_FIELD_ID, YUBIKEY_GRANT_SIGN_BUTTON_ID,
};
...
use crate::capacity::{format_bytes, NodeCapacity};
...
impl UiSurfaceApp for NodeUiState {
    fn surface(&mut self, viewport: UiRect) -> UiNode {
        build_layout(
            &self.report,
            &self.control,
            &self.setup_ui,
            &self.yubikey_grant_ui,
            self.active_panel,
            viewport.w,
            viewport.h,
            self.scroll_y,
    ...
    fn handle_event(&mut self, event: UiEvent) -> UiAppControl {
        match event {
            UiEvent::KeyDown {
                key: UiKey::Other(key),
            } if key as i32 == SDLK_R || key as i32 == SDLK_R - 32 => {
                self.report = MachineReport::collect();
                UiAppControl::dirty()
            }
            UiEvent::Wheel { delta_y, .. } => {
                self.scroll_y = (self.scroll_y - delta_y * 52.0).max(0.0);
                UiAppControl::dirty()
            }
    ...
    fn handle_action(&mut self, action: UiAction) -> UiAppControl {
        if is_initial_setup_action(&action) {
            return if self.handle_initial_setup_action(&action) {
                UiAppControl::dirty()
            } else {
                UiAppControl::clean()
            };
        }
        if is_yubikey_grant_action(&action) {
            return if self.handle_yubikey_grant_action(&action) {
                UiAppControl::dirty()
            } else {
                UiAppControl::clean()
            };
        ...
        match action {
            UiAction::Activated(hit) if hit.kind == HitKind::MenuItem => {
                let Some(active_panel) = nav_panel_for_id(hit.id) else {
                    return UiAppControl::clean();
                };
            ...
            UiAction::Activated(hit) if hit.kind == HitKind::Button => {
                if hit.id == ACTION_TRUST_RUN_NEXT {
                    return if self.run_next_trust_step() {
                        UiAppControl::dirty()
                    } else {
                        UiAppControl::clean()
                    };
                ...
                if self.control.apply_action(hit.id) {
