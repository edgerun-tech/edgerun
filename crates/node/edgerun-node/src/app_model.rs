//! App/capability runtime projection model.
//!
//! Device and service crates describe capabilities. The node turns each
//! provider descriptor into a runtime app projection. Cross-node capability
//! authority still comes from admitted work; this module only builds the local
//! runtime records used to expose provider capabilities consistently.
...
use crate::runtime::{
    runtime_app_projection_with_capabilities, runtime_capability_declaration, sha256,
};
...
impl CapabilityProviderApp {
    pub fn app_id(&self) -> [u8; 32] {
        self.runtime_projection.app_id
    }

    pub fn provided_capabilities(&self) -> &[RuntimeCapabilityDeclaration] {
        &self.runtime_projection.provided_capabilities
    }
}
...
pub fn capability_provider_app_record(
    ...
    capability_provider_app(descriptor, developer_id).runtime_projection
}
...
mod tests {
    ...
    fn provider_descriptor_becomes_stable_provider_app() {
        ...
        assert_eq!(
            first.runtime_projection.app_id,
            second.runtime_projection.app_id
        );
        assert_eq!(
            first.runtime_projection.release_id,
            second.runtime_projection.release_id
        );
        assert_eq!(first.runtime_projection.provided_capabilities.len(), 2);
        assert!(first.runtime_projection.required_capabilities.is_empty());
        assert!(first.runtime_projection.declared_routes.is_empty());
    }
}
