#![allow(dead_code)]
use super::link_manager::current_unix_secs;
use super::multicast::IpMreq;
use super::multicast::SockaddrIn;
use super::*;
use crate::{FrameType, LocalNode, MeshFrame, MeshFrameHeader, MeshRoute, sign_frame};
...
fn mesh_link_broadcast_fills_src_on_drain() {
    ...
    let mut router = MeshRouter::new(crate::LocalNode::new(src_id));
...
fn mesh_link_pump_with_no_transports_returns_zero() {
    let mut link = MeshLink::new();
    let mut router = MeshRouter::new(crate::LocalNode::new(node_id(0xAA)));
...
fn mesh_link_router_process_discovery_routes_discovery_frames() {
    ...
    let mut router = MeshRouter::new(crate::LocalNode::new(my_id));

    // Build a discovery frame from the peer
    let mut peer_router = MeshRouter::new(crate::LocalNode::new(peer_id));
...
fn mesh_link_inject_frame_not_for_us_is_forwarded() {
    ...
    let mut other_router = MeshRouter::new(crate::LocalNode::new(other_id));
    ...
    let mut router2 = MeshRouter::new(crate::LocalNode::new(src_id));
...
fn mesh_link_send_frame_no_route_returns_false() {
    let mut link = MeshLink::new();
    let router = MeshRouter::new(crate::LocalNode::new(node_id(0xAA)));
...
fn discovery_packet_from_local_advances_sequence() {
    let mut local = crate::LocalNode::new(node_id(0xAA));
    let table = crate::MeshRoutingTable::default();
...
fn mesh_link_drain_frames_fills_src_with_local_id() {
    ...
    let mut router = MeshRouter::new(crate::LocalNode::new(local_id));
...
fn integration_discovery_to_routing() {
    ...
    crate::sign_frame(&mut frame, &signing_key_a);
