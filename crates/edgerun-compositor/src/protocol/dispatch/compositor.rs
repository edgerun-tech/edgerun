...
impl RegionRegistry {
    pub fn new() -> Self {
        Self { regions: std::collections::HashMap::new() }
    }
        ...
        wl_compositor::surface_request::SET_OPAQUE_REGION => {
            ...
            if let Some(surface) = ctx.surfaces.get_mut(ctx.msg.sender_id) {
                if region_id != 0 {
                    ...
                    let rects = ctx.region_registry.get_rects(region_id)
...
pub fn handle_region(ctx: &mut DispatchContext) {
    ...
    match ctx.msg.opcode {
        ...
        wl_compositor::region_request::ADD => {
            ...
            let mut rects = ctx.region_registry.get_rects(region_id)
                .cloned().unwrap_or_default();
            rects.push(rect);
        ...
        wl_compositor::region_request::SUBTRACT => {
            ...
            let w = cursor_obj.int().unwrap_or(0);
            let h = cursor_obj.int().unwrap_or(0);
            let subtract_rect = DamageRect { x, y, width: w, height: h };
...
mod tests {
    ...
    fn test_region_registry_create_add_destroy() {
        ...
        // Add a rect
        reg.set_rects(region_id, vec![DamageRect { x: 0, y: 0, width: 100, height: 100 }]);
    ...
    fn test_region_registry_multiple_rects() {
        ...
        reg.set_rects(region_id, vec![
            DamageRect { x: 0, y: 0, width: 50, height: 50 },
            DamageRect { x: 60, y: 60, width: 40, height: 40 },
        ]);
    ...
    fn test_region_subtract_removes_overlap() {
        ...
        reg.set_rects(region_id, vec![
            DamageRect { x: 0, y: 0, width: 100, height: 100 },
            DamageRect { x: 200, y: 200, width: 50, height: 50 }, // non-overlapping
        ]);

        // Subtract a rect that overlaps the first one
        let subtract = DamageRect { x: 0, y: 0, width: 50, height: 50 };
    ...
    fn test_two_regions_independent() {
        let mut reg = RegionRegistry::new();
        reg.set_rects(1, vec![DamageRect { x: 0, y: 0, width: 10, height: 10 }]);
        reg.set_rects(2, vec![DamageRect { x: 100, y: 100, width: 20, height: 20 }]);
