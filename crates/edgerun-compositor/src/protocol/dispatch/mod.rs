...
pub fn assign_globals(mut global_name: u32) -> (u32, Vec<GlobalDescriptor>) {
    ...
    for &(interface, version) in GLOBALS {
        global_name += 1;
        descriptors.push(GlobalDescriptor {
            global_name,
            interface,
            version,
        });
...
pub fn process_message(
    ...
    match interface.as_str() {
        ...
        "wp_linux_drm_syncobj_manager_v1" => linux_ext::handle_syncobj(&mut ctx),
        "wp_linux_drm_syncobj_surface_v1" => linux_ext::handle_syncobj_surface(&mut ctx),
        "wp_linux_drm_syncobj_timeline_v1" => linux_ext::handle_syncobj_timeline(&mut ctx),
        ...
        "zwlr_primary_selection_manager_v1" => {
            wlroots_ext::handle_primary_selection_manager(&mut ctx)
        }
        "zwlr_primary_selection_device_v1" => {
            wlroots_ext::handle_primary_selection_device(&mut ctx)
        }
        "zwlr_primary_selection_offer_v1" => wlroots_ext::handle_primary_selection_offer(&mut ctx),
        "zwlr_primary_selection_source_v1" => {
            wlroots_ext::handle_primary_selection_source(&mut ctx)
        }
