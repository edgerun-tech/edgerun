...
pub fn handle_seat(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        wl_seat::seat_request::GET_KEYBOARD => {
            ...
            let kb_version = if let Some(res) = ctx
                .client_registries
                .get(&ctx.client_id)
                .and_then(|r| r.get(ctx.msg.sender_id))
            {
                res.version.min(7)
            } else {
                7
            ...
            eprintln!(
                "[edgerun-compositor] Client {} created wl_keyboard id={} (v{})",
                ctx.client_id, kb_id, kb_version
            );
            ...
            let fd = unsafe {
                libc::open(
                    b"/dev/shm/edgerun-km\0".as_ptr() as *const libc::c_char,
            ...
            if fd < 0 {
                ...
                let written =
                    unsafe { libc::write(fd, keymap_str.as_ptr() as *const _, keymap_str.len()) };
                if written < 0 {
                    ...
                    if dup_fd >= 0 {
                        if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                            ...
                            client
                                .send_message(wl_seat::keyboard_repeat_info_event(kb_id, 25, 300));
                            let _ = client.flush();
                        }
                    }
                }
        ...
        wl_seat::seat_request::GET_POINTER => {
            ...
            let ptr_version = if let Some(res) = ctx
                .client_registries
                .get(&ctx.client_id)
                .and_then(|r| r.get(ctx.msg.sender_id))
            {
            ...
            eprintln!(
                "[edgerun-compositor] Client {} created wl_pointer id={} (v{})",
                ctx.client_id, ptr_id, ptr_version
            );
        }
        wl_seat::seat_request::GET_TOUCH => {
            let mut cursor_obj = ArgCursor::from_message(&ctx.msg);
            let touch_id = cursor_obj.new_id().unwrap_or(0);
            let touch_version = if let Some(res) = ctx
                .client_registries
                .get(&ctx.client_id)
                .and_then(|r| r.get(ctx.msg.sender_id))
            {
            ...
            eprintln!(
                "[edgerun-compositor] Client {} created wl_touch id={} (v{})",
                ctx.client_id, touch_id, touch_version
            );
        }
