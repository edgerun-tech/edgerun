...
pub fn handle_screencopy_manager(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        screencopy::manager_request::CAPTURE_OUTPUT => {
            ...
            ctx.screencopy_state.pending_frames.insert(
                frame_id,
                ScreencopyFrame {
                    frame_id,
                    client_id: ctx.client_id,
                    width: ctx.screencopy_state.width,
                    height: ctx.screencopy_state.height,
                    stride: ctx.screencopy_state.stride,
                    format: 0x34325258,
                    region: None,
            ...
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                client.send_message(screencopy::frame_buffer_event(
                    frame_id,
                    0x34325258,
                    ctx.screencopy_state.width,
                    ctx.screencopy_state.height,
                    ctx.screencopy_state.stride,
                ));
                let _ = client.flush();
            }
        }
        screencopy::manager_request::CAPTURE_OUTPUT_REGION => {
            ...
            let capture_w = if w > 0 {
                w as u32
            } else {
                ctx.screencopy_state.width
            };
            let capture_h = if h > 0 {
                h as u32
            } else {
                ctx.screencopy_state.height
            };
...
pub fn handle_screencopy_frame(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        screencopy::frame_request::COPY | screencopy::frame_request::COPY_WITH_DAMAGE => {
            ...
            if let Some(buffer_info) = ctx.buffers.get(buffer_id) {
                if let Some(frame) = ctx.screencopy_state.pending_frames.get(&ctx.msg.sender_id) {
                    ...
                    if !src_pixels.is_empty() && pool_fd >= 0 {
                        ...
                        if buf_ptr != libc::MAP_FAILED {
                            ...
                            for row in 0..copy_h {
                                ...
                                {
                                    dst[dst_row..dst_row + copy_bytes].copy_from_slice(
                                        &src_pixels[src_row..src_row + copy_bytes],
                                    );
                                }
                            }
                    ...
                    if let Some(frame_mut) = ctx
                        .screencopy_state
                        .pending_frames
                        .get_mut(&ctx.msg.sender_id)
                    {
                    ...
                    if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                        client.send_message(screencopy::frame_flags_event(
                            ctx.msg.sender_id,
                            screencopy::frame_flags::Y_INVERT,
                        ));
        ...
        screencopy::frame_request::DESTROY => {
            ...
            ctx.screencopy_state
                .pending_frames
                .remove(&ctx.msg.sender_id);
        }
        _ => {}
    }
}
    ...
    match ctx.msg.opcode {
        primary_selection::manager_request::CREATE_DATA_SOURCE => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(
                    source_id,
                    primary_selection::ZWLR_PRIMARY_SELECTION_SOURCE_V1,
                    1,
                    ctx.client_id,
                );
            }
        ...
        primary_selection::manager_request::GET_PRIMARY_SELECTION => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(
                    device_id,
                    primary_selection::ZWLR_PRIMARY_SELECTION_DEVICE_V1,
                    1,
                    ctx.client_id,
                );
            }
...
pub fn handle_primary_selection_device(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        primary_selection::device_request::SET_SELECTION => {
            ...
            if source_id != 0 {
                *ctx.primary_selection_offer_counter += 1;
                *ctx.current_primary_selection =
                    Some(crate::protocol::dispatch::PrimarySelectionSource {
                        id: source_id,
    ...
    match ctx.msg.opcode {
        ...
        data_control::manager_request::GET_DATA_DEVICE => {
            ...
            if let Some(ref _source) = *ctx.current_data_source {
                ...
                if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                    client.send_message(data_control::data_control_device_data_offer_event(
                        device_id, offer_id,
                    ));
                    client.send_message(data_control::data_control_device_selection_event(
                        device_id, offer_id,
                    ));
                    if let Some(ref source) = *ctx.current_data_source {
                        for mime in &source.mime_types {
                            client.send_message(data_control::data_control_offer_event(
                                offer_id, mime,
                            ));
                        }
                    }
