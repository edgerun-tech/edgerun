...
use super::{DispatchContext, PointerConstraint, ConstraintType};
    ...
    match ctx.msg.opcode {
        zwp_pointer_constraints::constraints_request::LOCK_POINTER => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(locked_id, zwp_pointer_constraints::ZWP_LOCKED_POINTER_V1, 1, ctx.client_id);
            }
            ctx.pointer_constraints.insert(locked_id, PointerConstraint {
                constraint_id: locked_id, surface_id, pointer_id, lifetime,
                region: None, activated: true,
            });
            ctx.constraint_type_map.insert(locked_id, ConstraintType::Lock);
        ...
        zwp_pointer_constraints::constraints_request::CONFINE_POINTER => {
            ...
            let _region = cursor_obj.object().ok();
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(confined_id, zwp_pointer_constraints::ZWP_CONFINED_POINTER_V1, 1, ctx.client_id);
            }
            ctx.pointer_constraints.insert(confined_id, PointerConstraint {
                constraint_id: confined_id, surface_id, pointer_id, lifetime,
                region: None, activated: true,
            });
            ctx.constraint_type_map.insert(confined_id, ConstraintType::Confine);
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
...
pub fn handle_locked_pointer(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        zwp_pointer_constraints::locked_pointer_request::DESTROY => {
            ...
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                client.send_message(zwp_pointer_constraints::locked_pointer_unlocked_event(ctx.msg.sender_id));
            }
        }
...
pub fn handle_confined_pointer(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        zwp_pointer_constraints::confined_pointer_request::DESTROY => {
            ...
            ctx.constraint_type_map.remove(&ctx.msg.sender_id);
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                client.send_message(zwp_pointer_constraints::confined_pointer_unconfined_event(ctx.msg.sender_id));
            }
        }
...
pub fn handle_relative_pointer_manager(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        zwp_relative_pointer::relative_pointer_manager_request::GET_RELATIVE_POINTER => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(relative_pointer_id, "zwp_relative_pointer_v1", 1, ctx.client_id);
            }
