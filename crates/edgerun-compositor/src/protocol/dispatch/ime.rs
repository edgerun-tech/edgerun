    ...
    match ctx.msg.opcode {
        text_input_v3::manager_request::GET_TEXT_INPUT => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(
                    text_input_id,
                    text_input_v3::ZWP_TEXT_INPUT_V3,
                    1,
                    ctx.client_id,
                );
            }
        ...
        text_input_v3::text_input_request::SET_SURROUNDING_TEXT => {
            ...
            if let Ok(Some(text)) = cursor_obj.string() {
                ...
                if let Some(ti) = ctx.text_input_state.as_mut() {
                    ...
                    if let Some(ime) = &ctx.ime_state {
                        if let Some(ime_client) = ctx.server.client_mut(ime.client_id) {
                            ime_client.send_message(
                                input_method_v2::input_method_surround_text_event(
                                    ime.input_method_id,
                                    &ti.surrounding_text,
                                    ti.cursor,
                                    ti.anchor,
        ...
        text_input_v3::text_input_request::SET_TEXT_CHANGE_CAUSE => {
            ...
            if let Some(ti) = ctx.text_input_state.as_mut() {
                ...
                if let Some(ime) = &ctx.ime_state {
                    if let Some(ime_client) = ctx.server.client_mut(ime.client_id) {
                        ime_client.send_message(
                            input_method_v2::input_method_text_change_cause_event(
                                ime.input_method_id,
                                cause,
                            ),
                        );
    ...
    match ctx.msg.opcode {
        input_method_v2::manager_request::GET_INPUT_METHOD => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(
                    im_id,
                    input_method_v2::ZWP_INPUT_METHOD_V2,
                    1,
                    ctx.client_id,
                );
            }
            *ctx.ime_state = Some(IMEState::new(im_id, ctx.client_id));
            eprintln!(
                "[edgerun-compositor] IME server connected, client={}",
                ctx.client_id
            );
        }
    ...
    match ctx.msg.opcode {
        ...
        input_method_v2::input_method_request::GRAB_KEYBOARD => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                reg.register(
                    grab_id,
                    input_method_v2::ZWP_INPUT_METHOD_KEYBOARD_GRAB_V2,
                    1,
                    ctx.client_id,
                );
            }
            if let Some(ime) = ctx.ime_state.as_mut() {
                ...
                let fd = unsafe {
                    libc::open(
                        b"/dev/shm/edgerun-im-km\0".as_ptr() as *const libc::c_char,
                ...
                if fd >= 0 {
                    unsafe {
                        libc::unlink(b"/dev/shm/edgerun-im-km\0".as_ptr() as *const libc::c_char)
                    };
                    let written = unsafe {
                    ...
                    if written >= 0 {
                        ...
                        if dup_fd >= 0 {
                            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                                ...
                                client.send_message(
                                    input_method_v2::keyboard_grab_repeat_info_event(
                                        grab_id, 25, 300,
                                    ),
                                );
                                let _ = client.flush();
