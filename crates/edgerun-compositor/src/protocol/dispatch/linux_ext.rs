...
pub fn handle_dmabuf(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        ...
        linux_dmabuf::dmabuf_request::CREATE_PARAMS => {
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                client.send_message(linux_dmabuf::dmabuf_format_event(
                    ctx.msg.sender_id,
                    linux_dmabuf::dmabuf_format::XRGB8888,
                ));
                client.send_message(linux_dmabuf::dmabuf_format_event(
                    ctx.msg.sender_id,
                    linux_dmabuf::dmabuf_format::ARGB8888,
                ));
    ...
    match ctx.msg.opcode {
        ...
        linux_drm_syncobj::manager_request::GET_SURFACE => {
            ...
            if let Some(reg) = ctx.client_registries.get_mut(&ctx.client_id) {
                ...
                if ctx
                    ...
                    .syncobj_surface_map
                    .values()
                    .any(|&s| s == wl_surface_id)
                {
                    client_error(
                        ctx,
                        linux_drm_syncobj::error::SURFACE_EXISTS,
        ...
        linux_drm_syncobj::manager_request::IMPORT_TIMELINE => {
            ...
            if syncobj_fd < 0 {
                client_error(
                    ctx,
                    linux_drm_syncobj::error::INVALID_TIMELINE,
            ...
            match drm_syncobj::syncobj_import(ctx.drm_fd, syncobj_fd) {
                Ok(handle) => {
                    ctx.shell.syncobj_state.timelines.insert(
                        timeline_id,
                        SyncobjTimeline {
                            handle,
                            current_point: 0,
                ...
                Err(e) => {
                    eprintln!(
                        "[edgerun-compositor] Failed to import syncobj timeline: {}",
                        e
                    );
                    client_error(
                        ctx,
