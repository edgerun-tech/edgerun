...
pub fn handle_shm(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        wl_shm::shm_request::CREATE_POOL => {
            ...
            let client_pools = ctx.client_pool_map.entry(ctx.client_id).or_insert_with(std::collections::HashMap::new);
...
pub fn handle_shm_pool(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        wl_shm::shm_pool_request::CREATE_BUFFER => {
            ...
            let pool_fd = ctx.shm.pool_fd_by_client_id(ctx.msg.sender_id).unwrap_or(-1);

            ctx.buffers.register(buffer_id, ShmBufferInfo {
                pool_fd, offset, width, height, stride, format,
            });
