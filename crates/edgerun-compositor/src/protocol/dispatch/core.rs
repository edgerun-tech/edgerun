...
pub fn handle_registry(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        wl_core::registry_request::BIND => {
            ...
            eprintln!("[edgerun-compositor] Client {} binding {} (v{}) -> id={}", ctx.client_id, interface_name, version, id);
            ...
            match interface_name.as_str() {
                ...
                "wl_seat" => {
                    ...
                    ctx.seat.id = id;
                    ctx.seat.capabilities = wl_seat::capability::KEYBOARD | wl_seat::capability::POINTER | wl_seat::capability::TOUCH;
                    if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                        client.send_message(wl_seat::seat_capabilities_event(id, ctx.seat.capabilities));
                        if version >= 7 {
                ...
                "wl_output" => {
                    ...
                    if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                        ...
                        client.send_message(wire::Message {
                            sender_id: id, opcode: 0,
                            size: (8 + geo_args.len()) as u16,
                            args: geo_args, fds: Vec::new(),
                        });
                        ...
                        if version >= 4 {
                            client.send_message(wire::Message {
                                sender_id: id, opcode: 4, size: 8,
                                args: Vec::new(), fds: Vec::new(),
                            });
                        }
                        let _ = client.flush();
                    }
                ...
                "wp_cursor_shape_manager_v1" => {
                    ctx.client_cursor_shape_manager_ids.insert(ctx.client_id, id);
                }
                "xdg_activation_v1" => {}
                "wp_presentation" => {}
