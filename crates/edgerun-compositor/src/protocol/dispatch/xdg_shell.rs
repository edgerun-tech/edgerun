...
pub fn handle_wm_base(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        xdg_shell::xdg_wm_base_request::GET_XDG_SURFACE => {
            ...
            println!(
                "[edgerun-compositor] xdg_surface created: id={} wl_surface={}",
        ...
        xdg_shell::xdg_wm_base_request::CREATE_POSITIONER => {
            ...
            ctx.shell
                .set_positioner(pos_id, crate::compositor::shell::PositionerState::default());
        }
...
pub fn handle_surface(ctx: &mut DispatchContext) {
    match ctx.msg.opcode {
        xdg_shell::xdg_surface_request::GET_TOPLEVEL => {
            ...
            let wl_surface_id = ctx
                .xdg_surface_to_wl_surface
                .get(&ctx.msg.sender_id)
                .copied()
                .unwrap_or(ctx.msg.sender_id);
            ctx.shell.create_toplevel(toplevel_id, wl_surface_id);

            println!(
                "[edgerun-compositor] xdg_toplevel created: toplevel_id={} wl_surface={}",
                toplevel_id, wl_surface_id
            );
            ...
            *ctx.config_serial += 1;
            if let Some(client) = ctx.server.client_mut(ctx.client_id) {
                ...
                client.send_message(xdg_shell::xdg_toplevel_configure_bounds_event(
                    toplevel_id,
                    ctx.shell.output_width,
                    ctx.shell.output_height,
                ));
                ...
                client.send_message(xdg_shell::xdg_toplevel_wm_capabilities_event(
                    toplevel_id,
                    caps,
                ));
                let _ = client.flush();
        ...
        xdg_shell::xdg_surface_request::GET_POPUP => {
            ...
            let parent_surface = parent_id.and_then(|pid| {
                ctx.shell
                    ...
                    .values()
                    .find(|tl| tl.id == pid)
                    .map(|tl| tl.surface_id)
            });
            ctx.shell
                .create_popup(popup_id, ctx.msg.sender_id, parent_surface);
            ...
            if let Some(pos_id) = positioner_id {
                if let Some(pos) = ctx.shell.positioners.get(&pos_id) {
                    ctx.shell.set_popup_positioner(
                        ...
                        pos.anchor_rect_x,
                        pos.anchor_rect_y,
                        pos.anchor_rect_width,
                        pos.anchor_rect_height,
                        pos.anchor,
                        pos.gravity,
                        pos.offset_x,
                        pos.offset_y,
        ...
        xdg_shell::xdg_surface_request::ACK_CONFIGURE => {
            ...
            if let Some(tl) = ctx
                ...
                .toplevels
                .values_mut()
                .find(|tl| tl.surface_id == ctx.msg.sender_id)
            {
        ...
        xdg_shell::xdg_surface_request::SET_WINDOW_GEOMETRY => {
                ...
                .find(|tl| tl.surface_id == ctx.msg.sender_id)
            {
                ...
                tl.window_height = h;
                eprintln!(
                    "[edgerun-compositor] Window geometry for toplevel {}: x={} y={} w={} h={}",
                    tl.id, x, y, w, h
                );
            }
            if let Some(surface) = ctx.surfaces.get_mut(ctx.msg.sender_id) {
                ...
                surface.y = y;
                if w > 0 {
                    surface.width = w as u32;
                }
                if h > 0 {
                    surface.height = h as u32;
                }
            }
        }
    ...
    match ctx.msg.opcode {
        xdg_shell::xdg_toplevel_request::SET_TITLE => {
            ...
            if let Ok(Some(title)) = cursor_obj.string() {
                if let Some(tl) = ctx
                    ...
                    .toplevels
                    .values_mut()
                    .find(|tl| tl.id == ctx.msg.sender_id)
                {
        ...
        xdg_shell::xdg_toplevel_request::SHOW_WINDOW_MENU => {
            ...
            if let Some(tl) = ctx.shell.toplevels.get(&ctx.msg.sender_id) {
                eprintln!(
                    "[edgerun-compositor] SHOW_WINDOW_MENU for toplevel {} at ({}, {})",
                    tl.id, x, y
                );
            }
        ...
        xdg_shell::xdg_toplevel_request::MINIMIZE
        ...
        xdg_shell::xdg_toplevel_request::SET_FULLSCREEN => {
            ...
            if let Some(tl_id) = toplevel_id {
                ...
                let serial = ctx.shell.configure_toplevel_with_state(
                    tl_id,
                    ctx.shell.output_width,
                    ctx.shell.output_height,
                );
        ...
        xdg_shell::xdg_toplevel_request::MAXIMIZE
        ...
        xdg_shell::xdg_toplevel_request::UNMAXIMIZE
        ...
        xdg_shell::xdg_toplevel_request::MOVE => {
            ...
            if let Some(tl) = ctx.shell.toplevels.get(&ctx.msg.sender_id) {
                eprintln!(
                    "[edgerun-compositor] Toplevel {} initiated interactive move",
                    tl.id
                );
            }
        }
        xdg_shell::xdg_toplevel_request::RESIZE => {
            ...
            if let Some(tl) = ctx.shell.toplevels.get_mut(&ctx.msg.sender_id) {
                tl.resizing = true;
                eprintln!(
                    "[edgerun-compositor] Toplevel {} initiated interactive resize (edges={})",
                    tl.id, edges
                );
            }
        }
        xdg_shell::xdg_toplevel_request::SET_PARENT => {
            ...
            if let Some(tl) = ctx.shell.toplevels.get_mut(&ctx.msg.sender_id) {
                tl.parent = parent_id;
                eprintln!(
                    "[edgerun-compositor] Toplevel {} set parent to {:?}",
                    tl.id, parent_id
                );
            }
        }
