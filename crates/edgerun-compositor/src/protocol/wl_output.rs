...
pub fn output_geometry_event(
    ...
    args.extend_from_slice(&physical_w.to_le_bytes());
    args.extend_from_slice(&physical_h.to_le_bytes());
    args.extend_from_slice(&subpixel.to_le_bytes());
    crate::wire::encode::encode_string(&mut args, make);
    crate::wire::encode::encode_string(&mut args, model);
    args.extend_from_slice(&transform.to_le_bytes());
    Message {
        sender_id: output_id,
        opcode: GEOMETRY,
...
/// Build a mode event.
pub fn output_mode_event(output_id: u32, flags: u32, width: i32, height: i32, refresh: i32) -> Message {
    let mut args = Vec::new();
...
pub fn output_description_event(output_id: u32, desc: &str) -> Message {
    let mut args = Vec::new();
    crate::wire::encode::encode_string(&mut args, desc);
    Message {
        sender_id: output_id,
        opcode: DESCRIPTION,
        size: (8 + args.len()) as u16,
