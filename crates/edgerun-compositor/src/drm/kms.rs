...
pub fn read_events(fd: RawFd) -> io::Result<Vec<(u32, u64)>> {
    ...
    loop {
        ...
        let mut hdr = [0u8; 8];
        match nix_read_exact(fd, &mut hdr) {
            Ok(_n) => {}
            Err(e) if e.kind() == io::ErrorKind::WouldBlock => break, // drained
            Err(e) => return Err(e),
        }
        ...
        if length < 8 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "DRM event too short"));
        }

        // Read rest of event
        let mut rest = vec![0u8; length - 8];
        nix_read_exact(fd, &mut rest)?;
...
/// Non-blocking read that returns 0 if no data available.
fn nix_read_exact(fd: RawFd, buf: &mut [u8]) -> io::Result<usize> {
    let ret = unsafe {
    ...
    if ret == 0 {
        return Ok(0); // DRM returns 0 when no events — this is normal
    }
    Ok(ret as usize)
}
