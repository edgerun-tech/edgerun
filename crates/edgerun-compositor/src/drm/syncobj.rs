...
pub fn syncobj_create(fd: RawFd, flags: u32) -> io::Result<u32> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_CREATE, &mut create) };
    if ret < 0 {
...
pub fn syncobj_destroy(fd: RawFd, handle: u32) -> io::Result<()> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_DESTROY, &mut destroy) };
    if ret < 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(())
...
pub fn import_sync_file(fd: RawFd, syncobj_handle: u32, sync_file_fd: c_int) -> io::Result<()> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_IMPORT_SYNC_FILE, &mut import) };
    if ret < 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(())
...
pub fn export_sync_file(fd: RawFd, syncobj_handle: u32) -> io::Result<RawFd> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_EXPORT_SYNC_FILE, &mut export) };
    if ret < 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(export.fd)
...
pub fn timeline_signal(fd: RawFd, syncobj_handle: u32, point: u64) -> io::Result<()> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_TIMELINE_SIGNAL, &mut signal) };
    if ret < 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(())
...
pub fn syncobj_import(fd: RawFd, syncobj_fd: RawFd) -> io::Result<u32> {
    ...
    let ret = unsafe { libc::ioctl(fd, DRM_IOCTL_SYNCOBJ_FD_TO_HANDLE, &mut import) };
    if ret < 0 {
        Err(io::Error::last_os_error())
    } else {
        Ok(import.handle)
