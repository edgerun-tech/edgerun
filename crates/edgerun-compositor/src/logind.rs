...
pub fn logind_acquire_drm_fd(sysname: &str) -> io::Result<OwnedFd> {
    ...
    let (fd, _result) = dbus.call_method_with_fds_reply(
        ...
        b"uub",
        &[
            (major as u32).to_le_bytes().to_vec(),
            (minor as u32).to_le_bytes().to_vec(),
            vec![1u8],
        ],
    )?;
...
impl DbusConn {
    ...
    fn hello(&self) -> io::Result<String> {
        ...
        if reply.is_empty() || reply[0] != 2 {
            ...
            return Err(io::Error::new(
                io::ErrorKind::Other,
                "D-Bus Hello: unexpected reply type",
            ));
        }
        ...
        if reply[body_offset] != 0x73 {
            return Err(io::Error::new(
                ...
                "Expected string in Hello reply",
            ));
        }
    ...
    fn call_method_with_fds_reply(
        ...
        let (msg, _fds_out) = build_method_call_with_fds(
            destination,
            path,
            interface,
            member,
        ...
        if reply.is_empty() || reply[0] != 2 {
            ...
            if reply.len() > 0 && reply[0] == 3 {
                ...
                if body_offset + 5 < reply.len() && reply[body_offset] == 0x73 {
                    ...
                    return Err(io::Error::new(
                        io::ErrorKind::Other,
                        format!("D-Bus error: {}", error_name),
                    ));
                }
            }
            return Err(io::Error::new(
                io::ErrorKind::Other,
                "Unexpected D-Bus reply type",
            ));
        }
...
fn connect_unix_socket(addr: &str) -> io::Result<OwnedFd> {
    if !addr.starts_with("unix:path=") {
        return Err(io::Error::new(
            ...
            "Only unix:path D-Bus addresses supported",
        ));
    }
    ...
    loop {
        ...
        if pos >= msg.len() || pos % 8 == 0 {
            ...
            if body_len > 0 && msg.len() >= aligned_start + body_len {
                ...
                if next_aligned < msg.len()
                    && (next_aligned + body_len <= msg.len() || body_len == 0)
                {
                    return next_aligned;
                }
                // Fallback: return pos
        ...
        match type_code {
            ...
            2 => {
                ...
                pos += 4;
            } // BOOLEAN (actually stored as UINT32)
            3..=5 => {
                align_to_4_at(&mut pos);
                pos += 4;
            } // INT16, UINT16, INT32
            ...
            8 => {
                ...
                if pos + 4 <= msg.len() {
                    let slen =
                        u32::from_le_bytes([msg[pos], msg[pos + 1], msg[pos + 2], msg[pos + 3]])
                            as usize;
...
fn recv_ancillary_fds(fd: RawFd) -> io::Result<Vec<OwnedFd>> {
    ...
    while !cmsg_ptr.is_null() {
        ...
        if cmsg.cmsg_level == libc::SOL_SOCKET && cmsg.cmsg_type == libc::SCM_RIGHTS {
            ...
            let fd_ptr = unsafe { libc::CMSG_DATA(cmsg as *const libc::cmsghdr) as *const RawFd };
            for i in 0..num_fds {
...
fn get_device_major_minor(sysname: &str) -> io::Result<(u32, u32)> {
    ...
    if parts.len() != 2 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("Invalid dev content: {}", content),
        ));
    }
    let major = parts[0]
        .parse::<u32>()
