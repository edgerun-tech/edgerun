...
pub fn send_with_fds(fd: RawFd, data: &[u8], fds: &[RawFd]) -> io::Result<usize> {
    ...
    if !fds.is_empty() {
        let cmsg_data_len =
            std::mem::size_of::<libc::cmsghdr>() + fds.len() * std::mem::size_of::<RawFd>();
        ...
        let cmsg_len =
            unsafe { CMSG_LEN((fds.len() * std::mem::size_of::<RawFd>()) as libc::c_uint) };
        eprintln!(
            "[edgerun-compositor] send_with_fds: cmsg_len={}, cmsg_space={}, fds={:?} data_len={}",
            cmsg_len,
            cmsg_space,
            fds,
            data.len()
    ...
    if ret < 0 {
        ...
        eprintln!(
            "[edgerun-compositor] sendmsg failed: errno={} ({})",
