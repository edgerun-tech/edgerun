...
const WATCH_MASK: [bool; 576] = {
    ...
    while i < 576 {
        ...
        m[i] = (dist_sq >= 36 && dist_sq <= 64) || (x >= 10 && x <= 12 && y >= 10 && y <= 12);
        i += 1;
    }
    m
};
...
const CROSSHAIR_MASK: [bool; 576] = {
    ...
    while i < 576 {
        ...
        let y = i / 24;
        m[i] =
            (x >= 11 && x <= 12 && y >= 2 && y <= 21) || (x >= 2 && x <= 21 && y >= 11 && y <= 12);
        i += 1;
    }
    m
};
...
const NOT_ALLOWED_MASK: [bool; 576] = {
    ...
    while i < 576 {
        ...
        let dy = y as i32 - 11;
        let dist_sq = dx * dx + dy * dy;
        m[i] = (dist_sq >= 36 && dist_sq <= 64) || (x + y >= 16 && x + y <= 22);
        i += 1;
    }
    m
};
...
const CIRCLE_MINUS_MASK: [bool; 576] = {
    ...
    while i < 576 {
        ...
        let dy = y as i32 - 11;
        let dist_sq = dx * dx + dy * dy;
        m[i] = (dist_sq >= 36 && dist_sq <= 64) || (x >= 4 && x <= 18 && y >= 10 && y <= 12);
        i += 1;
    }
    m
};
...
impl Cursor {
    ...
    pub fn draw(&self, pixels: &mut [u8], fb_width: u32, fb_height: u32, fb_stride: u32) {
        if !self.visible {
            return;
        }
        ...
        for cy in 0..CURSOR_SIZE {
            for cx in 0..CURSOR_SIZE {
                ...
                if alpha == 255 {
                    ...
                    pixels[dst_off] = ((bitmap[src_off] as u32 * alpha as u32
                        + pixels[dst_off] as u32 * a_inv)
                        / 255) as u8;
                    pixels[dst_off + 1] = ((bitmap[src_off + 1] as u32 * alpha as u32
                        + pixels[dst_off + 1] as u32 * a_inv)
                        / 255) as u8;
                    pixels[dst_off + 2] = ((bitmap[src_off + 2] as u32 * alpha as u32
