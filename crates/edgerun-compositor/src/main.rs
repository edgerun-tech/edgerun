...
//!
//! Usage: `edgerun-compositor [--no-vt] [socket_path]`
//!
...
fn main() {
    let mut socket_path = "/tmp/edgerun-wayland-0".to_string();
    let mut enable_vt = std::env::var_os("EDGERUN_COMPOSITOR_NO_VT").is_none();
    ...
    let drm_device = match DrmDevice::open(&devices[0]) {
        ...
        Err(e) => {
            eprintln!(
                "[edgerun-compositor] Failed to open DRM device {}: {}",
                devices[0], e
            );
            std::process::exit(1);
        }
    ...
    let mut vt_manager = if enable_vt {
        match VtManager::open(0) {
            Ok(vt) => {
                println!(
                    "[edgerun-compositor] VT{} acquired — running as standalone compositor",
                    vt.vt_num()
                );
                Some(vt)
            }
        ...
        eprintln!("[edgerun-compositor] VT management disabled");
        None
    };
    ...
    println!(
        "[edgerun-compositor] {} connectors, {} CRTCs",
    ...
    let mut dumb = match DumbBuffer::create(
    ...
    let mut gl_compositor = GlCompositor::new(
        drm_device.as_raw_fd(),
        mode.hdisplay as u32,
        mode.vdisplay as u32,
    );
    ...
    shell.set_output_size(
        mode.hdisplay as i32,
        mode.vdisplay as i32,
        (mode.vrefresh * 1000) as i32,
        conn.mm_width as i32,
        conn.mm_height as i32,
        output_scale,
    );
    ...
    event_loop
        .add_read(server.listen_fd(), EventSource::WaylandListen)
    ...
    for dev_id in input_mgr.device_ids().collect::<Vec<_>>() {
        if let Some(fd) = input_mgr.device_fd(dev_id) {
            if let Err(e) = event_loop.add_read(fd, EventSource::EvdevDevice(dev_id)) {
                eprintln!(
                    "[edgerun-compositor] Failed to register evdev fd for device {dev_id}: {e}"
                );
            }
        }
    }
    ...
    if let Err(e) =
        event_loop.add_read_edge_triggered(drm_device.as_raw_fd(), EventSource::DrmDevice)
    {
    ...
    loop {
        ...
        if events.is_empty() && pending_render && !flip_pending {
            ...
            for surface in surfaces.surfaces() {
                shell
                    .syncobj_state
                    .wait_acquire(drm_device.as_raw_fd(), surface.id);
            }
            ...
            if let Some(gl) = gl_compositor.as_mut() {
                ...
                if let Ok(mapped) = dumb.map() {
                    let pixels =
                        unsafe { std::slice::from_raw_parts(mapped.as_ptr(), mapped.len()) };
                ...
                match kms::page_flip(
                    ...
                    Err(e) => {
                        ...
                        eprintln!("[edgerun-compositor] GPU page flip failed: {e}");
                        flip_pending = false;
                        pending_render = true;
                    }
                }
                ...
                match render_and_flip(
                    ...
                    Err(e) => {
                        eprintln!("[edgerun-compositor] Software page flip failed: {e}");
                        flip_pending = false;
                        pending_render = true;
                    }
                }
            ...
            match source {
                EventSource::WaylandListen => {
                    if evmask & (libc::EPOLLIN as u32) != 0 {
                        if let Ok(Some(client_id)) = server.accept_client() {
                            ...
                            event_loop
                                .add_read(client_fd, EventSource::WaylandClient(client_id))
                            ...
                            if let Some(client) = server.client_mut(client_id) {
                                match client.flush() {
                                    Ok(()) => println!(
                                        "[edgerun-compositor] globals flushed to client {}",
                                        client_id
                                    ),
                    ...
                    if evmask & (libc::EPOLLIN as u32) != 0 {
                        if let Some(client) = server.client_mut(client_id) {
                            if let Err(e) = client.recv() {
                                if e.kind() != io::ErrorKind::WouldBlock {
                                    println!(
                                        "[edgerun-compositor] Client {client_id} read error: {e}"
                                    );
                        ...
                        for msg in messages {
                            dispatch::process_message(
                                ...
                                &mut region_registry,
                                &mut client_registries,
                                &mut client_registry_ids,
                                &mut client_compositor_ids,
                                &mut client_shm_ids,
                                &mut client_seat_ids,
                                &mut client_xdg_base_ids,
                                &mut client_output_ids,
                                &mut client_keyboard_ids,
                                &mut client_pointer_ids,
                                &mut client_touch_ids,
                                &mut client_dmabuf_ids,
                                &mut client_pool_map,
                                &mut client_data_device_ids,
                                &mut client_data_source_ids,
                                &mut client_subcompositor_ids,
                                &mut client_decoration_manager_ids,
                                &mut client_decoration_ids,
                                &mut client_viewporter_ids,
                                &mut client_cursor_shape_manager_ids,
                                &mut client_cursor_shape_device_ids,
                                &mut client_cursor_surfaces,
                                &mut client_relative_pointer_ids,
                                &mut xdg_surface_to_wl_surface,
                                &mut dmabuf_pending,
                                &mut current_data_source,
                                &mut selection_offer_counter,
                                &mut config_serial,
                ...
                EventSource::DrmDevice => {
                    ...
                    for surface in surfaces.surfaces() {
                        shell
                            .syncobj_state
                            .signal_release(drm_device.as_raw_fd(), surface.id);
                    }
                }
