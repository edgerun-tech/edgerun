...
fn emit_memory_fill(code: &mut Vec<u8>) -> Result<()> {
    ...
    emit_zero_extend_eax(code);
    code.extend_from_slice(&[0x89, 0xD2]); // mov edx, edx
    code.extend_from_slice(&[0x80, 0xE1, 0xFF]); // and cl, 0xff
    code.extend_from_slice(&[0x4D, 0x8D, 0x04, 0x02]); // lea r8, [r10 + rax]
    let loop_start = code.len();
    code.extend_from_slice(&[0x48, 0x85, 0xD2]); // test rdx, rdx
    code.extend_from_slice(&[0x0F, 0x84]); // jz done
    let done_patch = code.len();
    code.extend_from_slice(&0i32.to_le_bytes());
    code.extend_from_slice(&[0x41, 0x88, 0x08]); // mov [r8], cl
    code.extend_from_slice(&[0x49, 0xFF, 0xC0]); // inc r8
    code.extend_from_slice(&[0x48, 0xFF, 0xCA]); // dec rdx
    emit_jmp_rel32(code, loop_start)?;
    let done = code.len();
...
mod tests {
    ...
    fn x86_supports_memory_load_store_encoding() {
        let mut ir = test_ir(
            ...
            vec![
                IrOp::LocalGet(0),
                IrOp::I32Const(123),
                IrOp::Store(StoreKind::I32, MemOp { offset: 4 }),
                IrOp::LocalGet(0),
                IrOp::Load(LoadKind::I32, MemOp { offset: 4 }),
                IrOp::End,
            ],
        );
        ir.uses_memory = true;
    ...
    #[test]
    fn x86_supports_memory_copy_encoding() {
        let mut ir = test_ir(
            "memcpy",
            FuncSig {
                params: vec![ValueType::I32, ValueType::I32, ValueType::I32],
        ...
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
        assert!(code.windows(3).any(|w| w == [0x41, 0x8A, 0x01]));
        assert!(code.windows(3).any(|w| w == [0x41, 0x88, 0x00]));
    }
    #[test]
    fn x86_supports_memory_fill_encoding() {
        let mut ir = test_ir(
            "memfill",
            FuncSig {
                params: vec![ValueType::I32, ValueType::I32, ValueType::I32],
        ...
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
        assert!(code.windows(3).any(|w| w == [0x41, 0x88, 0x08]));
    }
}
