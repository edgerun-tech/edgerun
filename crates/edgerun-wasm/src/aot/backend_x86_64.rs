...
fn local_type(ir: &FunctionIr, index: u32) -> Result<ValueType> {
    ir.locals.get(index as usize).copied().ok_or_else(|| anyhow::anyhow!("local index {} outside function frame", index))
}
...
fn emit_load(code: &mut Vec<u8>, kind: LoadKind, mem: MemOp) -> Result<()> {
    ...
    if matches!(kind, LoadKind::I32) { emit_zero_extend_eax(code); }
    code.push(0x50);
    Ok(())
}
...
fn emit_select(code: &mut Vec<u8>, ty: ValueType) {
    code.push(0x58); // pop rax = condition
    code.push(0x59); // pop rcx = false value
    code.push(0x5A); // pop rdx = true value
    code.extend_from_slice(&[0x85, 0xC0]); // test eax, eax
    code.extend_from_slice(&[0x48, 0x0F, 0x45, 0xCA]); // cmovne rcx, rdx
    if ty == ValueType::I32 {
        code.extend_from_slice(&[0x89, 0xC9]); // mov ecx, ecx
    }
    code.push(0x51); // push selected value
}
...
fn emit_return(code: &mut Vec<u8>, result: Option<ValueType>) {
    if let Some(ty) = result { code.push(0x58); if ty == ValueType::I32 { emit_zero_extend_eax(code); } }
...
mod tests {
    ...
    fn test_ir(name: &str, sig: FuncSig, locals: Vec<ValueType>, ops: Vec<IrOp>) -> FunctionIr {
        FunctionIr { index: 0, export_name: Some(name.to_string()), sig, locals, global_values: Vec::new(), uses_memory: false, ops }
    }

    #[test]
    fn x86_add_two_i32_params() {
        let ir = test_ir("add", FuncSig { params: vec![ValueType::I32, ValueType::I32], results: vec![ValueType::I32] }, vec![ValueType::I32, ValueType::I32], vec![IrOp::LocalGet(0), IrOp::LocalGet(1), IrOp::I32Add, IrOp::End]);
        verify_ir(&ir).unwrap();
    ...
    fn x86_supports_mutable_local() {
        let ir = test_ir("mut_local", FuncSig { params: vec![ValueType::I64], results: vec![ValueType::I64] }, vec![ValueType::I64, ValueType::I64], vec![IrOp::LocalGet(0), IrOp::I64Const(7), IrOp::I64Add, IrOp::LocalSet(1), IrOp::LocalGet(1), IrOp::End]);
        verify_ir(&ir).unwrap();
        assert!(!X86_64Backend::compile(&ir).unwrap().is_empty());
    }

    #[test]
    fn x86_supports_i32_comparison() {
        let ir = test_ir("lt", FuncSig { params: vec![ValueType::I32, ValueType::I32], results: vec![ValueType::I32] }, vec![ValueType::I32, ValueType::I32], vec![IrOp::LocalGet(0), IrOp::LocalGet(1), IrOp::I32LtS, IrOp::End]);
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
    ...
    fn x86_supports_global_set_then_get() {
        let mut ir = test_ir("global_set", FuncSig { params: vec![], results: vec![ValueType::I32] }, vec![], vec![IrOp::I32Const(99), IrOp::GlobalSet(0, ValueType::I32), IrOp::GlobalGet(0, ValueType::I32), IrOp::End]);
    ...
    fn x86_supports_br_if_depth0_to_block_end() {
        let ir = test_ir("branch", FuncSig { params: vec![ValueType::I32], results: vec![ValueType::I32] }, vec![ValueType::I32], vec![IrOp::Block, IrOp::LocalGet(0), IrOp::BrIf(0), IrOp::I32Const(7), IrOp::LocalSet(0), IrOp::BlockEnd, IrOp::LocalGet(0), IrOp::End]);
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
    ...
    #[test]
    fn x86_supports_memory_load_store_encoding() {
        let mut ir = test_ir("memory", FuncSig { params: vec![ValueType::I32], results: vec![ValueType::I32] }, vec![ValueType::I32], vec![IrOp::LocalGet(0), IrOp::I32Const(123), IrOp::Store(StoreKind::I32, MemOp { offset: 4 }), IrOp::LocalGet(0), IrOp::Load(LoadKind::I32, MemOp { offset: 4 }), IrOp::End]);
        ir.uses_memory = true;
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
        assert!(code.windows(4).any(|w| w == [0x41, 0x8B, 0x04, 0x02]));
        assert!(code.windows(4).any(|w| w == [0x41, 0x89, 0x0C, 0x02]));
    }

    #[test]
    fn x86_supports_select_i32() {
        let ir = test_ir("select", FuncSig { params: vec![ValueType::I32, ValueType::I32, ValueType::I32], results: vec![ValueType::I32] }, vec![ValueType::I32, ValueType::I32, ValueType::I32], vec![IrOp::LocalGet(0), IrOp::LocalGet(1), IrOp::LocalGet(2), IrOp::Select(ValueType::I32), IrOp::End]);
        verify_ir(&ir).unwrap();
        let code = X86_64Backend::compile(&ir).unwrap();
        assert!(code.windows(4).any(|w| w == [0x48, 0x0F, 0x45, 0xCA]));
    }
}
