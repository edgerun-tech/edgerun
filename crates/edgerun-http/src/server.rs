...
/// Unified HTTP server.
///
/// Listens on TCP and auto-detects HTTP/1.1 vs HTTP/2 on each connection.
/// Dispatches all requests to a single [`Handler`] implementation.
///
/// # Example
///
/// ```no_run
...
/// Handle a single connection — auto-detect HTTP/1.1 vs HTTP/2.
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    // HTTP/2 starts with the connection preface: "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n"
    // HTTP/1.x starts with a method like "GET / HTTP/1.1"
    ...
    if first_line.as_bytes().starts_with(H2_PREFACE) {
        // HTTP/2 connection preface detected
        let stream = reader.into_inner();
        handle_http2(stream, handler).await
    } else {
        // HTTP/1.x — process the first line as a request line
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    loop {
        ...
        // Keep-alive: read next request line with timeout
        current_line = match timeout(ka_timeout, reader.read_line()).await {
            Ok(Ok(Some(line))) => line,
            _ => break,
        };
    }
...
///
/// Reads the HTTP/2 frame stream, decodes HEADERS into Request via HPACK,
/// calls the Handler, and encodes the Response back as HTTP/2 frames.
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    // Read the remaining 24-byte preface (we already consumed "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n")
    // Actually we already read the preface line. h2spec sends the preface as the very first bytes.
    // Since BufReader may have buffered it, let's just proceed to read the SETTINGS frame.
    ...
    let mut expecting_continuation: Option<(u32, Vec<u8>)> = None; // (stream_id, header_block)
        ...
        if frame.frame_type == FrameType::Settings {
            ...
            match server.apply_client_settings(&settings_frame) {
                FrameAction::WriteFrames(frames) => {
                    ...
                    // Update our decoder with client's table size
        ...
        let action = match frame.frame_type {
            ...
            FrameType::Priority => {
                let pf = if frame.payload.len() >= 5 {
                    ...
                    crate::http2::frame::PriorityFrame::new(frame.stream_id, exclusive, stream_dependency, weight)
                } else {
            ...
            FrameType::Data => {
                // Data on a stream — if we have pending headers, buffer; otherwise ignore
                // For h2spec, we just acknowledge data and keep going
                FrameAction::None
            }
            FrameType::Headers => {
                ...
                if hf.end_stream && end_headers {
                    // Complete request — decode headers and call handler
                    process_complete_request(hf.stream_id, &hf.header_block, &mut decoder, &mut encoder, &mut server, &handler).await
                } else if end_headers && !hf.end_stream {
                    // Headers with body — validate and store pending
                    ...
                    expecting_contin = Some((hf.stream_id, hf.header_block.clone()));
                    FrameAction::None
                } else {
                    FrameAction::None
                }
            ...
            FrameType::Continuation => {
                if let Some((stream_id, ref mut block)) = expecting_contin {
                    ...
                    if end_headers {
                        ...
                        process_complete_request(stream_id, &block, &mut decoder, &mut encoder, &mut server, &handler).await
                    } else {
                        FrameAction::None
                    }
                } else {
...
async fn process_complete_request<S>(
    stream_id: u32,
    header_block: &[u8],
    decoder: &mut Decoder,
    encoder: &mut Encoder,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    let headers = match decoder.decode(header_block) {
        Ok(h) => h,
        Err(_) => return FrameAction::None, // Will be handled by caller via goaway
    };

    if let Err((ec, _)) = validate_request_headers(&headers) {
        return FrameAction::WriteFrames(vec![
            crate::http2::frame::RstStreamFrame::new(stream_id, ec).to_frame(),
        ]);
    }
    ...
    // Convert HTTP/2 pseudo-headers to our Request type
    ...
    // Collect regular headers (skip pseudo-headers)
    ...
    // Encode response as HTTP/2 HEADERS + DATA
    ...
    resp_frames.push(crate::http2::frame::HeadersFrame::new(stream_id, header_block, response.body().is_empty()).to_frame());

    // DATA frame for body
    if !response.body().is_empty() {
        resp_frames.push(crate::http2::frame::DataFrame::new(stream_id, response.body().to_vec(), true).to_frame());
    }
...
/// Read a 9-byte header + payload frame from the wire.
...
where
    S: AsyncWrite + Unpin,
{
    let goaway = crate::http2::frame::GoawayFrame::new(last_stream_id, error_code, debug.to_vec());
...
where
    S: AsyncWrite + Unpin,
{
    let rst = crate::http2::frame::RstStreamFrame::new(stream_id, error_code);
