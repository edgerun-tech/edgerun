...
pub use edgerun_bare_rt::sync::Mutex;
...
pub fn bare_io(error: edgerun_bare_rt::IoError) -> io::Error {
    match error {
...
pub async fn sleep(delay: time::Duration) {
    edgerun_bare_rt::sleep(delay).await;
}

pub fn timeout<F>(delay: time::Duration, future: F) -> edgerun_bare_rt::Timeout<F>
where
    F: core::future::Future,
{
    edgerun_bare_rt::timeout(delay, future)
}
...
where
    ...
    R: Send + 'static,
{
    edgerun_bare_rt::spawn_blocking(f)
}
