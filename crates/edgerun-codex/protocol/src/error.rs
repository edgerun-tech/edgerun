...
use crate::compat::async_utils::CancelErr;
use crate::compat::strings::truncate_middle_chars;
use crate::compat::strings::truncate_middle_with_token_budget;
...
fn retry_suffix(resets_at: Option<&DateTime>) -> String {
...
fn retry_suffix_after_or(resets_at: Option<&DateTime>) -> String {
    if let Some(resets_at) = resets_at {
        let formatted = format_retry_timestamp(resets_at);
...
fn format_retry_timestamp(resets_at: &DateTime) -> String {
...
#[cfg(test)]
thread_local! {
    static NOW_OVERRIDE: std::cell::RefCell<Option<DateTime>> =
        const { std::cell::RefCell::new(None) };
}

fn now_for_retry() -> DateTime {
    #[cfg(test)]
    {
