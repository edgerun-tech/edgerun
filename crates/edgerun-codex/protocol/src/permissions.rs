...
pub struct ReadDenyMatcher {
    ...
    deny_read_matchers: Vec<ReadGlobMatcher>,
    invalid_pattern: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct ReadGlobMatcher {
    pattern: String,
}

impl ReadGlobMatcher {
    fn is_match(&self, path: &Path) -> bool {
        let normalized = path.to_string_lossy().replace('\\', "/");
        glob_match_with_separator(&self.pattern, &normalized, '/')
    }
}
...
fn build_glob_matcher(pattern: &str) -> Option<ReadGlobMatcher> {
