...
fn parse_tool_input_schema_coerces_boolean_schemas() {
    ...
    //   semantics directly.
    let schema = parse_tool_input_schema(&edgerun_json::json!(true)).expect("parse schema");
...
fn parse_tool_input_schema_infers_object_shape_and_defaults_properties() {
    ...
    let schema = parse_tool_input_schema(&edgerun_json::json!({
        "properties": {
            "query": {"description": "search query"}
        }
    }))
...
fn parse_tool_input_schema_infers_string_from_enum_const_and_format_keywords() {
    ...
    let enum_schema = parse_tool_input_schema(&edgerun_json::json!({
        "enum": ["fast", "safe"]
    }))
    .expect("parse enum schema");
    let const_schema = parse_tool_input_schema(&edgerun_json::json!({
        "const": "file"
    }))
    .expect("parse const schema");
    let format_schema = parse_tool_input_schema(&edgerun_json::json!({
        "format": "date-time"
    }))
    ...
    assert_eq!(
        ...
        JsonSchema::string_enum(
            vec![edgerun_json::json!("fast"), edgerun_json::json!("safe")],
            /*description*/ None,
        )
    );
    assert_eq!(
        const_schema,
        JsonSchema::string_enum(vec![edgerun_json::json!("file")], /*description*/ None)
    );
...
fn parse_tool_input_schema_defaults_empty_schema_to_string() {
    ...
    let schema = parse_tool_input_schema(&edgerun_json::json!({})).expect("parse schema");
...
fn parse_tool_input_schema_rejects_singleton_null_type() {
    let err = parse_tool_input_schema(&edgerun_json::json!({
        "type": "null"
    }))
...
fn parse_tool_input_schema_preserves_explicit_enum_type_union() {
    ...
    let schema = super::parse_tool_input_schema(&edgerun_json::json!({
        "type": ["string", "null"],
        "enum": ["short", "medium", "long"],
        "description": "optional response length"
    }))
    ...
    assert_eq!(
        ...
        JsonSchema {
            ...
            enum_values: Some(vec![
                edgerun_json::json!("short"),
                edgerun_json::json!("medium"),
                edgerun_json::json!("long"),
            ]),
            ..Default::default()
        }
    );
        ...
        JsonSchema::object(
            BTreeMap::from([
                ...
                (
                    "scope".to_string(),
                    JsonSchema::string_enum(
                        vec![edgerun_json::json!("one"), edgerun_json::json!("two")],
                        /*description*/ None,
                    ),
                ),
            ]),
