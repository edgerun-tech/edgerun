...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    pretty_assertions::assert_eq!(
        edgerun_json::serde_json::to_value(&fork1_items).unwrap(),
        edgerun_json::serde_json::to_value(&expected_after_first).unwrap()
    );
    ...
    pretty_assertions::assert_eq!(
        edgerun_json::serde_json::to_value(&fork2_items).unwrap(),
        edgerun_json::serde_json::to_value(&expected_after_second).unwrap()
    );
}
...
async fn fork_thread_from_history_does_not_require_source_rollout_path() {
    ...
    let forked_items = forked_items
        .iter()
        .map(|item| edgerun_json::serde_json::to_value(item).unwrap())
        .collect::<Vec<_>>();
...
fn read_rollout_items(path: &std::path::Path) -> Vec<RolloutItem> {
    ...
    for line in text.lines() {
        ...
        let v: edgerun_json::serde_json::Value = match edgerun_json::serde_json::from_str(line) {
            Ok(value) => value,
            Err(err) => panic!("failed to parse rollout JSON line `{line}`: {err}"),
        };
        let rl: RolloutLine = match edgerun_json::serde_json::from_value(v) {
            Ok(line) => line,
