...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    ...
    for canonical_path in ["/repo/codex-rs/core", "/repo"] {
        ...
        let escaped_platform_path = edgerun_json::to_string(&platform_path)
...
fn last_user_message_text_from_body(body: &edgerun_json::Value) -> String {
    body["input"]
        .as_array()
        .expect("request input array")
        .iter()
        .filter(|item| item.get("role").and_then(edgerun_json::Value::as_str) == Some("user"))
        .filter_map(|item| item.get("content").and_then(edgerun_json::Value::as_array))
        .next_back()
        .expect("user message content")
        .iter()
        .filter(|span| span.get("type").and_then(edgerun_json::Value::as_str) == Some("input_text"))
        .filter_map(|span| span.get("text").and_then(edgerun_json::Value::as_str))
        .collect::<String>()
}
...
#[test]
fn format_guardian_action_pretty_truncates_large_string_fields() -> edgerun_json::Result<()> {
    let patch = "line\n".repeat(100_000);
...
#[test]
fn format_guardian_action_pretty_reports_no_truncation_for_small_payload() -> edgerun_json::Result<()>
{
    let action = GuardianApprovalRequest::ApplyPatch {
        id: "patch-1".to_string(),
        cwd: test_path_buf("/tmp").abs(),
...
#[test]
fn guardian_approval_request_to_json_renders_mcp_tool_call_shape() -> edgerun_json::Result<()> {
...
#[test]
fn guardian_approval_request_to_json_renders_network_access_trigger() -> edgerun_json::Result<()> {
...
fn guardian_assessment_action_redacts_apply_patch_patch_text() {
    ...
    assert_eq!(
        edgerun_json::to_serde_value(guardian_assessment_action(&action)).expect("serialize action"),
        edgerun_json::json!({
            "type": "apply_patch",
            "cwd": cwd,
            "files": [file],
...
-> anyhow::Result<()> {
    ...
    let guardian_assessment = edgerun_json::json!({
        "risk_level": "medium",
...
async fn guardian_review_surfaces_responses_api_errors_in_rejection_reason() -> anyhow::Result<()> {
    ...
    let _request_log = mount_response_once(
        &server,
        wiremock::ResponseTemplate::new(400).set_body_json(edgerun_json::json!({
            "error": {
...
async fn guardian_parallel_reviews_fork_from_last_committed_trunk_history() -> anyhow::Result<()> {
    ...
    let handle =
        ...
        let first_assessment = edgerun_json::json!({
            "risk_level": "low",
            "user_authorization": "high",
            "outcome": "allow",
        ...
        .to_string();
        let second_assessment = edgerun_json::json!({
            "risk_level": "low",
            "user_authorization": "high",
            "outcome": "allow",
        ...
        .to_string();
        let third_assessment = edgerun_json::json!({
            "risk_level": "low",
            "user_authorization": "high",
            "outcome": "allow",
