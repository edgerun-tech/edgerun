...
async fn build_turn_metadata_header_includes_has_changes_for_clean_repo() {
    ...
    let parsed: Value = edgerun_json::serde_json::from_str(&header).expect("valid json");
...
fn turn_metadata_state_uses_platform_sandbox_tag() {
    ...
    let json: Value = edgerun_json::serde_json::from_str(&header).expect("json");
...
fn turn_metadata_state_includes_model_and_reasoning_effort_only_in_request_meta() {
    ...
    let header = state.current_header_value().expect("header");
    let header_json: Value = edgerun_json::serde_json::from_str(&header).expect("json");
