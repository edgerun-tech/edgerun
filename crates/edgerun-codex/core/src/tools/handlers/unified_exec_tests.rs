...
async fn exec_command_pre_tool_use_payload_uses_raw_command() {
    let payload = ToolPayload::Function {
        arguments: edgerun_json::json!({ "cmd": "printf exec command" }).to_string(),
    };
    let (session, turn) = make_session_and_context().await;
    ...
    assert_eq!(
        ...
        Some(crate::tools::registry::PreToolUsePayload {
            ...
            tool_input: edgerun_json::json!({ "command": "printf exec command" }),
        })
    );
}
...
async fn exec_command_pre_tool_use_payload_skips_write_stdin() {
    let payload = ToolPayload::Function {
        arguments: edgerun_json::json!({ "chars": "echo hi" }).to_string(),
    };
    let (session, turn) = make_session_and_context().await;
...
async fn exec_command_post_tool_use_payload_uses_output_for_noninteractive_one_shot_commands() {
    let payload = ToolPayload::Function {
        arguments: edgerun_json::json!({ "cmd": "echo three", "tty": false }).to_string(),
    };
    ...
    assert_eq!(
        ...
        Some(crate::tools::registry::PostToolUsePayload {
            ...
            tool_input: edgerun_json::json!({ "command": "echo three" }),
            tool_response: edgerun_json::json!("three"),
        })
    );
}
...
async fn exec_command_post_tool_use_payload_uses_output_for_interactive_completion() {
    let payload = ToolPayload::Function {
        arguments: edgerun_json::json!({ "cmd": "echo three", "tty": true }).to_string(),
    };
    let output = ExecCommandToolOutput {
...
async fn write_stdin_post_tool_use_payload_uses_original_exec_call_id_and_command_on_completion() {
    ...
    assert_eq!(
        ...
        Some(crate::tools::registry::PostToolUsePayload {
            ...
            tool_input: edgerun_json::json!({ "command": "sleep 1; echo finished" }),
            tool_response: edgerun_json::json!("finished\n"),
        })
    );
}
...
async fn write_stdin_post_tool_use_payload_keeps_parallel_session_metadata_separate() {
    let payload = ToolPayload::Function {
        arguments: edgerun_json::json!({ "session_id": 45, "chars": "" }).to_string(),
    };
    ...
    assert_eq!(
        ...
        [
            Some(crate::tools::registry::PostToolUsePayload {
                ...
                tool_input: edgerun_json::json!({ "command": "sleep 1; echo beta" }),
                tool_response: edgerun_json::json!("beta\n"),
            }),
            Some(crate::tools::registry::PostToolUsePayload {
                tool_name: HookToolName::bash(),
                tool_use_id: "exec-call-a".to_string(),
                tool_input: edgerun_json::json!({ "command": "sleep 2; echo alpha" }),
                tool_response: edgerun_json::json!("alpha\n"),
            }),
        ]
    );
}
