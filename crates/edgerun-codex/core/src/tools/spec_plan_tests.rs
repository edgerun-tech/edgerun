...
fn namespace_specs_are_hidden_when_namespace_tools_are_disabled() {
    ...
    let (tools, registry) = build_specs(
        ...
        Some(HashMap::from([(
            ...
            mcp_tool("echo", "Echo", edgerun_json::json!({"type": "object"})),
        )])),
        /*deferred_mcp_tools*/ None,
        &[],
    );
...
fn test_build_specs_mcp_tools_sorted_by_name() {
    ...
    let tools_map = HashMap::from([
        (
            ...
            mcp_tool("do", "a", edgerun_json::json!({"type": "object"})),
        ),
        (
            ToolName::namespaced("test_server/", "something"),
            mcp_tool("something", "b", edgerun_json::json!({"type": "object"})),
        ),
        (
            ToolName::namespaced("test_server/", "cool"),
            mcp_tool("cool", "c", edgerun_json::json!({"type": "object"})),
        ),
    ]);
