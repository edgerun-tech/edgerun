        ...
        match edgerun_json::from_str(&arguments) {
            ...
            Err(e) => {
                ...
                return HandledMcpToolCall {
                    ...
                    tool_input: JsonValue::Object(edgerun_json::Map::new()),
                };
            }
        }
    };
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    turn_context: &TurnContext,
    server: &str,
    mut meta: Option<edgerun_json::Value>,
) -> anyhow::Result<Option<edgerun_json::Value>> {
    ...
    match meta.as_mut() {
        Some(edgerun_json::Value::Object(map)) => {
            map.insert(
        ...
        None => {
            ...
            meta = Some(edgerun_json::Value::Object(map));
        }
    }
...
fn sanitize_mcp_tool_result_for_model(
    ...
    result.map(|call_tool_result| CallToolResult {
        content: call_tool_result
            ...
            .iter()
            .map(|block| {
                if let Some(content_type) = block.get("type").and_then(edgerun_json::Value::as_str)
                    && content_type == "image"
                {
                    return edgerun_json::json!({
                        "type": "text",
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    codex_apps_meta: Option<edgerun_json::Map<String, edgerun_json::Value>>,
...
fn build_mcp_tool_call_request_meta(
    ...
    let mut request_meta = edgerun_json::Map::new();
    ...
    if server == CODEX_APPS_MCP_SERVER_NAME {
        ...
        codex_apps_meta.insert(
            "call_id".to_string(),
            edgerun_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_CODEX_APPS_META_KEY.to_string(),
            edgerun_json::Value::Object(codex_apps_meta),
        );
    }

    (!request_meta.is_empty()).then_some(edgerun_json::Value::Object(request_meta))
}
...
fn with_mcp_tool_call_thread_id_meta(
    ...
    match meta {
        Some(edgerun_json::Value::Object(mut map)) => {
            map.insert(
                MCP_TOOL_THREAD_ID_META_KEY.to_string(),
                edgerun_json::Value::String(thread_id.to_string()),
            );
            Some(edgerun_json::Value::Object(map))
        }
        None => {
...
struct McpToolApprovalElicitationRequest<'a> {
    ...
    tool_params: Option<&'a edgerun_json::Value>,
...
fn openai_file_input_params_for_server(
    server: &str,
    meta: Option<&edgerun_json::Map<String, edgerun_json::Value>>,
) -> Option<Vec<String>> {
...
fn build_mcp_tool_approval_elicitation_meta(
    server: &str,
    metadata: Option<&McpToolApprovalMetadata>,
    tool_params: Option<&edgerun_json::Value>,
    tool_params_display: Option<&[RenderedMcpToolApprovalParam]>,
    prompt_options: McpToolApprovalPromptOptions,
) -> Option<edgerun_json::Value> {
    let mut meta = edgerun_json::Map::new();
    meta.insert(
        MCP_TOOL_APPROVAL_KIND_KEY.to_string(),
        edgerun_json::Value::String(MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL.to_string()),
    );
    match (
        ...
        (true, false) => {
            meta.insert(
                MCP_TOOL_APPROVAL_PERSIST_KEY.to_string(),
                edgerun_json::Value::String(MCP_TOOL_APPROVAL_PERSIST_SESSION.to_string()),
            );
        }
        (false, true) => {
            meta.insert(
                MCP_TOOL_APPROVAL_PERSIST_KEY.to_string(),
                edgerun_json::Value::String(MCP_TOOL_APPROVAL_PERSIST_ALWAYS.to_string()),
            );
        }
        (false, false) => {}
    }
                ...
                || metadata.connector_description.is_some())
        {
            ...
            if let Some(connector_description) = metadata.connector_description.as_ref() {
                meta.insert(
                    ...
                    edgerun_json::Value::String(connector_description.clone()),
                );
            }
        }
    }
...
fn request_user_input_response_from_elicitation_content(
    content: Option<edgerun_json::Value>,
    ...
    let answers = content
        ...
        .filter_map(|(question_id, value)| {
            let answers = match value {
                edgerun_json::Value::String(answer) => vec![answer.clone()],
                edgerun_json::Value::Array(values) => values
                    .iter()
