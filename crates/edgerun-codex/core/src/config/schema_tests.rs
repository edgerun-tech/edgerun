...
fn config_schema_matches_fixture() {
    ...
    if fixture_value != schema_value {
        let expected =
            edgerun_json::to_string_pretty(&fixture_value).expect("serialize fixture json");
        let actual = edgerun_json::to_string_pretty(&schema_value).expect("serialize schema json");
