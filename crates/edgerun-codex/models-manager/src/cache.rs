...
impl ModelsCacheManager {
    ...
    async fn load(&self) -> io::Result<Option<ModelsCache>> {
        match fs::read(&self.cache_path).await {
            Ok(contents) => {
                let cache = edgerun_json::from_json_slice(&contents)
    ...
    async fn save_internal(&self, cache: &ModelsCache) -> io::Result<()> {
        ...
        let json = edgerun_json::to_vec_pretty(&cache.to_json())
            .map_err(|err| io::Error::new(ErrorKind::InvalidData, err.to_string()))?;
...
impl ToJson for ModelsCache {
    fn to_json(&self) -> JsonValue {
        let mut object = Map::new();
        object.push_field("fetched_at", format_rfc3339_utc_datetime(&self.fetched_at));
        if let Some(etag) = &self.etag {
            object.push_field("etag", etag.as_str());
        }
        if let Some(client_version) = &self.client_version {
            object.push_field("client_version", client_version.as_str());
        }
        object.push_field("models", self.models.to_json());
        JsonValue::Object(object)
    }
}

impl FromJson for ModelsCache {
    fn from_json(value: JsonValue) -> Result<Self, JsonValueError> {
        let mut object = value.into_object(core::any::type_name::<Self>())?;
        let fetched_at = object.take_required::<String>("fetched_at")?;
        let fetched_at = parse_rfc3339_utc_datetime(&fetched_at)
            .map_err(|err| JsonValueError::WrongType(err.to_string()))?;
        Ok(Self {
            fetched_at,
            etag: object.take_optional("etag")?,
            client_version: object.take_optional("client_version")?,
            models: object.take_required("models")?,
        })
    }
}
...
impl ModelsCache {
    ...
    fn is_fresh(&self, ttl: Duration) -> bool {
        is_fresh_since(Utc::now(), self.fetched_at, ttl)
    }
}
