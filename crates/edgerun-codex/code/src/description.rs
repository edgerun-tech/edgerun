...
use edgerun_json::{FromJson, JsonValueError, ToJson, Value as JsonValue};
...
impl ToJson for CodeModeToolKind {
...
impl FromJson for CodeModeToolKind {
    fn from_json(value: JsonValue) -> Result<Self, JsonValueError> {
        ...
        match value.as_str() {
            ...
            _ => Err(JsonValueError::WrongType(format!(
                "unknown code mode tool kind `{value}`"
            ))),
        }
    }
}
...
impl FromJson for CodeModeExecPragma {
    fn from_json(value: JsonValue) -> Result<Self, JsonValueError> {
        let mut object = value.into_object("CodeModeExecPragma")?;
        let yield_time_ms = object.take_optional("yield_time_ms")?;
        let max_output_tokens = object.take_optional("max_output_tokens")?;
        if let Some((key, _)) = object.into_vec().into_iter().next() {
            return Err(JsonValueError::WrongType(format!(
                "unsupported exec pragma field `{key}`"
            )));
        }
        Ok(Self {
            yield_time_ms,
...
pub fn parse_exec_source(input: &str) -> Result<ParsedExecSource, String> {
    ...
    let value: edgerun_json::Value = edgerun_json::from_str(directive).map_err(|err| {
        format!(
    ...
    let pragma: CodeModeExecPragma = edgerun_json::from_json_value(value).map_err(|err| {
        format!(
...
fn render_json_schema_array(map: &edgerun_json::Map<String, JsonValue>) -> String {
...
fn append_additional_properties_line(
    ...
    map: &edgerun_json::Map<String, JsonValue>,
    properties: &edgerun_json::Map<String, JsonValue>,
    line_prefix: &str,
) {
...
fn render_json_schema_object(map: &edgerun_json::Map<String, JsonValue>) -> String {
    let required = map
        .get("required")
