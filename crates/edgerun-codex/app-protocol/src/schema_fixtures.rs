    ...
    match value {
        Value::Array(items) => {
            ...
            for item in &items {
                ...
                let stable = edgerun_json::to_string(item).unwrap_or_default();
...
mod tests {
    ...
    fn canonicalize_json_sorts_string_arrays() {
        let value = edgerun_json::json!(["b", "a"]);
        let expected = edgerun_json::json!(["a", "b"]);
    ...
    fn canonicalize_json_sorts_schema_ref_arrays() {
        ...
        let expected = edgerun_json::json!([
            {"$ref": "#/definitions/A"},
            {"$ref": "#/definitions/B"}
        ]);
        assert_eq!(canonicalize_json(&value), expected);
