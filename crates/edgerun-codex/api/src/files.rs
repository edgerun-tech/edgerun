...
pub async fn upload_local_file(
    ...
    let create_payload =
        parse_create_file_response(&create_body).map_err(|source| OpenAiFileError::Decode {
            url: create_url.clone(),
            source,
        })?;
    ...
    loop {
        ...
        let finalize_payload = parse_download_link_response(&finalize_body).map_err(|source| {
        ...
        match finalize_payload.status.as_str() {
            ...
            "retry" => {
                ...
                edgerun_tokio::time::sleep(OPENAI_FILE_FINALIZE_RETRY_DELAY).await;
            }
            _ => {
...
fn authorized_request(
    auth: &dyn AuthProvider,
    method: edgerun_reqwest::Method,
    url: &str,
) -> edgerun_reqwest::RequestBuilder {
...
fn parse_create_file_response(body: &str) -> Result<CreateFileResponse, JsonValueError> {
    let tape = edgerun_json::parse_json_tape(body)?;
    let root = tape
        .root(body)
        .ok_or_else(|| JsonValueError::WrongType("missing JSON root".to_string()))?;
    Ok(CreateFileResponse {
        file_id: root.required_str("file_id").map(str::to_string)?,
        upload_url: root.required_str("upload_url").map(str::to_string)?,
    })
}

fn parse_download_link_response(body: &str) -> Result<DownloadLinkResponse, JsonValueError> {
    let tape = edgerun_json::parse_json_tape(body)?;
    let root = tape
        .root(body)
        .ok_or_else(|| JsonValueError::WrongType("missing JSON root".to_string()))?;
    Ok(DownloadLinkResponse {
        status: root.required_str("status").map(str::to_string)?,
        download_url: root
            .get("download_url")
...
mod tests {
    ...
    impl AuthProvider for ChatGptTestAuth {
        fn add_auth_headers(&self, headers: &mut edgerun_reqwest::header::HeaderMap) {
            headers.insert(
                edgerun_reqwest::header::AUTHORIZATION,
    ...
    async fn upload_local_file_returns_canonical_uri() {
        ...
        Mock::given(method("POST"))
            ...
            .and(body_json(edgerun_json::json!({
                "file_name": "hello.txt",
                "file_size": 5,
                "use_case": "codex",
            })))
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_json(edgerun_json::json!({"file_id": "file_123", "upload_url": format!("{}/upload/file_123", server.uri())})),
            )
            .mount(&server)
            .await;
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(move |_request: &Request| {
                if finalize_attempts_responder.fetch_add(1, Ordering::SeqCst) == 0 {
                    return ResponseTemplate::new(200).set_body_json(edgerun_json::json!({
                        "status": "retry"
                    }));
                }
