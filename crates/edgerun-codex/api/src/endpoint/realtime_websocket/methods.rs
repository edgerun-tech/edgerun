...
fn websocket_url_from_api_url(
    ...
    if intent.is_some() || model.is_some() || has_extra_query_params {
        if let Some(intent) = intent {
            url.append_query_pair("intent", intent);
        }
        if let Some(model) = model {
            url.append_query_pair("model", model);
        }
...
fn websocket_url_from_api_url_for_call(
    ...
    url.append_query_pair("call_id", call_id);
    Ok(url)
}
    ...
    async fn e2e_connect_and_exchange_events_against_mock_ws_server() {
        ...
        let server = edgerun_tokio::spawn(async move {
            ...
            let first = ws
                ...
                .into_text()
                .expect("text");
            let first_json: Value = edgerun_json::from_serde_str(&first).expect("json");
            ...
            let second = ws
                ...
                .into_text()
                .expect("text");
            let second_json: Value = edgerun_json::from_serde_str(&second).expect("json");
            ...
            let third = ws
                ...
                .into_text()
                .expect("text");
            let third_json: Value = edgerun_json::from_serde_str(&third).expect("json");
            ...
            let fourth = ws
                ...
                .into_text()
                .expect("text");
            let fourth_json: Value = edgerun_json::from_serde_str(&fourth).expect("json");
