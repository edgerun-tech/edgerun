...
use edgerun_json::Map as JsonMap;
...
fn parse_handoff_requested_event(item: &JsonMap) -> Option<RealtimeEvent> {
...
fn parse_noop_requested_event(item: &JsonMap) -> Option<RealtimeEvent> {
    let item_type = item.get("type").and_then(Value::as_str);
    let item_name = item.get("name").and_then(Value::as_str);
...
fn extract_input_transcript(arguments: &str) -> String {
    ...
    if let Ok(arguments_json) = edgerun_json::from_str(arguments)
