...
async fn connect_websocket(
    ...
    let response = connect_async_with_config(
        request,
...
fn ws_headers_to_api_headers(headers: &edgerun_tokio_tungstenite::http::HeaderMap) -> HeaderMap {
    ...
    for (name, value) in headers {
        ...
        let Ok(value) = value
            .to_str()
            .map_err(|_| ())
            .and_then(|value| edgerun_http::HeaderValue::from_str(value).map_err(|_| ()))
        else {
            continue;
        };
...
fn map_ws_error(err: WsError, url: &Url) -> ApiError {
    match err {
        WsError::Http(response) => {
            let status = edgerun_http::StatusCode::from_u16(response.status().as_u16())
                .unwrap_or(edgerun_http::StatusCode::INTERNAL_SERVER_ERROR);
            let headers = ws_headers_to_api_headers(response.headers());
            let body = response
                .body()
                .as_ref()
...
fn parse_wrapped_websocket_error_event(payload: &str) -> Option<WrappedWebsocketErrorEvent> {
    ...
    if root.get("type").and_then(|value| value.as_str()) != Some("error") {
        return None;
    }
    let status = root
        .get("status")
    ...
    let error = root.get("error").map(|error| WrappedWebsocketError {
        code: error
            .get("code")
    ...
    let headers = root
        .get("headers")
        .and_then(|headers| headers.to_json_value())
        .and_then(|headers| headers.into_object("websocket error headers").ok());
    Some(WrappedWebsocketErrorEvent {
        status,
        error,
        headers,
    })
...
fn json_headers_to_http_headers(headers: EdgeJsonMap) -> HeaderMap {
...
fn json_header_value(value: EdgeJsonValue) -> Option<HeaderValue> {
    let value = match value {
        EdgeJsonValue::String(value) => value,
        EdgeJsonValue::Number(value) => value.to_string(),
        EdgeJsonValue::Bool(value) => value.to_string(),
        _ => return None,
    };
...
async fn send_websocket_request(
    ...
    let request_text = match edgerun_json::to_string(&request_body) {
        Ok(text) => text,
        Err(err) => {
