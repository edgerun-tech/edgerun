...
var _tmpl$ = /*#__PURE__*/_$template(`<div class="relative w-full h-full"><canvas id=graph-canvas class="block w-full h-full"></canvas><canvas id=graph-overlay class="absolute top-0 left-0 w-full h-full pointer-events-none z-10">`);
/**
 * WebGL graph canvas with force-directed layout, pan/zoom, and interaction.
 */
import { onMount, onCleanup } from "solid-js";
import { nodesMap, edges, selectedNodeIds, highlightedNodeIds, hoveredNodeId, setHoveredNodeId, setFocusedNodeWithHighlight, zoom, setZoom, panX, setPanX, panY, setPanY, rotation, setRotation, boxSelectRect, setBoxSelectRect, rotationIndicator, updateNodePosition, fps, setFps, activeTagFilters, getFilteredNodes } from "../store.js";
import { LANG_COLORS, NODE_VS, NODE_FS, EDGE_VS, EDGE_FS, buildNodeProgram, buildEdgeProgram, screenToGraph } from "../lib/gl.js";
export function GraphCanvas() {
  ...
  function initEvents() {
    ...
    canvasRef.addEventListener("wheel", onWheel, {
      passive: false
    });
  ...
  function onMouseDown(e) {
    ...
    if (nodeId) {
      if (ctrlDown) {
        ...
        if (sel.has(nodeId)) sel.delete(nodeId);else sel.add(nodeId);
  ...
  return (() => {
    ...
    var _ref$ = canvasRef;
    typeof _ref$ === "function" ? _$use(_ref$, _el$2) : canvasRef = _el$2;
    var _ref$2 = overlayCanvasRef;
    typeof _ref$2 === "function" ? _$use(_ref$2, _el$3) : overlayCanvasRef = _el$3;
    return _el$;
  })();
}