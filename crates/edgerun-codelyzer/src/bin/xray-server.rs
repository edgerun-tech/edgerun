...
fn graph_response_for_root(
    ...
    data.cache_status = if force_rescan {
        "forced_rescan"
    } else {
        "miss"
    }
    .to_string();
    let envelope = GraphUpdateEnvelope {
        kind: XrayEnvelopeKind::GraphUpdate,
        data,
    };
...
fn analyze_repo_for_xray(
    root: &str,
    snapshot: Vec<filesystem::FileInfo>,
) -> Result<XrayGraphData, String> {
    ...
    let mut edges: Vec<XrayGraphEdge> = program
        ...
        .map(|edge| XrayGraphEdge {
            ...
            tags: vec![
                edge.source.clone(),
                format!("confidence:{:.1}", edge.confidence),
            ],
        })
        .collect();
...
fn respond_rkyv_bytes(
    stream: &mut TcpStream,
    status: u16,
    body: Result<Vec<u8>, rkyv::rancor::Error>,
) {
