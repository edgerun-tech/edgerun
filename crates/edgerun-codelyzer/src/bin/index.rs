...
fn signature_from_function(content: &str, function: &RawFunctionOwned) -> String {
    ...
    let first_line_end = content[start..end]
        .find('\n')
...
fn write_disk_index(
    ...
    for file in &index.files {
        ...
        let error_idx = file
            .parse_error
            .as_ref()
            .map_or(NO_STRING, |error| interner.map[error]);
...
fn index_codebase(root: &Path, index_path: &Path, refresh: bool) {
    ...
    println!("[index] Writing compact cache to {}", index_path.display());
...
fn query_index(index_path: &Path, args: &[String]) {
    ...
    if args.is_empty() {
        let parse_error_count = index
            .files
            .iter()
            .filter(|f| f.parse_error.is_some())
            .count();
    ...
    match query {
        ...
        "stats" => {
            ...
            println!("  Languages: {}", lang_count);
            println!(
                "  Average edges/function: {:.1}",
                edge_count as f64 / function_count.max(1) as f64
            );
            println!("\nTop 10 most-connected functions:");
            for (id, count) in top.iter().take(10) {
                if let Some(function) = function_by_id.get(id) {
                    println!("  {} ({}) - {} edges", function.name, function.file, count);
                } else {
...
mod tests {
    ...
    fn write_and_read_disk_index_round_trip() {
        ...
        write_disk_index(&index_path, root, index.generated_at, &index)
    ...
    fn load_disk_index_rejects_invalid_magic() {
        ...
        output.write_all(b"NOTINDEX").expect("write bad magic");
