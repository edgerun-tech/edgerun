...
fn run() -> Result<(), String> {
    ...
    match args.cmd.as_str() {
        ...
        "metadata" => metadata_command(&args, &ws),
        "clean" => clean_ram(),
        "check" | "build" | "test" | "run" => build_command(&args, &ws, &vfs),
        "fmt" => fmt_command(&args, &ws),
...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(arg) = raw.next() {
        match arg.as_str() {
            ...
            "--all" => args.fmt_all = true,
            "--target" => args.target = Some(need_value(&mut raw, "--target")?),
            arg if arg.starts_with("--target=") => {
                args.target = Some(arg["--target=".len()..].to_string())
            }
            "--crate-type" => args.crate_types.push(need_value(&mut raw, "--crate-type")?),
            arg if arg.starts_with("--crate-type=") => args
                .crate_types
                .push(arg["--crate-type=".len()..].to_string()),
            "--out" => args.out = Some(PathBuf::from(need_value(&mut raw, "--out")?)),
            "--no-deps" => args.metadata_no_deps = true,
            "--format-version" => {
                args.metadata_format_version =
                    parse_metadata_format_version(&need_value(&mut raw, "--format-version")?)?
            }
            arg if arg.starts_with("--format-version=") => {
                args.metadata_format_version =
                    parse_metadata_format_version(&arg["--format-version=".len()..])?
            }
            "--" => {
...
fn parse_metadata_format_version(value: &str) -> Result<u32, String> {
    let version = value
        .parse::<u32>()
        .map_err(|_| format!("--format-version requires an integer, got {value}"))?;
    if version != 1 {
        return Err(format!(
            "unsupported cargo metadata --format-version {version}; only 1 is implemented"
        ));
    }
    Ok(version)
}
    ...
    for line in lines.drain(..) {
        ...
        match section.as_str() {
            ...
            "lib" if key == "crate-type" => {
                manifest.lib_crate_types = parse_string_array(value);
                manifest.lib_proc_macro = manifest.lib_crate_types.iter().any(|v| v == "proc-macro")
            }
            "bin" if key == "name" => {
...
fn merge_dep(existing: &mut Dep, incoming: Dep) {
    ...
    if existing.rename == crate_name(&existing.package)
        && incoming_rename != crate_name(&incoming.package)
    {
...
fn target_lib(
    ...
    vfs.read_str(&path).map(|_| Target {
        ...
        required_features: Vec::new(),
        crate_types: toml.lib_crate_types.clone(),
    })
}
...
fn target_bins(vfs: &edgerun_vfs::VirtualFileSystem, toml: &Manifest, dir: &str) -> Vec<Target> {
    ...
    if vfs.read_str(&main).is_some() && !bins.iter().any(|bin| bin.path == main) {
        bins.push(Target {
            name: "main".to_string(),
            path: main,
            kind: TargetKind::Bin,
...
fn build_command(
    ...
    finish_failures("build", outcome.failed)
}
...
fn build_roots(
    ...
    log_verbose(|| format!("= edgerun-build jobs {jobs}"));
...
fn finish_failures(kind: &str, failures: Vec<String>) -> Result<(), String> {
...
fn fmt_command(args: &Args, ws: &Workspace) -> Result<(), String> {
    let packages = select_fmt_packages(args, ws)?;
    let mut failures = Vec::new();
    for package in packages {
        if let Err(err) = fmt_package(args, &ws.packages[&package]) {
            if args.keep_going {
    ...
    finish_failures("format", failures)
}

fn select_fmt_packages(args: &Args, ws: &Workspace) -> Result<Vec<String>, String> {
    if args.fmt_all {
...
fn fmt_package(args: &Args, pkg: &Package) -> Result<(), String> {
    let files = fmt_files(args, pkg)?;
    if files.is_empty() {
        return Ok(());
    }
    ...
    cmd.current_dir(&args.workspace)
        .arg("--edition")
        .arg(&pkg.edition);
    if args.fmt_check || args.rest.iter().any(|arg| arg == "--check") {
        cmd.arg("--check");
    }
    ...
    let status = cmd
        .status()
        .map_err(|err| format!("run rustfmt for {}: {err}", pkg.name))?;
    if status.success() {
        Ok(())
    } else {
        Err(format!("rustfmt for {} exited with {status}", pkg.name))
    }
}

fn fmt_files(args: &Args, pkg: &Package) -> Result<Vec<PathBuf>, String> {
    let mut files = BTreeSet::new();
    if let Some(lib) = &pkg.lib {
        insert_existing_fmt_file(args, &mut files, &lib.path)?;
    }
    for target in pkg.bins.iter().chain(pkg.examples.iter()) {
        insert_existing_fmt_file(args, &mut files, &target.path)?;
    }
    insert_existing_fmt_file(args, &mut files, &format!("{}/build.rs", pkg.dir))?;
    collect_rs_files(&args.workspace.join(&pkg.dir).join("tests"), &mut files)?;
    collect_rs_files(&args.workspace.join(&pkg.dir).join("benches"), &mut files)?;
    Ok(files.into_iter().collect())
}

fn insert_existing_fmt_file(
    args: &Args,
    files: &mut BTreeSet<PathBuf>,
    rel: &str,
) -> Result<(), String> {
    let path = args.workspace.join(rel);
    match fs::metadata(&path) {
        ...
        Err(err) => Err(format!("metadata {}: {err}", path.display())),
    }
}

fn collect_rs_files(dir: &Path, files: &mut BTreeSet<PathBuf>) -> Result<(), String> {
    let entries = match fs::read_dir(dir) {
        Ok(entries) => entries,
        Err(err) if err.kind() == ErrorKind::NotFound => return Ok(()),
        Err(err) => return Err(format!("read_dir {}: {err}", dir.display())),
    };
    for entry in entries {
...
fn build_package_command(
    ...
    for node in &nodes {
        ...
        if let Some(lib) = &pkg.lib {
            let artifact = if args.target.is_some() && lib.kind == TargetKind::ProcMacro {
                let host_out_dir = out_dir.join("host");
                fs::create_dir_all(&host_out_dir)
                    .map_err(|err| format!("mkdir {}: {err}", host_out_dir.display()))?;
                let mut compiling = BTreeSet::new();
                compile_host_artifact(
                    args,
                    ws,
                    &nodes,
                    &node.package,
        ...
        if is_root && should_compile_bins(args) {
            for bin in &pkg.bins {
                ...
                mirror_cargo_compatible_artifact(args, bin, &artifact.path)?;
                if args.cmd == "run" {
        ...
        if is_root && args.cmd == "build" && examples_requested(args) {
            for example in &pkg.examples {
                ...
                .and_then(|artifact| {
                    mirror_cargo_compatible_artifact(args, example, &artifact.path)?;
                    Ok(artifact)
                }) {
                    if args.keep_going {
...
fn mirror_cargo_compatible_artifact(
    args: &Args,
    target: &Target,
    artifact: &Path,
) -> Result<(), String> {
    ...
    let triple = match &args.target {
        ...
        None => current_rustc_host_triple()?,
    };
    let target_root = env::var_os("CARGO_TARGET_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| args.workspace.join("target"));
    let out_dir = target_root.join(triple).join(profile);
    fs::create_dir_all(&out_dir).map_err(|err| format!("mkdir {}: {err}", out_dir.display()))?;
    copy_if_changed(artifact, &out_dir.join(executable_file_name(&target.name)))
}

fn current_rustc_host_triple() -> Result<String, String> {
    let output = Command::new("rustc")
        .arg("-vV")
        .output()
        .map_err(|err| format!("run rustc -vV: {err}"))?;
    ...
    for (path, bytes) in vfs.files() {
            ...
            .any(|dir| path == dir || path.starts_with(&format!("{dir}/")))
        {
            ...
            if path.ends_with(".rs") {
                for include in
                    literal_include_bytes_paths(path, String::from_utf8_lossy(bytes).as_ref())
                {
...
fn compile_target(
    ...
    if !args.fresh && fs::read_to_string(&stamp_path).is_ok_and(|stamp| stamp == fingerprint) {
        if let Ok(path) = find_artifact(out_dir, &target_crate_name, target.kind, metadata_only) {
            log_verbose(|| format!("= reuse {}", path.display()));
    ...
    cmd.arg("--crate-name")
        ...
        .arg("--color")
        .arg("never");
    if args.target.is_some() && target.kind != TargetKind::ProcMacro {
        cmd.arg("-L")
            .arg(format!("dependency={}", out_dir.join("host").display()));
    }
    ...
    match target.kind {
        TargetKind::Lib => {
            if metadata_only {
                ...
                let crate_types = if args
                    .package
                    .as_deref()
                    .is_some_and(|package| package == pkg.name)
                    && !args.crate_types.is_empty()
                {
                    &args.crate_types
                } else {
                    &target.crate_types
...
fn compile_host_artifact(
    ...
    src_root: &Path,
    out_dir: &Path,
    artifacts: &mut BTreeMap<String, BuiltArtifact>,
    compiling: &mut BTreeSet<String>,
    metadata_only: bool,
) -> Result<BuiltArtifact, String> {
    if let Some(artifact) = artifacts.get(package_name) {
        return Ok(artifact.clone());
    }
    if !compiling.insert(package_name.to_string()) {
        return Err(format!(
            "cyclic host dependency while compiling {package_name}"
        ));
    }
    let pkg = ws
        .packages
        .get(package_name)
        .ok_or_else(|| format!("missing package {package_name}"))?;
    let node = nodes
        .iter()
        .find(|node| node.package == package_name)
        .ok_or_else(|| format!("missing build node {package_name}"))?;
    for dep in pkg.deps.values() {
        if ws.packages.contains_key(&dep.package) {
            compile_host_artifact(
                args,
    ...
    let Some(lib) = &pkg.lib else {
        compiling.remove(package_name);
        return Err(format!("host package {package_name} has no library target"));
    };
    let mut host_args = args.clone();
    host_args.target = None;
    ...
    compiling.remove(package_name);
    artifacts.insert(package_name.to_string(), artifact.clone());
    Ok(artifact)
}
...
fn run_rustc(cmd: &mut Command) -> Result<(), String> {
    ...
    replay_compiler_output(&output.stdout, &output.stderr);
    Err(format!(
        "rustc exited with {}; set EDGERUN_BUILD_VERBOSE=1 to show the full rustc command",
        output.status
    ))
}

fn replay_compiler_output(stdout: &[u8], stderr: &[u8]) {
    if !stdout.is_empty() {
...
fn build_verbose() -> bool {
    matches!(
        env::var("EDGERUN_BUILD_VERBOSE").as_deref(),
        Ok("1") | Ok("true") | Ok("yes") | Ok("on")
    )
}

fn log_verbose(message: impl FnOnce() -> String) {
    if build_verbose() {
        eprintln!("{}", message());
    }
}
...
fn bin_selected(args: &Args, target: &Target) -> bool {
    match args.bin.as_deref() {
        Some(name) => crate_name(name) == crate_name(&target.name),
        None => true,
    }
}
...
fn example_selected(args: &Args, target: &Target) -> bool {
    args.examples
        || args
            .example
            .as_deref()
            .is_some_and(|name| crate_name(name) == crate_name(&target.name))
}
...
fn metadata_command(args: &Args, ws: &Workspace) -> Result<(), String> {
    if args.metadata_format_version != 1 {
        return Err(format!(
            "unsupported cargo metadata --format-version {}; only 1 is implemented",
            args.metadata_format_version
        ));
    }

    let package_names = if let Some(package) = &args.package {
        if !ws.packages.contains_key(package) {
            return Err(format!("package not found: {package}"));
        }
        vec![package.clone()]
    } else {
        ws.packages.keys().cloned().collect::<Vec<_>>()
    };
    let package_ids = package_names
        .iter()
        .map(|name| metadata_package_id(&ws.packages[name]))
        .collect::<Vec<_>>();
    ...
    for (index, name) in package_names.iter().enumerate() {
        ...
        metadata_write_package(&mut out, args, &ws.packages[name], args.metadata_no_deps)?;
    }
    out.push_str("],\"workspace_members\":[");
    for (index, id) in package_ids.iter().enumerate() {
        if index > 0 {
            out.push(',');
        }
        json_string(&mut out, id);
    }
    out.push_str("],\"workspace_default_members\":[");
    for (index, id) in package_ids.iter().enumerate() {
        if index > 0 {
            out.push(',');
        }
    ...
    if args.metadata_no_deps {
        ...
        metadata_write_resolve(&mut out, ws, &package_names);
    }
    out.push_str(",\"target_directory\":");
    json_string(
        &mut out,
        &args.workspace.join("target").display().to_string(),
    );
    out.push_str(",\"version\":1,\"workspace_root\":");
    json_string(&mut out, &args.workspace.display().to_string());
    out.push_str(",\"metadata\":null}");
    println!("{out}");
    Ok(())
}

fn metadata_package_id(pkg: &Package) -> String {
    format!("path+file://{}#{}@{}", pkg.dir, pkg.name, pkg.version)
}

fn metadata_write_package(
    out: &mut String,
    args: &Args,
    pkg: &Package,
    no_deps: bool,
    ...
    json_string(out, &pkg.version);
    out.push_str(",\"id\":");
    json_string(out, &metadata_package_id(pkg));
    out.push_str(",\"license\":null,\"license_file\":null,\"description\":null,\"source\":null");
    ...
    if !no_deps {
        ...
        for dep in pkg.deps.values() {
            ...
            first = false;
            metadata_write_dependency(out, args, dep);
        }
    }
    out.push_str("],\"targets\":[");
    let mut first_target = true;
    if let Some(target) = &pkg.lib {
        metadata_write_target(out, args, target, &mut first_target)?;
    }
    for target in &pkg.bins {
        metadata_write_target(out, args, target, &mut first_target)?;
    }
    for target in &pkg.examples {
        metadata_write_target(out, args, target, &mut first_target)?;
    }
    out.push_str("],\"features\":{");
    for (index, (name, values)) in pkg.features.iter().enumerate() {
        if index > 0 {
            out.push(',');
        }
    ...
    out.push_str(",\"metadata\":null,\"publish\":null,\"authors\":[],\"categories\":[],\"keywords\":[],\"readme\":null,\"repository\":null,\"homepage\":null,\"documentation\":null,\"edition\":");
    json_string(out, &pkg.edition);
    out.push_str(",\"links\":null,\"default_run\":null,\"rust_version\":null}");
    Ok(())
}

fn metadata_write_dependency(out: &mut String, args: &Args, dep: &Dep) {
    out.push('{');
    out.push_str("\"name\":");
    json_string(out, &dep.package);
    out.push_str(",\"source\":null,\"req\":\"*\",\"kind\":null,\"rename\":");
    if dep.rename == dep.package {
        out.push_str("null");
    } else {
        json_string(out, &dep.rename);
    }
    out.push_str(",\"optional\":");
    out.push_str(if dep.optional { "true" } else { "false" });
    out.push_str(",\"uses_default_features\":");
    out.push_str(if dep.default_features {
        "true"
    } else {
        "false"
    });
    out.push_str(",\"features\":");
    json_string_array(out, &dep.features);
    out.push_str(",\"target\":null,\"registry\":null");
    if let Some(path) = &dep.path {
        out.push_str(",\"path\":");
        json_string(out, &args.workspace.join(path).display().to_string());
    }
    out.push('}');
}

fn metadata_write_target(
    out: &mut String,
    args: &Args,
    target: &Target,
    first_target: &mut bool,
) -> Result<(), String> {
    if !*first_target {
        out.push(',');
    }
    *first_target = false;
    out.push('{');
    out.push_str("\"kind\":");
    json_string_array(out, &metadata_target_kind(target.kind));
    out.push_str(",\"crate_types\":");
    let crate_types = if target.crate_types.is_empty() {
        metadata_target_crate_types(target.kind)
    } else {
        target.crate_types.clone()
    };
    json_string_array(out, &crate_types);
    out.push_str(",\"name\":");
    json_string(out, &target.name);
    out.push_str(",\"src_path\":");
    json_string(
        out,
        &args.workspace.join(&target.path).display().to_string(),
    );
    out.push_str(",\"edition\":\"2024\",\"doc\":true,\"doctest\":true,\"test\":true");
    out.push('}');
    Ok(())
}

fn metadata_target_kind(kind: TargetKind) -> Vec<String> {
    match kind {
        TargetKind::Lib => vec!["lib".to_string()],
        TargetKind::ProcMacro => vec!["proc-macro".to_string()],
        TargetKind::Bin => vec!["bin".to_string()],
        TargetKind::Example => vec!["example".to_string()],
    }
}

fn metadata_target_crate_types(kind: TargetKind) -> Vec<String> {
    match kind {
        TargetKind::Lib => vec!["lib".to_string()],
        TargetKind::ProcMacro => vec!["proc-macro".to_string()],
        TargetKind::Bin | TargetKind::Example => vec!["bin".to_string()],
    }
}

fn metadata_write_resolve(out: &mut String, ws: &Workspace, package_names: &[String]) {
    out.push_str("{\"nodes\":[");
    for (index, name) in package_names.iter().enumerate() {
        if index > 0 {
            out.push(',');
        ...
        for dep in pkg.deps.values() {
            if let Some(dep_pkg) = ws.packages.get(&dep.package) {
                if !first_dep {
                    out.push(',');
                }
                first_dep = false;
                json_string(out, &metadata_package_id(dep_pkg));
            }
        }
        out.push_str("],\"deps\":[],\"features\":[]}");
    }
    out.push_str("],\"root\":null}");
}
...
fn print_help() {
    println!(
        "edgerun-build <doctor|graph|check|build|test|run|fmt|clean> [options]\n\
         options: -p NAME --lib --bin NAME --bins --example NAME --examples --all-targets --features a,b --all-features --no-default-features --release --target TRIPLE --workspace PATH --out PATH --keep-going -j N --jobs N --keep-ram --fresh --check --all\n\
         output: quiet on success by default; set EDGERUN_BUILD_VERBOSE=1 for reuse and rustc command tracing"
    );
}
