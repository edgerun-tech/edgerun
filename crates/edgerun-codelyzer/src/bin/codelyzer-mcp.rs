...
use edgerun_codelyzer::{
    analyzer::analyze_full, filesystem, mcp_permission, mcp_rust_ast, uir::Program, WIRE_PROTOCOL,
};
...
fn call_tool_response(state: &mut ServerState, id: Value, name: &str, args: Value) -> Value {
    ...
    match result {
        Ok(value) => {
            json!({ "jsonrpc": "2.0", "id": id, "result": { "content": [{ "type": "text", "text": pretty_mcp_json(&value) }], "structuredContent": value } })
        }
        Err(err) => {
            json!({ "jsonrpc": "2.0", "id": id, "result": { "isError": true, "content": [{ "type": "text", "text": err }] } })
        }
    }
}
...
fn ensure_graph(state: &mut ServerState, rescan: bool) -> Result<&CachedGraph, String> {
    ...
    let should_scan = rescan
        || state.graph.as_ref().map(|g| g.fingerprint.as_str()) != Some(fingerprint.as_str());
    if should_scan {
...
fn invalidate_graph(state: &mut ServerState) {
    state.graph = None;
}
...
fn tool_search_symbols(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    for func in graph.program.functions_ordered() {
        let haystack =
            format!("{} {} {} {}", func.id, func.name, func.file, func.language).to_lowercase();
...
fn tool_get_node(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    let func = graph
        .program
        .get_by_legacy(node_id)
        .ok_or_else(|| format!("node not found: {node_id}"))?;
    Ok(
        json!({ "id": func.id, "name": func.name, "file": func.file, "language": func.language, "is_static": func.is_static, "commit": func.last_modified_commit, "incoming": graph.program.get_callers(node_id).len(), "outgoing": graph.program.get_callees(node_id).len() }),
    )
}
...
fn tool_related_nodes(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    for edge in &graph.program.edges {
        if edge.caller == node_id || edge.callee == node_id {
            let other = if edge.caller == node_id {
                &edge.callee
            } else {
                &edge.caller
            };
...
fn tool_read_file(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    let mut content =
        fs::read_to_string(&full_path).map_err(|err| format!("read failed: {err}"))?;
    let truncated = content.len() > max_bytes;
    if truncated {
        content.truncate(max_bytes);
    }
    Ok(
        json!({ "path": path, "absolute_path": full_path, "truncated": truncated, "content": content }),
    )
}
    ...
    for file in &files {
        ...
        for (line_no, line) in content.lines().enumerate() {
            if line.to_lowercase().contains(&needle) {
                results
                    .push(json!({ "path": file.path, "line": line_no + 1, "text": line.trim() }));
                if results.len() >= limit {
                    return Ok(
                        json!({ "query": query, "count": results.len(), "matches": results }),
                    );
                }
            }
        }
    ...
    if full_path.exists() {
        ...
        if let Some(expected) = args.get("expected_contains").and_then(Value::as_str) {
            ...
            if !existing.contains(expected) {
                return Err("expected_contains was not found; refusing write".to_string());
            }
        }
        if backup {
        ...
        if !create {
            return Err("file does not exist; pass create=true".to_string());
        }
...
fn tool_replace_text(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    if matches > 1 && !all {
        return Err(format!(
            "old text matched {matches} times; pass all=true or use more context"
        ));
    }
    let replaced = if all {
...
fn tool_replace_lines(state: &mut ServerState, args: Value) -> Result<Value, String> {
    ...
    if end > lines.len() {
        return Err(format!(
            "end line {end} exceeds file line count {}",
            lines.len()
        ));
    }
...
fn read_resource_response(state: &mut ServerState, id: Value, uri: &str) -> Value {
    ...
    match result {
        Ok(value) => {
            json!({ "jsonrpc": "2.0", "id": id, "result": { "contents": [{ "uri": uri, "mimeType": "application/json", "text": pretty_mcp_json(&value) }] } })
        }
...
fn pretty_mcp_json(value: &Value) -> String {
