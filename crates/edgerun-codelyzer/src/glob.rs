...
pub fn glob_match_with_separator(pattern: &str, path: &str, sep: char) -> bool {
    ...
    if let Some(rest) = pattern.strip_prefix("**") {
        ...
        for start in 0..=path.len() {
            if match_glob(match_pat, 0, &path[start..], 0, sep) {
                return true;
            }
    ...
    match_glob(pattern, 0, path, 0, sep)
}

fn match_glob(pat: &str, pi: usize, path: &str, ti: usize, sep: char) -> bool {
    let mut p_idx = pi;
    let mut t_idx = ti;
    ...
    while p_idx < pat.len() || t_idx < path.len() {
        ...
        match (pc, tc) {
            ...
            (Some("*"), Some(_)) => {
                ...
                if next_is_sep {
                    ...
                    if let Some(pos) = path[t_idx..].find(sep) {
                        ...
                        if match_glob(rest_pat, 0, remaining, 0, sep) {
                            return true;
                        }
                    }
                    return false;
                    ...
                    if rest_pat.is_empty() || match_glob(rest_pat, 0, &path[t_idx..], 0, sep) {
                        return true;
                    }
                    ...
                    while i < path.len() {
                        ...
                        if match_glob(rest_pat, 0, &path[i..], 0, sep) {
                            return true;
                        }
                        i += 1;
                    }
            ...
            (Some("["), Some(t)) => {
                ...
                if !match_char_class(pat, p_idx, ch) {
                    return false;
                }
...
fn match_char_class(pat: &str, start: usize, ch: char) -> bool {
    let rest = &pat[start..];
    ...
    if class.starts_with('!') || class.starts_with('^') {
        let inner = &class[1..];
        !class_contains(inner, ch)
    } else {
        class_contains(class, ch)
    }
}
