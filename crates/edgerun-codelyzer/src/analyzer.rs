...
fn build_program(
    ...
    // Phase 2: Parse files with one parser pool; analyzer defaults should stay
    // predictable and avoid saturating the host while scanning large workspaces.
...
fn build_program_vfs(
    ...
    // Phase 2: Parse files with one parser pool; keep VFS analysis bounded by
    // default just like direct filesystem analysis.
    ...
    // Pre-allocate with reasonable capacity to reduce reallocations
    let mut all_functions_final: Vec<(String, String, bool)> = Vec::with_capacity(256);
    let mut all_calls_final: Vec<(String, String, String, crate::uir::CallKind)> =
    ...
    for (file, caller_name, callee_name, kind) in &all_calls_final {
        ...
        changes
            .edges_added
            .push((caller_fid.to_legacy(), callee_fid.to_legacy(), *kind));
    }
