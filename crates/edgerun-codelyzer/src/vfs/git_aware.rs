...
impl GitignoreRule {
    ...
    fn matches(&self, path: &str) -> bool {
        ...
        crate::glob::glob_match(&self.pattern, basename)
    }
...
impl GitAwarePersist {
    ...
    fn pattern_matches(&self, pattern: &Path, path: &Path) -> bool {
        ...
        if pattern_str.contains('*') {
            return crate::glob::glob_match(&pattern_str, &path_str);
        }
...
mod tests {
    use super::*;
    use crate::vfs::test_support::TempDir;
    ...
    fn test_real_workspace_gitignore() {
        ...
        assert!(persist.should_persist(Path::new("crates/edgerun-codelyzer/src/vfs/mod.rs")));
        assert!(persist.should_persist(Path::new("crates/edgerun-codelyzer/Cargo.toml")));
