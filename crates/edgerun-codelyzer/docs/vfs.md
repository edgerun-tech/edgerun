...
### 1. Mount helper — automatic write-back
...
1. Mounts a tmpfs (RAM disk) at `/tmp/codelyzer-vfs-ram`
...
```rust
use edgerun_codelyzer::vfs::GitAwarePersist;
...
The codelyzer VFS module currently exposes library APIs. The old daemon installer and
benchmark binary references were removed because the crate no longer defines
those binaries.
