...
mod real {
    ...
    #[repr(C)]
    pub struct AInputEvent { _private: [u8; 0] }
    ...
    mod libinput {
        ...
        pub type AInputQueue_finishEvent = unsafe extern "C" fn(*mut AInputQueue, *mut AInputEvent, c_int);
    ...
    pub struct AInputDevice { queue: *mut AInputQueue }
    unsafe impl Send for AInputDevice {}

    impl AInputDevice {
        pub unsafe fn from_raw(queue: *mut AInputQueue) -> Self { Self { queue } }

        pub fn poll_event(&self) -> Option<InputEventRecord> {
            if self.queue.is_null() { return None; }
            ...
            unsafe {
                ...
                let record = if event_type == AINPUT_EVENT_TYPE_KEY {
                    InputEventRecord {
                        ...
                        value: if (fns.get_action)(event) == AKEY_EVENT_ACTION_DOWN { 1 } else { 0 },
                    }
    ...
    pub struct AndroidInputProvider { device: Option<AInputDevice> }
    ...
    impl CapabilityProvider for AndroidInputProvider {
        fn descriptor(&self) -> CapabilityDescriptor {
            capability_descriptor("android-input", "android", CapabilityRole::Input,
                &[CapabilityModality::Touch], &[edgerun_capabilities::CapabilityEventKind::Text],
...
mod real {
    ...
    impl CapabilityProvider for AndroidInputProvider {
        fn descriptor(&self) -> CapabilityDescriptor {
            capability_descriptor("android-input-stub", "stub", CapabilityRole::Input,
                &[CapabilityModality::Touch], &[edgerun_capabilities::CapabilityEventKind::Text],
                &[CapabilityOperation::Query], Vec::new())
        }
    }
...
pub use real::{AndroidInputProvider, AInputDevice};
