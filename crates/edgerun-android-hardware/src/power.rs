...
mod real {
    ...
    pub struct AndroidPowerProvider { last_info: Option<BatteryInfo> }

    impl AndroidPowerProvider {
        pub fn new() -> Self { Self { last_info: None } }
        ...
        pub fn get_battery_info(&mut self) -> Result<BatteryInfo, CapabilityError> {
            let base = if std::path::Path::new("/sys/class/power_supply/battery").exists() {
                ...
                return Err(CapabilityError::Provider("No power supply found in sysfs".into()));
            };

            let level_percent = read_sysfs_prop(&format!("{base}/capacity")).and_then(|s| s.parse().ok()).unwrap_or(-1);
            ...
            let temperature_celsius = read_sysfs_prop(&format!("{base}/temp"))
                .and_then(|s| s.parse::<i32>().ok()).map(|t| t as f32 / 10.0).unwrap_or(0.0);
            let voltage_mv = read_sysfs_prop(&format!("{base}/voltage_now")).and_then(|s| s.parse().ok())
                .or_else(|| read_sysfs_prop(&format!("{base}/voltage_avg")).and_then(|s| s.parse().ok()))
                .unwrap_or(0) as i32;

            let info = BatteryInfo { level_percent, is_charging, health, temperature_celsius, voltage_mv };
    ...
    impl CapabilityProvider for AndroidPowerProvider {
        fn descriptor(&self) -> CapabilityDescriptor {
            capability_descriptor("android-power", "android", CapabilityRole::Input,
...
mod real {
    ...
    impl AndroidPowerProvider {
        ...
        pub fn is_on_ac_power(&self) -> bool { false }
    }
    impl CapabilityProvider for AndroidPowerProvider {
        fn descriptor(&self) -> CapabilityDescriptor {
            capability_descriptor("android-power-stub", "stub", CapabilityRole::Input,
                &[CapabilityModality::Other],
                &[edgerun_capabilities::CapabilityEventKind::Text],
                &[CapabilityOperation::Query], Vec::new())
        }
