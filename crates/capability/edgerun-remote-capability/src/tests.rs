...
impl RemoteCapabilityProvider for DummyProvider {
    ...
    fn open_session(
        ...
        edgerun_protocols::core_protocol::protocol::capability_runtime::CapabilitySessionAccept,
        CapabilityError,
    > {
        self.opened = true;
...
fn wifi_remote_adapter_streams_scan() {
    ...
    let mut adapter =
        WifiRemoteAdapter::new(device, default_wifi_descriptor("dummy-wifi", "wlan0"));
    let open = CapabilitySessionOpen {
        version: 1,
        session_id: b"s".to_vec(),
        selector: None,
...
fn wifi_control_remote_adapter_returns_interface_info() {
    ...
    let mut adapter =
        WifiControlRemoteAdapter::new(device, default_wifi_descriptor("dummy-wifi", "wlan0"));
    let open = CapabilitySessionOpen {
        version: 1,
        session_id: b"s".to_vec(),
        selector: None,
...
fn camera_remote_adapter_streams_capture() {
    let mut device = DummyCameraDevice {
...
fn paired_camera_remote_adapter_streams_capture() {
    let mut device = DummyPairedCameraDevice {
        captures: VecDeque::new(),
    };
...
fn speaker_playback_request_payload_roundtrip() {
    let request = AudioPlaybackRequest {
        ...
        target_output_level_percent: Some(75),
    };
...
fn bluetooth_connections_payload_roundtrip() {
    let connections = vec![BluetoothConnectionInfo {
        ...
        outbound: true,
        state: 1,
        local_name: Some("Connected".into()),
        service_uuids: Vec::new(),
        profiles: vec![BluetoothProfile::AudioSink],
        trusted: Some(true),
        paired: Some(false),
    }];
...
fn capability_error_result_displays_error_reason() {
    ...
    let result = capability_error_result(
        &invocation,
        CapabilityError::PermissionDenied("nope".into()),
    );
