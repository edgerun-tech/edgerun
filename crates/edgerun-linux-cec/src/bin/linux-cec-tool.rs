use edgerun_linux_cec::{discover_cec_adapters, LinuxCecAdapter};
...
fn main() {
    ...
    match args.get(1).map(|value| value.as_str()) {
        ...
        Some("targets") => match edgerun_linux_cec::discover_gpu_cec_targets() {
            Ok(targets) => {
...
where
    F: FnOnce(&mut LinuxCecAdapter) -> Result<(), edgerun_capabilities::CapabilityError>,
{
