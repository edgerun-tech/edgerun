...
impl Duid {
    ...
    pub fn generate_llt(mac: [u8; 6]) -> Self {
        let epoch = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs() as u32;
        // RFC 8415: time is seconds since 2000-01-01 00:00 UTC
        // UNIX epoch is 1970-01-01, so subtract 30 years
        let time_since_2000 = epoch.saturating_sub(946684800);
        Self::llt(1, time_since_2000, &mac) // hw_type 1 = Ethernet
    }
...
pub fn default_server_duid() -> Duid {
    // Use a pseudo-random identifier since we may not have a MAC
    ...
    let epoch = SystemTime::now()
        ...
        .as_secs() as u32;
    let time_since_2000 = epoch.saturating_sub(946684800);
    Duid::llt(1, time_since_2000, &bytes)
}
