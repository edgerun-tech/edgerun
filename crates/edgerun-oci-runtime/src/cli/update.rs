...
pub fn cmd_update(_opts: &crate::cli::GlobalOpts, args: &[String]) -> io::Result<()> {
    ...
    let spec: OciSpec = if let Ok(data) = fs::read(&config_path) {
        crate::json::parse_oci_spec(&data).map_err(|e| {
            io::Error::new(io::ErrorKind::InvalidData, format!("invalid OCI config: {}", e))
        })?
    } else {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "cannot read config.json"));
    };
    ...
    let cgroup_root = std::path::Path::new("/sys/fs/cgroup").join(cgroup_path.trim_start_matches('/'));
...
fn apply_update(cgroup_root: &std::path::Path, opts: &UpdateOpts) -> io::Result<()> {
    ...
    if let Some(limit) = opts.memory {
        if limit >= 0 {
            crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "memory.max", &limit.to_string());
        }
    }
    if let Some(swap) = opts.memory_swap {
        if swap >= 0 {
            crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "memory.swap.max", &swap.to_string());
        }
    }
    ...
    if let Some(shares) = opts.cpu_shares {
        ...
        crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "cpu.weight", &weight.to_string());
    }
    if let Some(quota) = opts.cpu_quota {
        let period = opts.cpu_period.unwrap_or(100_000);
        crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "cpu.max", &format!("{} {}", quota, period));
        ...
        if let Ok(current) = fs::read_to_string(cgroup_root.join("cpu.max")) {
            if let Some(quota_str) = current.split_whitespace().next() {
                crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "cpu.max", &format!("{} {}", quota_str.trim(), period));
            }
        }
    }
    if let Some(rt_runtime) = opts.cpu_rt_runtime {
        if let Some(rt_period) = opts.cpu_rt_period {
            crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "cpu.max.rt", &format!("{} {}", rt_runtime, rt_period));
        }
    }
    ...
    if let Some(limit) = opts.pids_limit {
        if limit > 0 {
            crate::cgroups::setup_container_cgroups_from_file(cgroup_root, "pids.max", &limit.to_string());
        }
    }
...
fn parse_update_flag(args: &[String]) -> io::Result<UpdateOpts> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--memory" | "-m" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--memory requires a value"));
                }
            ...
            "--memory-swap" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--memory-swap requires a value"));
                }
            ...
            "--cpu-shares" | "-c" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpu-shares requires a value"));
                }
                opts.cpu_shares = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid cpu-shares value: {}", args[i]))
                })?);
            }
            "--cpu-quota" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpu-quota requires a value"));
                }
                opts.cpu_quota = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid cpu-quota value: {}", args[i]))
                })?);
            }
            "--cpu-period" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpu-period requires a value"));
                }
                opts.cpu_period = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid cpu-period value: {}", args[i]))
                })?);
            }
            "--cpu-rt-runtime" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpu-rt-runtime requires a value"));
                }
                opts.cpu_rt_runtime = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid cpu-rt-runtime value: {}", args[i]))
                })?);
            }
            "--cpu-rt-period" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpu-rt-period requires a value"));
                }
                opts.cpu_rt_period = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid cpu-rt-period value: {}", args[i]))
                })?);
            }
            "--cpuset-cpus" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpuset-cpus requires a value"));
                }
            ...
            "--cpuset-mems" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--cpuset-mems requires a value"));
                }
            ...
            "--pids-limit" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--pids-limit requires a value"));
                }
                opts.pids_limit = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid pids-limit value: {}", args[i]))
                })?);
            }
            "--blkio-weight" => {
                i += 1;
                if i >= args.len() {
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "--blkio-weight requires a value"));
                }
                opts.blkio_weight = Some(args[i].parse().map_err(|_| {
                    io::Error::new(io::ErrorKind::InvalidInput, format!("invalid blkio-weight value: {}", args[i]))
                })?);
            }
            _ => {
                return Err(io::Error::new(io::ErrorKind::InvalidInput,
    ...
    if opts.memory.is_none() && opts.memory_swap.is_none() && opts.cpu_shares.is_none()
        && opts.cpu_quota.is_none() && opts.cpu_period.is_none() && opts.pids_limit.is_none()
        && opts.blkio_weight.is_none() && opts.cpuset_cpus.is_none() && opts.cpuset_mems.is_none()
        && opts.cpu_rt_runtime.is_none() && opts.cpu_rt_period.is_none() {
        return Err(io::Error::new(io::ErrorKind::InvalidInput,
...
fn parse_memory_arg(s: &str) -> io::Result<i64> {
    let s = s.trim();
    if s.is_empty() {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "empty memory value"));
    }
    ...
    let num: i64 = num_str.parse().map_err(|_| {
        io::Error::new(io::ErrorKind::InvalidInput, format!("invalid memory value: {}", s))
    })?;
