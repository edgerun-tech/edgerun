...
//! It stores typed, derived rows behind a small append-only log. Local
//! audit/replay state still comes from the event log; network authority comes
//! from admitted work and its proofs.
...
mod tests {
    ...
    fn temp_db_path(name: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "edgerun-storage-derived-db-{name}-{}-{unique}.edb",
            std::process::id()
        ))
    }
