//! edgerun storage layer — durable local audit log, encrypted blobs, and rebuildable indexes.
//!
//! Implements the reference storage profile from the protocol spec (§6.1–§6.3, §19.9):
//!
//! - **Local audit log**: durable append/read/scan for already-signed stream events
//! - **File indexes**: binary append-only logs with in-memory HashMaps (rebuildable from local audit log)
//! - **Blob store**: AES-GCM encrypted ciphertext on filesystem, content-addressed
//!
//! Storage proves local durability and replay. Network routing, admission, and
//! payable authority come from admitted work and the proofs that bind to it.
//!
...
//!
//! ## Rebuildability (§6.3)
//! File indexes can be deleted and rebuilt from the local audit log + encrypted blobs.
