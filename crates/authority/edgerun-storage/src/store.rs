//! Unified node storage — combines local audit events, file-based indexes, and encrypted blobs.
//!
//! The `NodeStore` is the high-level storage interface used by the node daemon.
//! It maintains local evidence and rebuildable projections; it does not admit
//! network work or make storage records payable by itself.
//! It coordinates:
...
//! - Append-only local audit log — already-signed stream events on the filesystem
...
pub struct NodeStoreConfig {
    ...
    ///
    /// Creates:
    /// - `{data_root}/events/` — append-only local audit log files (one per stream)
...
impl NodeStore {
    ...
    ///
    /// The event log is the local audit source for stream replay, but callers
    /// using the index for fast lookup should still reject stale or forged index
    /// records immediately.
    ...
    ///
    /// Indexes are NOT the local audit source and SHOULD be rebuildable from
    /// the event log.
    ///
