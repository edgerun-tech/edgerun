//! EdgeRun wallet app.
//!
//! This app has no public HTTP route. It is intended to be verified as an
//! `edgerun-node` runtime projection and addressed through `RuntimeAppMessage` payloads.
...
use crate::runtime_api::{self, RuntimeAppInstall};
use edgerun_protocols::core_protocol::protocol::edgerun_wallet_v0::{
    Asset, PaymentRequest, Receipt, TxRef,
};
use edgerun_wire::{
    Archive, Deserialize, RuntimeAppMessage, SDK_WIRE_ABI_VERSION, SdkWireRecord, Serialize,
    WireError, from_bytes, sdk_wire_bytes, to_bytes,
};
...
impl WalletAppState {
    ...
    fn create_payment_request(&mut self, draft: PaymentRequestDraft) -> WalletAppResult {
        if !is_valid_decimal_amount(&draft.settlement_amount) {
...
pub fn runtime_app_projection(developer_id: [u8; 32]) -> RuntimeAppInstall {
...
pub fn runtime_app_message(
    ...
    RuntimeAppMessage {
        ...
        to_app_id,
        message_kind,
        payload_sha256: crate::sha256(&payload),
        payload,
    }
}
...
fn validate_balance(balance: &WalletBalance) -> Result<(), String> {
    if !is_valid_decimal_amount(&balance.confirmed_amount) {
        return Err("invalid confirmed amount".to_string());
    }
    if !is_valid_decimal_amount(&balance.pending_amount) {
        return Err("invalid pending amount".to_string());
    }
    Ok(())
}

fn is_valid_decimal_amount(input: &str) -> bool {
    let s = input.trim();
    if s.is_empty()
        || s.starts_with('-')
        || s.starts_with('+')
    ...
    let (int_part, frac_part) = if let Some(pos) = s.find('.') {
        let (int_part, rest) = s.split_at(pos);
        (int_part, &rest[1..])
    } else {
        (s, "")
    };
