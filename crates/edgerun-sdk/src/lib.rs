...
pub struct UnitManifest {
    ...
    pub standard_id: i32,
    pub abi: &'static str,
    pub deterministic: Determinism,
    pub wasm_path: &'static str,
    pub manifest_path: &'static str,
    pub wasm_sha256: &'static str,
    pub imports: &'static [ApiFunction],
    pub exports: &'static [ApiFunction],
}
...
pub struct SegmentManifest {
    ...
    pub composition_sha256: &'static str,
...
pub fn sha256_hex(data: &[u8]) -> [u8; 64] {
    ...
    while index < digest.len() {
        out[index * 2] = hex_nibble(digest[index] >> 4);
        out[index * 2 + 1] = hex_nibble(digest[index] & 0x0f);
        index += 1;
    }
    out
}

pub fn str_eq(left: &str, right: &str) -> bool {
...
impl Sha256State {
    ...
    pub(crate) fn update_hex(&mut self, bytes: &[u8]) {
        let mut index = 0;
        while index < bytes.len() {
            self.update(&[
                hex_nibble(bytes[index] >> 4),
                hex_nibble(bytes[index] & 0x0f),
            ]);
            index += 1;
        }
    }
