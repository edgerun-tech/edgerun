...
//!
//! This module does not own admission, settlement custody, or HTTP ingress. It
//! defines rkyv-native marketplace facts that SDK callers can record as local
//! audit events and admitted work, then project back into browse, checkout, run,
//! and cache state.
...
pub struct MarketplaceCheckout {
    ...
    pub run_target_node: Option<NodeRef>,
...
pub struct MarketplaceLegacySettlementCommandDraft {
...
#[rkyv(crate = edgerun_wire)]
pub struct MarketplaceRunReceipt {
    pub listing_id: String,
    pub package_id: String,
    pub receipt_id: String,
    pub run_or_cache_admission_id: Vec<u8>,
    pub runtime_event_id: Vec<u8>,
}
...
pub enum MarketplaceEvent {
    ...
    RunAuthorized {
        checkout_id: String,
        package_id: String,
        run_or_cache_admission_id: Vec<u8>,
        authorized_at_ms: u64,
    },
    PackageCached(MarketplaceRunReceipt),
}
...
pub struct MarketplaceProjection {
    ...
    pub runs: Vec<MarketplaceRunReceipt>,
}
...
pub fn create_checkout_for_listing(
    ...
    run_target_node: Option<NodeRef>,
...
pub fn build_legacy_settlement_command_for_payment_request(
    payment_request: &PaymentRequest,
    draft: MarketplaceLegacySettlementCommandDraft,
) -> CommandEnvelope {
    // Legacy bridge only: this creates an older command envelope carrying the
    // payment request bytes. It does not admit settlement work or make the
    // request payable; callers must bind the request to admitted work/receipts.
...
pub fn apply_marketplace_event(projection: &mut MarketplaceProjection, event: &MarketplaceEvent) {
    match event {
        ...
        MarketplaceEvent::RunAuthorized { .. } => {}
        MarketplaceEvent::PackageCached(receipt) => {
            projection.runs.push(receipt.clone());
        }
    }
}

pub fn authorize_run_or_cache(
    projection: &MarketplaceProjection,
    checkout_id: &str,
    package_id: &str,
    run_or_cache_admission_id: Vec<u8>,
) -> Result<MarketplaceRunReceipt, MarketplaceError> {
    ...
    Ok(MarketplaceRunReceipt {
        listing_id: listing.listing_id.clone(),
        package_id: package_id.into(),
        receipt_id: checkout.receipt_id.clone(),
        run_or_cache_admission_id,
        runtime_event_id: Vec::new(),
    })
}
...
mod tests {
    ...
    #[test]
    fn legacy_settlement_command_carries_listing_payment_request_intent() {
        let request = build_payment_request_for_listing(
            &listing(),
            "pr-1",
            1_700_000_000_000,
        ...
        let command = build_legacy_settlement_command_for_payment_request(
            &request,
            MarketplaceLegacySettlementCommandDraft {
        ...
        match command.payload {
            ...
            _ => panic!("legacy settlement command should carry inline payment request"),
        }
    }

    #[test]
    fn projection_marks_checkout_paid_from_audit_events() {
        let events = vec![
            MarketplaceEvent::PackagePublished(package()),
            MarketplaceEvent::ListingPublished(listing()),
            MarketplaceEvent::CheckoutCreated(checkout()),
    ...
    #[test]
    fn run_or_cache_requires_paid_checkout() {
        ...
        assert_eq!(
            authorize_run_or_cache(
                &projection,
                "checkout-1",
                "pkg-storefront",
                b"run-admission".to_vec()
            ),
    ...
    #[test]
    fn paid_checkout_authorizes_run_or_cache_for_listing_package() {
        let projection = project_marketplace_events(&[
            MarketplaceEvent::PackagePublished(package()),
            MarketplaceEvent::ListingPublished(listing()),
            MarketplaceEvent::CheckoutCreated(checkout()),
        ...
        let receipt = authorize_run_or_cache(
            &projection,
            "checkout-1",
            "pkg-storefront",
            b"run-admission".to_vec(),
        )
        .expect("paid checkout should authorize run/cache");

        assert_eq!(receipt.receipt_id, "receipt-1");
        assert_eq!(
            receipt.run_or_cache_admission_id,
            b"run-admission".to_vec()
        );
    }
