...
document.addEventListener('click',async event=>{const trigger=event.target.closest('[hx-get]');if(!trigger)return;event.preventDefault();const target=document.querySelector(trigger.getAttribute('hx-target')||'');if(!target)return;const path=trigger.getAttribute('hx-get');const response=await fetch(path,{headers:workspaceHeaders(path)});if(!response.ok)return;const html=await response.text();const swap=trigger.getAttribute('hx-swap')||'innerHTML';if(swap==='outerHTML')target.outerHTML=html;else target.innerHTML=html;const hash=trigger.getAttribute('data-dash-hash');if(hash&&location.hash!==hash)history.pushState(null,'',hash);if(trigger.classList.contains('dash-tab')){const hashes={'/surface/blog':'#build-log','/surface/git':'#code','/surface/mail':'#mail'};const next=hashes[path]||location.pathname;if(location.hash!==next)history.pushState(null,'',next);document.querySelectorAll('.dash-tab[aria-current]').forEach(node=>node.removeAttribute('aria-current'));trigger.setAttribute('aria-current','page')}else if(path.startsWith('/surface/git')){document.querySelectorAll('.dash-tab[aria-current]').forEach(node=>node.removeAttribute('aria-current'));const codeTab=document.querySelector('[hx-get="/surface/git"]');if(codeTab)codeTab.setAttribute('aria-current','page')}});
