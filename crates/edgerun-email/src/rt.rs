...
fn bare_io(error: edgerun_bare_rt::IoError) -> io::Error {
    match error {
        edgerun_bare_rt::IoError::UnexpectedEof => {
            io::Error::new(io::ErrorKind::UnexpectedEof, error)
        }
        edgerun_bare_rt::IoError::WriteZero => io::Error::new(io::ErrorKind::WriteZero, error),
        edgerun_bare_rt::IoError::Other(_) => io::Error::other(error),
    }
}
...
impl<T: edgerun_bare_rt::AsyncRead + Unpin> AsyncRead for T {
    fn poll_read(
        ...
        edgerun_bare_rt::AsyncRead::poll_read(self, cx, buf).map_err(bare_io)
    }
}
...
impl<T: edgerun_bare_rt::AsyncWrite + Unpin> AsyncWrite for T {
    fn poll_write(
        ...
        edgerun_bare_rt::AsyncWrite::poll_write(self, cx, buf).map_err(bare_io)
    }

    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        edgerun_bare_rt::AsyncWrite::poll_flush(self, cx).map_err(bare_io)
    }

    fn poll_shutdown(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        edgerun_bare_rt::AsyncWrite::poll_shutdown(self, cx).map_err(bare_io)
    }
}
...
pub struct LockFuture<'a, T> {
    mutex: &'a edgerun_bare_rt::Mutex<T>,
}

impl<'a, T> Future for LockFuture<'a, T> {
    type Output = edgerun_bare_rt::MutexGuard<'a, T>;
...
pub struct ReadLockFuture<'a, T> {
    lock: &'a edgerun_bare_rt::RwLock<T>,
}

impl<'a, T> Future for ReadLockFuture<'a, T> {
    type Output = edgerun_bare_rt::RwLockReadGuard<'a, T>;
...
impl<'a, T> Future for WriteLockFuture<'a, T> {
    type Output = edgerun_bare_rt::RwLockWriteGuard<'a, T>;
