use edgerun_rt::{AsyncTcpListener, AsyncReadExt, AsyncWriteExt, Runtime};
use std::io::Write;
...
fn main() {
    ...
    edgerun_rt::spawn(async {
        ...
        loop {
            ...
            i += 1;
            eprintln!("  [heartbeat {} | last_step={}]", i, STEP.load(Ordering::SeqCst));
        }
    });
    ...
    rt.block_on(async {
        ...
        step(3, &format!("accept returned: {:?}", accept_result.as_ref().map(|(_, a)| a)));
