...
fn signal_wakes_multiple_waiters() {
    ...
    assert!(matches!(Pin::new(&mut signal).poll(&mut cx1), Poll::Pending));
    assert!(matches!(Pin::new(&mut signal).poll(&mut cx2), Poll::Pending));

    signal.raise();

    assert!(matches!(Pin::new(&mut signal).poll(&mut cx1), Poll::Ready(())));
    assert!(matches!(Pin::new(&mut signal).poll(&mut cx2), Poll::Ready(())));
}
