...
//! Secret operations are recorded as signed events in the node-local audit stream
//! (not a separate service log). This keeps secret mutations in the
//! cryptographically linked, prev_hash-chained local evidence trail.
...
/// Callback type for recording a secret event into the node-local audit stream.
...
/// Create a no-op event recorder for use when local audit-stream integration
...
/// - Secret operations are recorded via `record_event` into the node-local
///   signed audit stream (not a separate service log).
pub struct Backend {
    blobs: BlobStore,
    index: FileIndex,
...
impl Backend {
    ...
    pub fn put(
        ...
        // Record event in the node-local audit stream.
    ...
    pub fn delete(&mut self, coll: &str, key: &str) -> io::Result<bool> {
        ...
        // Record the deletion event in the node-local audit stream.
        if existed {
    ...
    /// In production, these come from replaying the node-local signed audit
    /// stream and filtering for secret_* event types.
    ///
    /// Process:
