...
- `runtime_app_install(developer_id, host)` builds a `RuntimeAppInstall`.
...
## 2. Node Installs The App

`edgerun-node::runtime::RuntimeKernel::install_app` is the authority step.

The node records `RUNTIME_EVENT_APP_INSTALLED` with the rkyv archived
`RuntimeAppInstall` payload. After this event, the event log is the source of
truth for the installed app identity, release, manifest hash, declared routes,
storage namespaces, and capability declarations.

## 3. Node Grants Declared Routes

`RuntimeKernel::grant_http_route` only accepts routes already present in the
installed app declaration. A route not declared by the app is rejected and
logged as a denied route grant.
...
3. route the accepted dispatch to the installed app handler/process,
...
- node storage/event log remains authoritative state.

## Current Gap

The install/route/storage model exists in `edgerun-node::runtime`, and the app
can declare install records through `edgerun-sdk::runtime_api`. The missing
bridge is the live app runner: `HttpNodeBinding` does not yet convert accepted
HTTP frames into `RuntimeHttpRequest` plus app IPC/handler invocation.
