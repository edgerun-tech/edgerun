...
//! State is tracked per event and can be mirrored into the node-local audit
//! stream when a node runtime is configured.
...
fn next_generated_id(prefix: &str) -> String {
    let id = NEXT_ID.fetch_add(1, Ordering::Relaxed);
    let mut input = Vec::new();
    input.extend_from_slice(b"edgerun-exchange-api.generated-id.v1");
    input.push(0);
    input.extend_from_slice(prefix.as_bytes());
    input.push(0);
    input.extend_from_slice(&id.to_le_bytes());
    let digest = edgerun_sdk::sha256(&input);
    format!(
        "{prefix}{:016x}{:016x}",
