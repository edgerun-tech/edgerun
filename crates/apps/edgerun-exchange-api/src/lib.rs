...
//!
//! The API does not own admission, settlement authority, or a stream runtime.
//! Exchange API records process-local `ExchangeEvent`s in its store; node-owned
//! code may persist those events as local audit evidence and bind settlement
//! decisions to admitted work, receipts, and proofs.
...
pub fn runtime_app_projection(developer_id: [u8; 32], host: impl AsRef<[u8]>) -> RuntimeAppInstall {
    let app_id = runtime_api::app_id(APP_SLUG, &developer_id);
    let manifest_sha256 = runtime_api::manifest_sha256(APP_SLUG, APP_VERSION, ROUTE_SPECS);
    let release_id = runtime_api::release_id(&app_id, APP_VERSION, &manifest_sha256);
