...
pub fn consume_one(sig: &str) -> io::Result<(String, &str)> {
    if sig.is_empty() {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "empty sig"));
    }
    match sig.as_bytes()[0] {
        b'a' => {
            ...
            if el.starts_with('{') {
                ...
                if end == 0 {
                    return Err(io::Error::new(io::ErrorKind::InvalidData, "bad dict"));
                }
        ...
        b'(' => {
            ...
            if end == 0 {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "bad struct"));
            }
...
impl Rdr {
    ...
    fn by(&mut self, n: usize) -> io::Result<&[u8]> {
        if self.p + n > self.d.len() {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "trunc"));
        }
        let s = self.p;
        self.p += n;
        Ok(&self.d[s..s + n])
...
pub fn decode_msg(data: &[u8]) -> io::Result<Msg> {
    ...
    if data[0] != BYTE_ORDER {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "order"));
    }
    let mt = match data[1] {
        1 => MType::Call,
        2 => MType::Return,
    ...
    while rdr.rem() > 0 {
        let fc = rdr.u8()?;
        if rdr.u8()? != b'v' {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "not var"));
        }
        let sg = rdr.sg()?;
