...
<main class="app">
  <section class="list">
    <div class="top"><div class="brand">Edgerun Mail</div><div class="who">ken@edgerun.tech</div><div class="search"><svg><use href="#i-search"/></svg><input id="search" placeholder="Search mail" autocomplete="off"></div></div>
  ...
  <section class="pane">
    ...
    <form id="compose" class="compose">
      ...
      <div class="row"><button class="icon primary" type="submit" title="Send" aria-label="Send"><svg><use href="#i-send"/></svg></button><button id="cancel" class="icon" type="button" title="Cancel" aria-label="Cancel"><svg><use href="#i-x"/></svg></button><div class="grow"></div></div>
    </form>
...
function setActionState(){const ids=actionIds(),items=ids.map(id=>messages.find(m=>m.id===id)).filter(Boolean);markRead.disabled=!items.some(m=>m.unread);markUnread.disabled=!items.some(m=>!m.unread);deleteMsg.disabled=!items.length}
...
document.getElementById('composeBtn').onclick=()=>{composeEl.classList.add('open');to.focus()};
document.getElementById('cancel').onclick=()=>composeEl.classList.remove('open');
...
composeAttachments.onclick=e=>{const b=e.target.closest('button');if(b){draftAttachments.splice(Number(b.dataset.idx),1);renderDraftAttachments()}};
filePick.onchange=async()=>{for(const file of filePick.files){const data=await new Promise((ok,bad)=>{const r=new FileReader();r.onload=()=>ok(String(r.result).split(',')[1]||'');r.onerror=bad;r.readAsDataURL(file)});draftAttachments.push({name:file.name,contentType:file.type||'application/octet-stream',size:file.size,data})}filePick.value='';renderDraftAttachments();flash('Attached')};
composeEl.onsubmit=async e=>{e.preventDefault();const draft={to:to.value,subject:subject.value,body:body.value,attachments:draftAttachments.map(({name,contentType,data})=>({name,contentType,data}))};flash('Sending...');composeEl.classList.remove('open');try{await api('/api/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(draft)});to.value=subject.value=body.value='';draftAttachments=[];renderDraftAttachments();flash('Sent');load(true)}catch(err){composeEl.classList.add('open');flash(err.message,true)}};
