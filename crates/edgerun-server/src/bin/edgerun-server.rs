...
function dashRoute(){const raw=location.hash||'#build-log';const [hash,query='']=raw.split('?');let path={'#build-log':'/surface/blog','#blog':'/surface/blog','#mail':'/surface/mail','#about':'/surface/blog','#feed':'/surface/blog'}[hash];if(!path&&(hash==='#code'||hash==='#git'||hash==='#crates'||hash==='#source'))path='/surface/git';if(!path&&hash.startsWith('#code/'))path='/surface/git/'+hash.slice(6);return{path,query:new URLSearchParams(query)}}
function filterDashCards(value){const q=(value||'').trim().toLowerCase();const cards=[...document.querySelectorAll('[data-search-card]')];let visible=0;cards.forEach(card=>{const hit=!q||(card.getAttribute('data-search-text')||card.textContent||'').toLowerCase().includes(q);card.hidden=!hit;if(hit)visible++});const empty=document.querySelector('[data-search-empty]');if(empty)empty.hidden=visible!==0||!q}
