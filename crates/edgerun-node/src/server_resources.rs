...
use edgerun_core::protocol::{CommandRef, ObjectKind, ObjectRef};
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DerivedDnsRecord { pub name: String, pub rr_type: String, pub value: String }

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct NodeFacts { pub public_ipv4: Option<String>, pub public_ipv6: Option<String>, pub mail_host_prefix: String }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MailRoute { pub address: String, pub target: String }

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct HttpRoute { pub domain: String, pub content_ref: ContentRef }

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum GenerationEventKind { Planned = 1, Staged = 2, HealthPassed = 3, Promoted = 4, Rollback = 5, Failed = 6 }

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct GenerationEvent { pub generation_id: [u8; 32], pub kind: GenerationEventKind, pub plan_hash: [u8; 32], pub reason: String }

pub fn is_server_resource_command(command_type: i32) -> bool { matches!(command_type, 1012..=1023) }
...
where I: IntoIterator<Item = ServerResourceEvent> {
    let mut projection = ServerResourceProjection::default();
    for event in events { apply_server_resource_event(&mut projection, event)?; }
...
pub fn project_server_resources(store: &NodeStore, stream_id: &[u8]) -> Result<ServerResourceProjection, String> {
    ...
    if head_seq == 0 { return Ok(ServerResourceProjection::default()); }
    ...
    for (seq, _hash, _ver) in events {
        ...
        if let Ok(resource_event) = decode_committed_resource_event(&object.content) { apply_server_resource_event(&mut projection, resource_event)?; }
    }
    let plan = compile_server_plan(&projection, &NodeFacts::default());
    projection.derived_dns_records = plan.dns_records;
    projection.required_ports = plan.required_ports;
...
pub fn result_object_for_event(store: &mut NodeStore, stream_id: &[u8], event: &ServerResourceEvent) -> Result<ObjectRef, String> {
    ...
    store.put_object(&payload, ObjectKind::DerivedView as i32, &[stream_id.to_vec()]).map_err(|e| format!("storage_failed: {e}"))
}

fn normalize_and_validate_event(event: ServerResourceEvent) -> Result<ServerResourceEvent, String> {
    match event {
        ServerResourceEvent::ClaimDomain { domain } => { validate_domain(&domain)?; Ok(ServerResourceEvent::ClaimDomain { domain: normalize_domain(&domain) }) }
        ServerResourceEvent::ReleaseDomain { domain } => { validate_domain(&domain)?; Ok(ServerResourceEvent::ReleaseDomain { domain: normalize_domain(&domain) }) }
        ServerResourceEvent::AddMailbox { address } => { validate_address(&address)?; Ok(ServerResourceEvent::AddMailbox { address: normalize_address(&address) }) }
        ServerResourceEvent::RemoveMailbox { address } => { validate_address(&address)?; Ok(ServerResourceEvent::RemoveMailbox { address: normalize_address(&address) }) }
        ServerResourceEvent::AddAlias { address, target } => { validate_address(&address)?; validate_address(&target)?; Ok(ServerResourceEvent::AddAlias { address: normalize_address(&address), target: normalize_address(&target) }) }
        ServerResourceEvent::RemoveAlias { address } => { validate_address(&address)?; Ok(ServerResourceEvent::RemoveAlias { address: normalize_address(&address) }) }
        ServerResourceEvent::AuthorizeContentSource { repo, allowed_ref, allowed_paths } => {
        ...
        ServerResourceEvent::PublishWebsite { domain, content_ref } => { validate_domain(&domain)?; validate_content_ref(&content_ref)?; Ok(ServerResourceEvent::PublishWebsite { domain: normalize_domain(&domain), content_ref }) }
        ServerResourceEvent::UnpublishWebsite { domain } => { validate_domain(&domain)?; Ok(ServerResourceEvent::UnpublishWebsite { domain: normalize_domain(&domain) }) }
        ServerResourceEvent::SetAuthoritativeDns { domain, enabled } => { validate_domain(&domain)?; Ok(ServerResourceEvent::SetAuthoritativeDns { domain: normalize_domain(&domain), enabled }) }
        ServerResourceEvent::RequestCertificate { name } => { validate_domain(&name)?; Ok(ServerResourceEvent::RequestCertificate { name: normalize_domain(&name) }) }
...
fn require_email(plan: &mut DerivedServerPlan) { plan.required_services.insert("edgerun-email".into()); plan.required_ports.extend([25, 465, 587, 993]); }
...
fn validate_content_authorized(projection: &ServerResourceProjection, content_ref: &ContentRef) -> Result<(), String> {
    ...
    if !auth.allowed_paths.iter().any(|p| content_ref.path == *p || content_ref.path.starts_with(&format!("{}/", p.trim_end_matches('/')))) { return Err("content_path_not_authorized".into()); }
    Ok(())
}
...
fn normalize_domain(domain: &str) -> String { domain.trim().trim_end_matches('.').to_ascii_lowercase() }
...
fn address_domain(address: &str) -> Result<&str, String> { address.split_once('@').map(|(_, d)| d).ok_or_else(|| "invalid_email_address".into()) }
