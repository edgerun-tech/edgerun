...
//!
//! The node owns exactly one event stream. Exchange code must not create a
//! separate stream runtime. It only produces `ExchangeEvent` values; this module
//! stores their payload objects and appends signed envelopes to the node stream.
...
pub fn append_exchange_event_to_node_stream(
    ...
    append_exchange_event_to_node_stream_with_recipients(store, stream_id, signer, event, &recipients)
}
...
pub fn append_exchange_events_to_node_stream(
    ...
    for event in events {
        seqs.push(append_exchange_event_to_node_stream(store, stream_id, signer, event)?);
    }
    Ok(seqs)
}
