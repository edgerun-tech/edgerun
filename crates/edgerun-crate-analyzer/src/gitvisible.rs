...
impl Default for VisibilityPolicy {
    fn default() -> Self {
        Self {
            ...
            hidden_patterns: vec![
                ".git/**".into(), "target/**".into(), "*.secret".into(),
                "*.key".into(), "*.pem".into(), ".env*".into(),
                "**/secrets/**".into(), "**/private/**".into(),
            ],
        }
    }
}
...
pub fn is_path_visible(path: &Path, policy: &VisibilityPolicy, crate_dir: &Path) -> bool {
    ...
    for hidden in &policy.hidden_patterns {
        if match_glob(&rel_str, hidden) { return false; }
    }
    for visible in &policy.visible_patterns {
        if match_glob(&rel_str, visible) { return true; }
    }
    false
}
