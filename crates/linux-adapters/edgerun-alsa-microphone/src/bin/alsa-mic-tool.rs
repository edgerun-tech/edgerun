...
fn usage() {
    eprintln!(
        "usage: alsa-mic-tool list | capture <card> <device> <duration_ms> [rate] [channels]"
    );
}
...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some("capture") => {
            ...
            let Some(pcm) = discover_alsa_pcms().ok().and_then(|v| {
                v.into_iter()
                    .find(|p| p.card_index == card && p.device_index == device)
            }) else {
