...
pub fn default_root_hints() -> Vec<RootHint> {
    vec![
        RootHint {
            name: "a.root-servers.net".into(),
            addrs: vec!["198.41.0.4".parse().unwrap()],
        },
        RootHint {
            name: "b.root-servers.net".into(),
            addrs: vec!["199.9.14.201".parse().unwrap()],
        },
        RootHint {
            name: "c.root-servers.net".into(),
            addrs: vec!["192.33.4.12".parse().unwrap()],
        },
        RootHint {
            name: "d.root-servers.net".into(),
            addrs: vec!["199.7.91.13".parse().unwrap()],
        },
        RootHint {
            name: "e.root-servers.net".into(),
            addrs: vec!["192.203.230.10".parse().unwrap()],
        },
        RootHint {
            name: "f.root-servers.net".into(),
            addrs: vec!["192.5.5.241".parse().unwrap()],
        },
        RootHint {
            name: "g.root-servers.net".into(),
            addrs: vec!["192.112.36.4".parse().unwrap()],
        },
        RootHint {
            name: "h.root-servers.net".into(),
            addrs: vec!["198.97.190.53".parse().unwrap()],
        },
        RootHint {
            name: "i.root-servers.net".into(),
            addrs: vec!["192.36.148.17".parse().unwrap()],
        },
        RootHint {
            name: "j.root-servers.net".into(),
            addrs: vec!["192.58.128.30".parse().unwrap()],
        },
        RootHint {
            name: "k.root-servers.net".into(),
            addrs: vec!["193.0.14.129".parse().unwrap()],
        },
        RootHint {
            name: "l.root-servers.net".into(),
            addrs: vec!["199.7.83.42".parse().unwrap()],
        },
        RootHint {
            name: "m.root-servers.net".into(),
            addrs: vec!["202.12.27.33".parse().unwrap()],
        },
    ]
}
...
impl RecursiveResolver {
    ...
    pub async fn resolve(
        ...
        let root_addrs: Vec<Ipv4Addr> = self
            .root_hints
            .iter()
            .flat_map(|h| h.addrs.iter().copied())
            .collect();
        self.iterative_resolve(name_lower, qtype, root_addrs, 0)
            .await
    }
            ...
            for server in &servers {
                ...
                let response =
                        ...
                        .await
                    {
                        ...
                        Ok(Err(e)) => {
                            edgerun_log::debug!(
                                "edgerun-dns: recursive query to {} failed: {}",
                                server,
                                e
                            );
                            continue;
                        ...
                        Err(_) => {
                            edgerun_log::debug!(
                                "edgerun-dns: recursive query to {} timed out",
                                server
                            );
                            continue;
                        }
                    ...
                    DnsResponseCode::NoError => {
                        ...
                        if let Some(ns_names) = self.extract_ns_referral(&resp) {
                            ...
                            if !ns_addrs.is_empty() {
                                return self
                                    .iterative_resolve(name, qtype, ns_addrs, depth + 1)
                                    .await;
                            }
                            ...
                            for ns_name in &ns_names {
                                if let Ok(addrs) = self
                                    ...
                                    .await
                                {
                                    let ip_addrs: Vec<Ipv4Addr> = addrs
                                        .iter()
                                        .filter_map(|r| {
                                            if let DnsRecordData::A(ip) = &r.data {
                                                Some(*ip)
                                            } else {
                                                None
                                            }
                                    ...
                                    if !ip_addrs.is_empty() {
                                        return self
                                            .iterative_resolve(name, qtype, ip_addrs, depth + 1)
                                            .await;
                                    }
                                }
                            }
    ...
    fn extract_ns_referral(&self, resp: &DnsMessage) -> Option<Vec<String>> {
        let ns_names: Vec<_> = resp
            ...
            .filter_map(|r| {
                if let DnsRecordData::NS(name) = &r.data {
                    Some(name.clone())
                } else {
                    None
                }
    ...
    fn cache_answer(&self, name: &str, qtype: DnsRecordType, records: &[DnsRecord]) {
        ...
        self.answer_cache
            .lock()
            .unwrap()
            .insert(key, (records.to_vec(), expiry));
    }
            ...
            if rr.rtype == DnsRecordType::A {
                if let DnsRecordData::A(ip) = &rr.data {
                    ...
                    self.ns_cache
                        .lock()
                        .unwrap()
                        .insert(rr.name.to_lowercase(), (*ip, expiry));
                }
            }
                            ...
                            && glue.name.to_lowercase() == name.to_lowercase()
                        {
                            if let DnsRecordData::A(ip) = &glue.data {
                                ...
                                self.ns_cache
                                    .lock()
                                    .unwrap()
                                    .insert(name.to_lowercase(), (*ip, expiry));
                            }
                        }
                    }
                }
...
mod tests {
    ...
    fn test_extract_ns_referral() {
        ...
        resp.authority.push(DnsRecord::ns(
            "example.com".into(),
