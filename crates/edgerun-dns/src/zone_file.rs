...
fn parse_err(line: usize, msg: &str) -> ZoneFileError {
    ZoneFileError { line, message: msg.to_string() }
}
...
impl ZoneFileParser {
    fn new() -> Self {
        Self { origin: String::new(), default_ttl: 3600, last_owner: String::new(), records: Vec::new(), line_num: 0 }
    }
    ...
    fn join_continuations(&self, input: &str) -> String {
        ...
        for line in input.lines() {
            ...
            if in_parens {
                ...
                if trimmed.contains(')') { in_parens = false; } else { result.push(' '); }
            } else {
    ...
    fn parse_directive(&mut self, line: &str) -> Result<(), ZoneFileError> {
        ...
        match directive.as_str() {
            "$ORIGIN" => {
                ...
                if self.origin.is_empty() { return Err(parse_err(self.line_num, "$ORIGIN value is empty")); }
            }
            "$TTL" => {
                self.default_ttl = value.parse().map_err(|_| parse_err(self.line_num, &format!("invalid $TTL: {}", value)))?;
            }
            "$INCLUDE" => { edgerun_log::warn!("edgerun-dns: $INCLUDE not supported, line {}", self.line_num); }
            _ => { return Err(parse_err(self.line_num, &format!("unknown directive: {}", parts[0]))); }
        }
        Ok(())
    }
    ...
    fn parse_record_line(&mut self, line: &str) -> Result<(), ZoneFileError> {
        ...
        if pos >= tokens.len() { return Err(parse_err(self.line_num, "expected record type")); }

        let mut ttl = self.default_ttl;
        if let Ok(parsed_ttl) = tokens[pos].parse::<u32>() {
            ttl = parsed_ttl; pos += 1;
            if pos >= tokens.len() { return Err(parse_err(self.line_num, "expected record type")); }
        }
        ...
        let rtype = rtype_from_str(&tokens[pos].to_uppercase()).ok_or_else(|| parse_err(self.line_num, &format!("unknown record type: {}", tokens[pos])))?;
        pos += 1;
    ...
    fn build_record(&self, owner: &str, rtype: DnsRecordType, ttl: u32, rdata: &[String]) -> Result<DnsRecord, ZoneFileError> {
        ...
        let parse_u16 = |s: &String, field: &str| -> Result<u16, ZoneFileError> {
            s.parse().map_err(|_| parse_err(ln, &format!("invalid {}: {}", field, s)))
        };
        ...
        match rtype {
            DnsRecordType::A => {
                let ip = Ipv4Addr::from_str(expect(0, "A: expected IPv4")?).map_err(|e| parse_err(ln, &e.to_string()))?;
                Ok(DnsRecord::a(owner.to_string(), ip, ttl))
            }
            DnsRecordType::AAAA => {
                let ip = std::net::Ipv6Addr::from_str(expect(0, "AAAA: expected IPv6")?).map_err(|e| parse_err(ln, &e.to_string()))?;
                Ok(DnsRecord::aaaa(owner.to_string(), ip, ttl))
            }
            DnsRecordType::CNAME => Ok(DnsRecord::cname(owner.to_string(), normalize_target(expect(0, "CNAME: expected target")?, &self.origin), ttl)),
            DnsRecordType::NS => Ok(DnsRecord::ns(owner.to_string(), normalize_target(expect(0, "NS: expected nameserver")?, &self.origin), ttl)),
            DnsRecordType::PTR => Ok(DnsRecord::ptr(owner.to_string(), normalize_target(expect(0, "PTR: expected target")?, &self.origin), ttl)),
            DnsRecordType::MX => {
                let priority = parse_u16(expect(0, "MX: expected priority")?, "priority")?;
                Ok(DnsRecord::mx(owner.to_string(), priority, normalize_target(expect(1, "MX: expected exchange")?, &self.origin), ttl))
            }
            DnsRecordType::TXT => {
                let text = rdata.iter().map(|s| s.replace('"', "")).collect::<Vec<_>>().join(" ");
                Ok(DnsRecord::txt(owner.to_string(), text.trim().to_string(), ttl))
            }
            DnsRecordType::SRV => {
                Ok(DnsRecord { name: owner.to_string(), rtype, rclass: 1, ttl,
...
fn rtype_from_str(s: &str) -> Option<DnsRecordType> {
    match s {
        "A"=>Some(DnsRecordType::A),"AAAA"=>Some(DnsRecordType::AAAA),"CNAME"=>Some(DnsRecordType::CNAME),
        "NS"=>Some(DnsRecordType::NS),"MX"=>Some(DnsRecordType::MX),"TXT"=>Some(DnsRecordType::TXT),
        "PTR"=>Some(DnsRecordType::PTR),"SOA"=>Some(DnsRecordType::SOA),"SRV"=>Some(DnsRecordType::SRV),
        "NAPTR"=>Some(DnsRecordType::NAPTR),"CAA"=>Some(DnsRecordType::CAA),"TLSA"=>Some(DnsRecordType::TLSA),
...
mod tests {
    ...
    fn test_parse_cname() {
        ...
        let zone = parse_zone_file(z).unwrap();
        assert_eq!(zone.resolve("blog.example.com", DnsRecordType::CNAME).unwrap().len(), 1);
    }
    #[test]
    fn test_parse_error() {
    ...
    fn test_relative_owner() {
        let z = "$TTL 3600\n$ORIGIN example.com.\nwww IN A 1.2.3.4\n    IN A 1.2.3.5\n";
        let zone = parse_zone_file(z).unwrap();
        assert_eq!(zone.resolve("www.example.com", DnsRecordType::A).unwrap().len(), 2);
    }
}
