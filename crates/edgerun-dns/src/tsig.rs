...
//! let signed_wire = signer.sign(&query_wire, 0)?;
//!
...
impl TsigSigner {
    ...
    pub fn sign_wire(&self, wire: &[u8], request_mac: &[u8]) -> Result<Vec<u8>, io::Error> {
        ...
        let signed = self.sign(&msg, request_mac)?;
        Ok(signed.to_wire())
    }
...
impl TsigRdata {
    /// Serialize TSIG RDATA without the MAC (for MAC computation).
    pub fn to_wire_without_mac(&self) -> Vec<u8> {
        let mut buf = Vec::new();
        buf.extend(encode_tsig_name(&self.algorithm));
        // 48-bit time
        ...
        // MAC size placeholder (will be replaced)
