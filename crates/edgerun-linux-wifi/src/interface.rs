...
use crate::eapol::parse_mac_string;
...
pub(crate) fn read_rfkill_state(phy_name: &str, rfkill_root: &Path) -> WifiPowerState {
...
pub(crate) fn set_essid(name: &str, ssid: &str) -> Result<(), CapabilityError> {
    let fd = open_ioctl_socket().map_err(CapabilityError::Provider)?;
    let mut buf = [0u8; 34];
...
pub(crate) fn set_frequency_mhz(name: &str, frequency_mhz: u32) -> Result<(), CapabilityError> {
    let fd = open_ioctl_socket().map_err(CapabilityError::Provider)?;
    let mut req = make_iwreq(name);
    req.u = IwreqData {
        freq: IwFreq {
...
pub fn wifi_mode_to_u32(mode: WifiInterfaceMode) -> u32 {
    match mode {
...
pub fn wifi_mode_from_u32(mode: u32) -> WifiInterfaceMode {
    match mode {
...
pub(crate) fn query_mode(name: &str) -> Result<WifiInterfaceMode, CapabilityError> {
    let fd = open_ioctl_socket().map_err(CapabilityError::Provider)?;
    let mut req = make_iwreq(name);
...
pub(crate) fn set_mode(name: &str, mode: WifiInterfaceMode) -> Result<(), CapabilityError> {
    let fd = open_ioctl_socket().map_err(CapabilityError::Provider)?;
    let mut req = make_iwreq(name);
    req.u = IwreqData {
...
pub fn read_wireless_quality(interface_name: &str) -> Option<i16> {
...
pub fn set_rfkill_block(phy_name: &str, blocked: bool) -> Result<bool, CapabilityError> {
