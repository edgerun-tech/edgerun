...
pub fn put_nla(buf: &mut Vec<u8>, attr_type: u16, data: &[u8]) {
...
pub fn put_nla_u32(buf: &mut Vec<u8>, attr_type: u16, value: u32) {
    put_nla(buf, attr_type, &value.to_ne_bytes());
}

pub fn put_nla_string(buf: &mut Vec<u8>, attr_type: u16, s: &[u8]) {
    put_nla(buf, attr_type, s);
}

pub fn put_nla_u16(buf: &mut Vec<u8>, attr_type: u16, value: u16) {
    put_nla(buf, attr_type, &value.to_ne_bytes());
}

pub fn put_nla_u8(buf: &mut Vec<u8>, attr_type: u16, value: u8) {
    put_nla(buf, attr_type, &[value]);
}

pub fn put_nla_nested(buf: &mut Vec<u8>, attr_type: u16, inner: &[u8]) {
...
pub(crate) fn parse_ctrl_getfamily_response(data: &[u8]) -> Result<u16, CapabilityError> {
