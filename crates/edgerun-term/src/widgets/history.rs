...
impl HistoryMenu {
    ...
    pub fn layout(
        ...
        let w = ((width as f32 * 0.78) as i32).max(1);
        let h = ((height as f32 * 0.82) as i32).max(1);
        let x0 = ((width as i32 - w) / 2).max(0);
        let y0 = ((height as i32 - h) / 2).max(0);
        let x1 = (x0 + w).min(width as i32);
        let y1 = (y0 + h).min(height as i32);
    ...
    pub fn draw_cpu(&mut self, glyphs: &mut GlyphCache, frame: &mut [u8], width: u32, height: u32) {
        ...
        crate::render::primitives::fill_rect(
            frame,
            width,
            height,
            x0,
