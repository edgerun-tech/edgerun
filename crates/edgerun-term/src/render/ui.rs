...
use crate::render::{GlyphCache, draw_text_line_clipped, fill_rect, rainbow};
...
pub fn draw_tab_bar_cpu(
    ...
    let text_y = bar_top + ((tab_bar_height as i32 - glyphs.cell_height() as i32) / 2).max(0);
    ...
    for (idx, tab) in tabs.iter().enumerate() {
        ...
        let phase = t + idx as f32 * 0.8;
        let [r, g, b] = rainbow(phase);
        ...
        let text_color = if idx == active { [0, 0, 0, 255] } else { FG };
        let text_x = x + 7;
...
pub fn build_tab_bar_gpu(
    ...
    let text_y = bar_top + ((tab_bar_height as f32 - glyphs.cell_height() as f32) / 2.0).max(0.0);
    let t = start_time.elapsed().as_secs_f32();
    ...
    for (idx, tab) in tabs.iter().enumerate() {
        ...
        let phase = t + idx as f32 * 0.8;
        let [r, g, b] = crate::gpu::GpuRenderer::rainbow(phase);
        let bg = if idx == active {
            Rgba { r, g, b, a: 180 }
        } else {
            Rgba {
