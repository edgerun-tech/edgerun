...
pub const FG: [u8; 4] = [230, 230, 230, 255];
...
fn write_clipboard_text(text: &str) {
    let cb = match CLIPBOARD_HOOK.lock() {
...
pub fn resize_cells(buf: &mut Vec<Cell>, cols: usize, rows: usize, default_fg: Rgba, default_bg: Rgba) {
...
impl Terminal {
    ...
    pub fn scroll_view_offset(&mut self, lines: i32) {
        ...
        self.view_offset = target as usize;
    }

    pub fn display_cell(&self, col: usize, row: usize) -> Cell {
        if col >= self.cols {
            return Cell::blank_with(self.default_fg, self.default_bg);
        }
    ...
    pub fn put_char(&mut self, ch: char) {
        ...
        let line_limit = self.right_margin.min(self.cols.saturating_sub(1)).saturating_add(1);
    ...
    pub fn leave_alt_screen(&mut self) {
        ...
        // Clear the visible grid so programs like neovim leave a clean screen
        // when they exit the alternate screen.
        self.clear_all();
    }
...
impl Perform for GridPerformer<'_> {
    fn hook(&mut self, params: &Params, _intermediates: &[u8], _ignored: bool, action: char) {
        ...
        let param_at = |idx: usize| -> Option<u16> {
            params
                .iter()
                .nth(idx)
                .and_then(|p| p.get(0))
        ...
        match action {
            ...
            'h' | 'l' => {
                    ...
                    && matches!(action, 's' | 't')
                {
                    ...
                    self.grid.set_left_right_margin(left, right.saturating_sub(1));
                }
            }
            ...
            'm' => {
                ...
                while idx < flat_params.len() {
                    ...
                    match p {
                        ...
                        30..=37 => self
                            .grid
                            .set_fg(self.grid.palette_color((p - 30) as usize)),
                        90..=97 => self
                            .grid
                        ...
                        40..=47 => self
                            .grid
                            .set_bg(self.grid.palette_color((p - 40) as usize)),
                        100..=107 => self
                            .grid
                        ...
                        if let Some(spec) = merged.get(idx + 1) {
                            if let Ok(text) = std::str::from_utf8(spec) {
                                if let Some(color) = parse_osc_color(text) {
                                    ...
                                    self.grid
                                        .reset_palette_indices(Some(vec![color_idx]));
                                }
                            }
                            idx += 2;
                            continue;
...
mod tests {
    ...
    fn osc11_updates_default_background_and_cells() {
        ...
        assert_eq!(bg, Rgba { r: 0x11, g: 0x22, b: 0x33, a: 255 });
    ...
    fn osc11_query_reports_current_background() {
        ...
        assert_eq!(String::from_utf8(out).unwrap(), "\u{1b}]11;rgb:01/02/03\u{07}");
    }
