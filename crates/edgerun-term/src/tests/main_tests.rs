use super::*;
use crate::app_render::{UiMode, current_ui_mode};
use crate::suggest::guess_command_paths;
use std::fs;
use std::io;
use std::time::{Duration, Instant};
use tempfile;
use term::render::{draw_background, draw_grid, draw_text_line, draw_text_line_clipped};
use term::terminal::{DEFAULT_BG, DEFAULT_FG, Rgba, ansi_color, brightened, xterm_color};
use vte::Parser as VteParser;
use winit::keyboard::ModifiersState as ModState;
...
fn cell_background_pixels_are_rendered() {
    ...
    term.pen_bg = bg;
    term.put_char(' ');
    term.view_offset = 1; // hide cursor overlay so background is testable
...
fn alt_screen_round_trip_restores_primary() {
    ...
    assert!(!term.in_alt_screen());
    // We now clear the primary grid when leaving alt screen to mimic
    // curses/nvim clean exits. Content is gone but scrollback remains.
    assert_eq!(term.display_cell(0, 0).text, " ");
    assert_eq!(term.display_cell(1, 0).text, " ");
}

#[test]
fn alt_screen_exit_clears_visible_grid() {
    let mut term = Terminal::new(3, 2);
    term.put_char('h');
    term.put_char('i');
    ...
    // Exiting programs like neovim should leave a clean slate.
    assert_eq!(term.display_cell(0, 0).text, " ");
    assert_eq!(term.display_cell(1, 0).text, " ");
...
#[test]
fn function_key_sequence_encodes_modifiers() {
    let mut mods = ModState::empty();
    let seq = input::function_key_sequence(NamedKey::F5, mods).unwrap();
    assert_eq!(seq, "\x1b[15;1~");
    mods.set(ModState::SHIFT, true);
    let seq = input::function_key_sequence(NamedKey::F1, mods).unwrap();
    assert_eq!(seq, "\x1b[1;2P");
}
...
#[test]
fn primary_selection_bracketed_paste_wraps() {
    let data = b"printf 'x'";
    let wrapped = compose_bracketed_paste(data, true);
    assert_eq!(wrapped, b"\x1b[200~printf 'x'\x1b[201~");
}

#[test]
fn non_bracketed_paste_passthrough() {
    let data = b"printf 'x'";
    let wrapped = compose_bracketed_paste(data, false);
    assert_eq!(wrapped, data);
}
