...
mod wasm {
    ...
    struct AppState {
        ...
        cell_w: u32,
        cell_h: u32,
        layout: edgerun_term_core::render::layout::LayoutMetrics,
        frame: Vec<u8>,
    ...
    pub async fn start() -> Result<(), JsValue> {
        ...
        let render_backend = if has_gpu {
            match init_gpu(&canvas, width, height).await {
                Ok((gpu, _format)) => {
                    set_status(&status_el, Some("WebGPU active"));
                    RenderBackend::WebGpu(Rc::new(RefCell::new(gpu)))
                }
                Err(err) => {
                    web_sys::console::warn_1(&JsValue::from_str(&format!(
                        "WebGPU init failed, falling back to 2D: {err:?}"
                    )));
                    set_status(&status_el, Some("WebGPU unavailable, using 2D canvas"));
                    RenderBackend::Canvas2d(init_canvas_2d(&canvas)?)
                }
            }
        } else {
            set_status(&status_el, Some("WebGPU disabled, using 2D canvas"));
            RenderBackend::Canvas2d(init_canvas_2d(&canvas)?)
        };
    ...
    fn render_gpu(
        ...
        let mut encoder = gpu
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("edgerun-term-web-encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("edgerun-term-web-pass"),
    ...
    fn render_canvas_2d(
        ...
        let needs_init = frame
            .image_data
            .as_ref()
            .map(|data| data.width() != width || data.height() != height)
            .unwrap_or(true);
        if needs_init {
        ...
        if let Some(data) = frame.image_data.as_ref() {
            let src = Uint8ClampedArray::view(&frame.frame);
            data.data().set(&src, 0);
    ...
    async fn init_gpu(
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("edgerun-term-web-device"),
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("edgerun-term-web-sampler"),
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("edgerun-term-web-bind-group-layout"),
            entries: &[
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("edgerun-term-web-shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shader.wgsl").into()),
        });

        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("edgerun-term-web-pipeline-layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });

        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("edgerun-term-web-pipeline"),
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("edgerun-term-web-quad"),
    ...
    fn create_frame_texture(
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("edgerun-term-web-frame"),
            size: wgpu::Extent3d {
                width: width.max(1),
                height: height.max(1),
                depth_or_array_layers: 1,
    ...
    fn create_bind_group_with_layout(
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("edgerun-term-web-bind-group"),
            layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
