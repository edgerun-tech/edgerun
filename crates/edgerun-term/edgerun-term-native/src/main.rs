...
use dirs::config_dir;
use edgerun_term_core::gpu::GpuRenderer;
use edgerun_term_core::render::layout::{LayoutMetrics, compute_layout};
use edgerun_term_core::render::{FONT_DATA, FONT_SIZE, GlyphCache};
use edgerun_term_core::terminal::Rgba;
use edgerun_term_core::terminal::{
    GridPerformer, Terminal, copy_text_to_clipboard, selection_text, write_bytes,
};
use edgerun_term_ui::app_render::{RenderInputs, TabRender, render_frame};
use edgerun_term_ui::debug::{DebugOverlay, DebugRenderMode};
use edgerun_term_ui::input;
use edgerun_term_ui::suggest::{looks_like_path, path_completion_suggestions};
use edgerun_term_ui::widgets::history::{MenuColumn, MenuEntry};
use edgerun_term_ui::widgets::settings::SystemFont;
use edgerun_term_ui::widgets::{
                        ...
                        if tab.terminal.mouse_btn_report {
                            let kind = match delta {
                                winit::event::MouseScrollDelta::LineDelta(_, y) if y > 0.0 => {
                                    edgerun_term_core::terminal::MouseEventKind::WheelUp
                                }
                                winit::event::MouseScrollDelta::LineDelta(_, y) if y < 0.0 => {
                                    edgerun_term_core::terminal::MouseEventKind::WheelDown
                                }
                                ...
                                _ => edgerun_term_core::terminal::MouseEventKind::WheelUp,
                            };
                ...
                if let Some(tab) = tabs.get_mut(active_tab) {
                    ...
                    if reporting {
                        ...
                        tab.terminal.report_mouse_event(
                            cell.0,
                            cell.1,
                            edgerun_term_core::terminal::MouseEventKind::Motion,
                            &shell.writer,
                        );
                        tab.link_hover = None;
                        tab.hover_link = None;
                                    ...
                                    if reporting {
                                        if let Some((c, r)) = cell {
                                            ...
                                            tab.terminal.report_mouse_event(
                                                c,
                                                r,
                                                edgerun_term_core::terminal::MouseEventKind::Press(0),
                                                &shell.writer,
                                            );
                                        }
                                        tab.selecting = false;
                                    ...
                                    if reporting {
                                        if let Some((c, r)) = cell {
                                            ...
                                            tab.terminal.report_mouse_event(
                                                c,
                                                r,
                                                edgerun_term_core::terminal::MouseEventKind::Release,
                                                &shell.writer,
                                            );
                                        }
                                    } else if tab.selecting {
...
fn help_state_path() -> Option<PathBuf> {
    #[cfg(test)]
    if let Some(path) = HELP_STATE_OVERRIDE
        .get()
...
#[cfg(test)]
static HELP_STATE_OVERRIDE: OnceLock<Mutex<Option<PathBuf>>> = OnceLock::new();
...
mod ui_tests {
    ...
    fn help_visibility_persists_to_config() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("term").join("help.json");
        let override_slot = HELP_STATE_OVERRIDE.get_or_init(|| Mutex::new(None));
        *override_slot.lock().unwrap() = Some(path.clone());
