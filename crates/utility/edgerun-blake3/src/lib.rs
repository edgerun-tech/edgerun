...
impl Hash {
    #[inline]
    pub const fn as_bytes(&self) -> &[u8; OUT_LEN] { &self.0 }
...
impl ChunkState {
    fn new(key: [u32; 8], chunk_counter: u64, flags: u32) -> Self {
        Self { cv: key, chunk_counter, buf: [0; BLOCK_LEN], buf_len: 0, blocks_compressed: 0, flags }
    }
...
impl Hasher {
    ...
    pub fn finalize(&self) -> Hash {
        ...
        while i > 0 {
            i -= 1;
            output = parent_output(self.cv_stack[i], output.chaining_value(), self.key, self.flags);
        }
        output.root_hash()
    }
    ...
    fn add_chunk_cv(&mut self, mut new_cv: [u32; 8], mut total_chunks: u64) {
        while (total_chunks & 1) == 0 {
            self.cv_stack_len -= 1;
            new_cv = parent_cv(self.cv_stack[self.cv_stack_len], new_cv, self.key, self.flags);
            total_chunks >>= 1;
        }
...
pub const fn chunk_count_4096(len: usize) -> usize {
    if len == 0 { 1 } else { (len + CONTENT_CHUNK_LEN - 1) / CONTENT_CHUNK_LEN }
}
...
fn parent_output(left: [u32; 8], right: [u32; 8], key: [u32; 8], flags: u32) -> Output {
    ...
    Output { input_cv: key, block_words, counter: 0, block_len: BLOCK_LEN as u32, flags: flags | PARENT }
}
...
mod tests {
    ...
    #[test]
    fn known_empty() {
        assert_eq!(hex(hash(b"")), "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9ad c112b7cc9a93cae41f3262".replace(' ', ""));
    }
    ...
    fn one_shot_equals_streaming() {
        ...
        let mut hasher = Hasher::new();
        for part in data.chunks(17) { hasher.update(part); }
