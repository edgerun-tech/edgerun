...
impl<'a, 'font> UiPainter<'a, 'font> {
    ...
    pub fn with_icon_set(mut self, icon_set: UiIconSet) -> Self {
        self.icon_set = icon_set;
        self
    }

    pub fn set_icon_set(&mut self, icon_set: UiIconSet) {
        self.icon_set = icon_set;
    }

    pub fn with_asset_pack(mut self, pack: UiAssetPackSpec) -> Result<Self, UiAssetPackError> {
        pack.validate(UiAssetLimits::DEFAULT)?;
        self.icon_set = pack.icons.provider;
        Ok(self)
    }

    pub fn set_asset_pack(&mut self, pack: UiAssetPackSpec) -> Result<(), UiAssetPackError> {
        pack.validate(UiAssetLimits::DEFAULT)?;
        self.icon_set = pack.icons.provider;
        Ok(())
    }
    ...
    pub fn drag_source(
        ...
        self.scene
            .push_drag_source(GpuDragSource::new(scope_id, item_id, index, x, y, w, h));
    }

    pub fn drop_target(&mut self, scope_id: u32, index: usize, x: f32, y: f32, w: f32, h: f32) {
        self.scene
            .push_drop_target(GpuDropTarget::new(scope_id, index, x, y, w, h));
    }

    pub fn transition(&mut self, spec: UiTransitionSpec) {
        self.scene.push_transition(spec);
    }
    ...
    pub fn button(&mut self, rect: UiRect, label: &str, style: ButtonStyle, id: u32, active: bool) {
        ...
        let (fill, border, text) = match style {
            ...
            ButtonStyle::Secondary => (colors.row.with_alpha(0.82), colors.border, colors.text),
        ...
        if !matches!(style, ButtonStyle::Ghost) {
            self.scene.push_rect(GpuRect::border(
                rect.x,
                rect.y,
                rect.w,
                rect.h,
                self.theme.radius.control,
                border.with_alpha(if active { 0.32 } else { 0.18 }),
            ));
        }
    ...
    pub fn icon_button(&mut self, rect: UiRect, icon: UiIcon, id: u32, active: bool) {
        let hit = icon_hit_rect(rect);
        self.hit(HitKind::Button, id, hit.x, hit.y, hit.w, hit.h);
        let fill = if active {
            self.theme.colors.row.with_alpha(0.74)
        } else {
            self.theme.colors.row.with_alpha(0.22)
        };
        ...
        self.border_rect(
            ...
            self.theme
                .colors
                .border
                .with_alpha(if active { 0.24 } else { 0.10 }),
        );
        self.icon(
            icon_only_button_icon_rect(rect),
            icon,
            if active {
    ...
    pub fn icon(&mut self, rect: UiRect, icon: UiIcon, color: Color4) {
        match self.icon_set {
            UiIconSet::Tabler => {
                ...
                {
                    let atlas_rect = icon.tabler_svg_atlas_rect();
            ...
            UiIconSet::Lucide => {
                ...
                {
                    let atlas_rect = icon.lucide_svg_atlas_rect();
                    self.scene.push_icon_quad(rect, atlas_rect, color);
                    return;
                }
            }
    ...
    pub fn icon_tone(&mut self, rect: UiRect, icon: UiIcon, tone: UiIconTone) {
        self.icon(rect, icon, tone.color(self.theme));
    }
    ...
    pub fn select_trigger(&mut self, rect: UiRect, label: &str, value: &str, id: u32) {
        ...
        self.fill_rect(rect, radius, colors.row.with_alpha(0.42));
        self.border_rect(rect, radius, colors.border.with_alpha(0.58));
        self.bounded_label(
            rect.x + 14.0,
            rect.y + 8.0,
            (rect.w - 48.0).max(0.0),
    ...
    pub fn tooltip(&mut self, rect: UiRect, text: &str) {
        ...
        self.bounded_label(
            rect.x + spacing::TOOLTIP_PAD_X,
            rect.y + (rect.h - 14.0) * 0.5,
            (rect.w - spacing::TOOLTIP_PAD_X * 2.0).max(0.0),
            text,
            2.0,
            colors.text,
        );
    ...
    pub fn dialog(&mut self, rect: UiRect, title: &str, body: &str, icon: UiIcon) {
        ...
        self.icon(
            UiRect::new(
                rect.x + spacing::DIALOG_CONTENT_INSET_X,
                rect.y + spacing::DIALOG_CONTENT_INSET_X,
                34.0,
                34.0,
            ),
            icon,
        ...
        self.bounded_label(
            rect.x + spacing::DIALOG_CONTENT_INSET_X + 46.0,
            rect.y + spacing::DIALOG_CONTENT_INSET_X,
            rect.w - 84.0,
            title,
            2.0,
        ...
        self.divider(
            rect.x + spacing::DIALOG_CONTENT_INSET_X,
            rect.y + 74.0,
            rect.w - spacing::DIALOG_CONTENT_INSET_X * 2.0,
            Axis::Horizontal,
        );
    }
    ...
    pub fn toast(&mut self, rect: UiRect, message: &str, icon: UiIcon, accent: Color4) {
        ...
        self.icon(
            UiRect::new(
                rect.x + spacing::TOAST_PAD_X,
                rect.y + (rect.h - 22.0) * 0.5,
                22.0,
                22.0,
            ),
            icon,
        ...
        self.bounded_label(
            rect.x + spacing::TOAST_PAD_X + 32.0,
            rect.y + (rect.h - 14.0) * 0.5,
            (rect.w - spacing::TOAST_PAD_X * 2.0 - 32.0).max(0.0),
            message,
            2.0,
            self.theme.colors.text,
        );
    ...
    pub fn toggle(&mut self, x: f32, y: f32, on: bool, id: u32) {
        ...
        self.scene.push_rect(GpuRect::border(
            ...
            24.0,
            12.0,
            color.with_alpha(if on { 0.42 } else { 0.20 }),
        ));
        let knob_x = if on { x + 24.0 } else { x + 4.0 };
        self.scene
            .push_rect(GpuRect::fill(knob_x, y + 4.0, 16.0, 16.0, 8.0, colors.text));
    }
    ...
    pub fn list_row(&mut self, rect: UiRect, title: &str, detail: &str, accent: Color4, id: u32) {
        self.hit(HitKind::ListRow, id, rect.x, rect.y, rect.w, rect.h);
        let colors = self.theme.colors;
        self.fill_rect(rect, self.theme.radius.card, colors.row.with_alpha(0.42));
        self.scene
