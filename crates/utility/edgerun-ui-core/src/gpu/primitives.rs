...
#[derive(Clone, Debug)]
pub struct UiTrustProjectionRow {
    pub object: String,
    pub hash: String,
    pub state: String,
}

impl UiTrustProjectionRow {
    pub fn new(
...
pub struct UiWorkProjection {
    ...
    pub admission_rows: Vec<UiTrustProjectionRow>,
    pub cache_rows: Vec<UiTrustProjectionRow>,
    pub grant_rows: Vec<UiTrustProjectionRow>,
    pub session_rows: Vec<UiTrustProjectionRow>,
    pub audit_rows: Vec<UiTrustProjectionRow>,
}
    ...
    pub fn parse_key_values(input: &str) -> Option<Self> {
        ...
        for line in input.lines() {
            ...
            match key {
                ...
                "admission_row" => projection.admission_rows.push(parse_projection_row(value)?),
                "cache_row" => projection.cache_rows.push(parse_projection_row(value)?),
                "grant_row" => projection.grant_rows.push(parse_projection_row(value)?),
                "session_row" => projection.session_rows.push(parse_projection_row(value)?),
                "audit_row" => projection.audit_rows.push(parse_projection_row(value)?),
                _ => return None,
            }
        }
...
fn parse_projection_row(value: &str) -> Option<UiTrustProjectionRow> {
    let mut parts = value.split('|').map(str::trim);
    let object = parts.next()?;
    let hash = parts.next()?;
    let state = parts.next()?;
    if object.is_empty() || hash.is_empty() || state.is_empty() || parts.next().is_some() {
        return None;
    }
    Some(UiTrustProjectionRow::new(object, hash, state))
}
