use super::{Color4, UiRect, UiResolvedTheme, palette};
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct UiStyleGradient {
    pub from: UiStyleColor,
    pub to: UiStyleColor,
}

impl UiStyleGradient {
    pub const fn new(from: UiStyleColor, to: UiStyleColor) -> Self {
        Self { from, to }
    }
}
...
pub struct UiStyle {
    ...
    pub gradient: Option<UiStyleGradient>,
...
impl UiStyle {
    ...
    fn apply_color_class(&mut self, class: &str) -> bool {
        ...
        if let Some(gradient) = semantic_gradient(class) {
    ...
    fn apply_shadow_class(&mut self, class: &str) -> bool {
        self.shadow = match class {
            "shadow-sm" => 8.0,
            "shadow" | "shadow-md" => 14.0,
            "shadow-lg" => 24.0,
            _ => return false,
        };
        true
...
fn semantic_bg_color(class: &str) -> Option<UiStyleColor> {
    match class {
        "bg-black" => Some(UiStyleColor::fixed(palette::BLACK)),
        "bg-slate-50" => Some(UiStyleColor::fixed(palette::SLATE_50)),
...
fn semantic_gradient(class: &str) -> Option<UiStyleGradient> {
    match class {
        "bg-panel-gradient" | "bg-card-gradient" => Some(UiStyleGradient::new(
            UiStyleColor::semantic(UiColorToken::Panel),
            UiStyleColor::semantic(UiColorToken::Row),
        )),
        "bg-muted-gradient" => Some(UiStyleGradient::new(
            UiStyleColor::semantic(UiColorToken::Row),
            UiStyleColor::semantic(UiColorToken::Panel),
        )),
        "bg-accent-gradient" => Some(UiStyleGradient::new(
            UiStyleColor::semantic(UiColorToken::Accent),
            UiStyleColor::semantic(UiColorToken::Info),
        )),
        _ => None,
    }
}
...
fn tailwind_class_color(value: &str) -> Option<Color4> {
    let color = match value {
        "slate-50" => palette::SLATE_50,
        "slate-400" => palette::SLATE_400,
        "slate-700" => palette::SLATE_700,
        "slate-800" => palette::SLATE_800,
        "slate-900" => palette::SLATE_900,
        "slate-950" => palette::SLATE_950,
        "sky-50" => palette::SKY_50,
        "cyan-600" => palette::CYAN_600,
        "emerald-500" => palette::EMERALD_500,
        "amber-500" => palette::AMBER_500,
        "violet-500" => palette::VIOLET_500,
        "rose-600" => palette::ROSE_600,
        "black" => palette::BLACK,
        _ => return None,
    };
    Some(color)
...
mod tests {
    ...
    fn parses_grid_span_class() {
        ...
        let style = UiStyle::parse("bento grid-cols-4 col-span-2 row-span-3");
        assert_eq!(style.row_span, 3);
    }
    ...
    fn resolves_semantic_and_tailwind_color_classes_from_shared_palette() {
        ...
        assert_eq!(tailwind.bg, Some(UiStyleColor::fixed(palette::SLATE_900)));
        assert_eq!(tailwind.text, UiStyleColor::fixed(palette::CYAN_600));
    }
