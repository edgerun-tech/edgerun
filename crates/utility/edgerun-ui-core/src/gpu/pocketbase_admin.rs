...
fn render_pocketbase_admin(
    ...
    ui.bounded_label(43.0, 10.0, 164.0, "EdgeRun PocketBase", 2.0, text_color);
    header_link(ui, 225.0, "Collections", true);
    header_link(ui, 326.0, "Logs", false);
    header_link(ui, 379.0, "Settings", false);
    header_link(ui, 454.0, "Backups", false);
    header_link(ui, 532.0, "Crons", false);
    ...
    ui.bounded_label(user_x + 32.0, 10.0, 104.0, "superuser", 2.0, text_color);
    ...
    let active = state.collections.first();
...
fn header_link(ui: &mut UiPainter<'_, '_>, x: f32, label: &str, active: bool) {
    ...
    if active {
        ui.fill_rect(
            UiRect::new(x - 6.0, 6.0, 88.0, 26.0),
            5.0,
            Color4::rgba_u8(255, 255, 255, 28),
        );
    }
    ui.bounded_label(x, 10.0, 92.0, label, 2.0, text_color);
}
...
mod tests {
    ...
    fn pocketbase_admin_scene_is_dark_and_interactive() {
        ...
        let atlas = FontAtlas::load_geist(24.0).expect("font atlas");
