...
impl SdlDirectDrawCapability {
    ...
    ///
    /// App code must use `instantiate_sdl_app`. Direct scene access is intended
    /// only for trusted framework surfaces, diagnostics, and render backend
    /// tests.
...
/// Canonical SDL entry point for app surfaces.
///
/// This function owns SDL input capture, forwards events through the shared UI
/// runtime, rebuilds the app `UiNode` surface, rasterizes text/icons, and
/// renders through the OpenGL backend. App crates should not wire SDL events or
/// drag/drop/scroll/focus behavior themselves.
pub fn instantiate_sdl_app<A: UiSurfaceApp + 'static>(
    options: SdlGlWindowOptions<'_>,
    app: A,
) -> Result<(), String> {
    let app = Rc::new(RefCell::new(app));
    let event_app = Rc::clone(&app);
    let layout_app = Rc::clone(&app);
    run_sdl_gl_layout_window(
        options,
        move |event| match event {
            SdlInputEvent::CloseRequested => SdlEventResult::quit(),
            SdlInputEvent::KeyDown { key, .. } if key == SDL_KEY_ESCAPE => SdlEventResult::quit(),
            SdlInputEvent::KeyDown { key, .. } => {
                let control = event_app.borrow_mut().handle_event(UiEvent::KeyDown {
                    key: map_sdl_key(key),
                });
                SdlEventResult {
                    dirty: control.dirty,
            ...
            SdlInputEvent::TextInput { text } => {
                let control = event_app
                    .borrow_mut()
                    .handle_event(UiEvent::TextInput(text));
                SdlEventResult {
                    dirty: control.dirty,
                    quit: control.quit,
                }
            ...
            SdlInputEvent::MouseWheel { y } => {
                let control = event_app.borrow_mut().handle_event(UiEvent::Wheel {
                    x: 0.0,
                    y: 0.0,
                    delta_y: y,
                });
            ...
            SdlInputEvent::UiAction { action } => {
                let control = event_app.borrow_mut().handle_action(action);
                SdlEventResult {
                    dirty: control.dirty,
                    quit: control.quit,
                }
            ...
            SdlInputEvent::Tick => {
                let control = event_app.borrow_mut().tick(16);
                SdlEventResult {
                    dirty: control.dirty,
                    quit: control.quit,
                }
        ...
        move |width, height| {
            layout_app.borrow_mut().surface(UiRect::new(
                0.0,
                0.0,
                width.max(0) as f32,
                height.max(0) as f32,
...
fn cursor_for_hit_kind(hit: super::runtime::GpuHit) -> SdlCursorKind {
    match hit.kind {
        ...
        HitKind::ScrollArea => SdlCursorKind::Arrow,
        HitKind::Button
        | HitKind::Tab
        | HitKind::Toggle
        | HitKind::ListRow
...
/// Framework-only SDL loop with direct scene rendering.
///
/// This bypasses `UiNode` surface construction and exists for renderer
/// diagnostics, backend tests, and trusted framework surfaces. Product app
/// crates should use `instantiate_sdl_app`.
...
fn run_sdl_gl_window_backend(
    ...
    let atlas = FontAtlas::load_geist(32.0)?;
    ...
    while running {
        ...
        let frame = runtime.handle_frame(&scene, UiFrameInput::new(16));
        if frame.needs_redraw || frame.transitions_active {
            dirty = true;
        }
