...
use crate::{tabler_generated::TABLER_ICONS, Color, Painter, Rect};
...
impl<'a> Painter<'a> {
    pub fn tabler_icon(&mut self, name: TablerIconName, rect: Rect, color: Color) {
        ...
        let stroke = (scale * 2.0).round().max(1.0) as i32;
...
fn draw_svg_path(p: &mut Painter<'_>, path: &str, ox: f32, oy: f32, scale: f32, stroke: i32, color: Color) {
    ...
    while i < bytes.len() {
        ...
        match cmd {
            b'M' | b'm' => {
                let Some(x) = read_num(bytes, &mut i) else { break; };
                let Some(y) = read_num(bytes, &mut i) else { break; };
                let pt = if cmd == b'm' { Pt { x: cur.x + x, y: cur.y + y } } else { Pt { x, y } };
                cur = pt;
                start = pt;
            ...
            b'L' | b'l' => {
                let Some(x) = read_num(bytes, &mut i) else { break; };
                let Some(y) = read_num(bytes, &mut i) else { break; };
                let next = if cmd == b'l' { Pt { x: cur.x + x, y: cur.y + y } } else { Pt { x, y } };
                stroke_line(p, cur, next, ox, oy, scale, stroke, color);
                cur = next;
            }
            b'H' | b'h' => {
                let Some(x) = read_num(bytes, &mut i) else { break; };
                let next = if cmd == b'h' { Pt { x: cur.x + x, y: cur.y } } else { Pt { x, y: cur.y } };
                stroke_line(p, cur, next, ox, oy, scale, stroke, color);
                cur = next;
            }
            b'V' | b'v' => {
                let Some(y) = read_num(bytes, &mut i) else { break; };
                let next = if cmd == b'v' { Pt { x: cur.x, y: cur.y + y } } else { Pt { x: cur.x, y } };
                stroke_line(p, cur, next, ox, oy, scale, stroke, color);
                cur = next;
            }
            b'C' | b'c' => {
                let Some(x1) = read_num(bytes, &mut i) else { break; };
                let Some(y1) = read_num(bytes, &mut i) else { break; };
                let Some(x2) = read_num(bytes, &mut i) else { break; };
                let Some(y2) = read_num(bytes, &mut i) else { break; };
                let Some(x3) = read_num(bytes, &mut i) else { break; };
                let Some(y3) = read_num(bytes, &mut i) else { break; };
            ...
            b'A' | b'a' => {
                for _ in 0..5 { let _ = read_num(bytes, &mut i); }
                let Some(x) = read_num(bytes, &mut i) else { break; };
                let Some(y) = read_num(bytes, &mut i) else { break; };
                let next = rel(cmd, cur, x, y);
                stroke_line(p, cur, next, ox, oy, scale, stroke, color);
...
fn rel(cmd: u8, cur: Pt, x: f32, y: f32) -> Pt {
    if cmd.is_ascii_lowercase() { Pt { x: cur.x + x, y: cur.y + y } } else { Pt { x, y } }
}

fn stroke_cubic(p: &mut Painter<'_>, a: Pt, b: Pt, c: Pt, d: Pt, ox: f32, oy: f32, scale: f32, stroke: i32, color: Color) {
    let mut prev = a;
    for step in 1..=10 {
        let t = step as f32 / 10.0;
        let nt = 1.0 - t;
        let next = Pt {
            x: nt * nt * nt * a.x + 3.0 * nt * nt * t * b.x + 3.0 * nt * t * t * c.x + t * t * t * d.x,
            y: nt * nt * nt * a.y + 3.0 * nt * nt * t * b.y + 3.0 * nt * t * t * c.y + t * t * t * d.y,
        };
...
fn stroke_quad(p: &mut Painter<'_>, a: Pt, b: Pt, c: Pt, ox: f32, oy: f32, scale: f32, stroke: i32, color: Color) {
    let mut prev = a;
    for step in 1..=8 {
...
fn stroke_line(p: &mut Painter<'_>, a: Pt, b: Pt, ox: f32, oy: f32, scale: f32, stroke: i32, color: Color) {
    let x0 = (ox + a.x * scale).round() as i32;
    let y0 = (oy + a.y * scale).round() as i32;
    let x1 = (ox + b.x * scale).round() as i32;
    let y1 = (oy + b.y * scale).round() as i32;
    let dx = x1 - x0;
    let dy = y1 - y0;
    ...
    for i in 0..=steps {
        let x = x0 + dx * i / steps;
        let y = y0 + dy * i / steps;
        p.rounded_rect(Rect::new(x - r / 2, y - r / 2, r as u32, r as u32), r as u32, color);
    }
}

fn is_cmd(b: u8) -> bool {
    matches!(b, b'M' | b'm' | b'L' | b'l' | b'H' | b'h' | b'V' | b'v' | b'C' | b'c' | b'S' | b's' | b'Q' | b'q' | b'A' | b'a' | b'Z' | b'z')
}
