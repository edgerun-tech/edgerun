...
//!
//! ```
//! use edgerun_http_client::http::middleware::{Chain, Middleware, Next};
//! use edgerun_http_client::http::{into_handler, Handler, Request, Response, StatusCode};
...
//! let cors = edgerun_http_client::http::middleware::middleware_fn(|req, next| {
//!     Box::pin(async move {
...
///
/// ```
/// use edgerun_http_client::http::Request;
///
...
///
/// ```text
/// let handler = edgerun_http_client::http::middleware::Chain::new(my_handler)
///     .with(cors())      // outermost
///     .with(logging())
