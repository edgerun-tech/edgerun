use super::*;
use crate::http::runtime::net::UdpSocket;
...
fn test_initial_packet_handshake_timeout() {
    use crate::http::http3::quic::frame::QuicFrame;
    use crate::http::http3::quic::packet;
    ...
    rt.block_on(async {
        ...
        let result = crate::http::runtime::timeout(
...
fn test_http3_request_response_roundtrip() {
    ...
    use crate::http::http3::connection::Http3Connection;
    use crate::http::http3::{QpackDecoder, QpackEncoder};
...
fn test_recv_path_expands_truncated_packet_number_before_ack_tracking() {
    ...
    let packet = QuicPacket {
        header: crate::http::http3::quic::packet::QuicPacketHeader {
...
fn test_server_sends_1rtt_response() {
    ...
    assert_eq!(
        ...
        crate::http::http3::quic::packet::PacketType::OneRtt
    );
}
...
fn test_set_active_path() {
    let mut conn = QuicConnection::dummy();
    let local: crate::http::runtime::net::SocketAddr = "127.0.0.1:12345".parse().unwrap();
    let remote: crate::http::runtime::net::SocketAddr = "192.168.1.1:443".parse().unwrap();
...
fn test_integration_full_http3_flow() {
    ...
    let status = crate::http::StatusCode::new(200).unwrap();
    let resp_headers = crate::http::HeaderMap::new();
...
fn test_integration_push_flow() {
    ...
    let mut encoder = crate::http::http3::QpackEncoder::new();
    let (header_block, _) = encoder.encode(&[(":status", "200")]).unwrap();
    let headers_frame = Http3Frame::Headers { header_block };
...
fn test_integration_connection_migration() {
    ...
    let local: crate::http::runtime::net::SocketAddr = "10.0.0.1:50000".parse().unwrap();
    let remote: crate::http::runtime::net::SocketAddr = "10.0.0.2:443".parse().unwrap();
