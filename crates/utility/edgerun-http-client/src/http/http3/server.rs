...
//! # Example
//! ```no_run
//! use edgerun_http_client::http::http3::Http3Server;
...
struct AddressValidationState {
    ...
    last_activity: crate::http::runtime::time::Instant,
}
...
impl AddressValidationState {
    fn new() -> Self {
        AddressValidationState {
            ...
            last_activity: crate::http::runtime::time::Instant::now(),
        }
    }
    ...
    fn record_received(&mut self, bytes: u64) {
        ...
        self.last_activity = crate::http::runtime::time::Instant::now();
    }
...
impl Http3Server {
    ...
    pub async fn bind<A: crate::http::runtime::net::ToSocketAddrs>(
        addr: A,
        cert_and_key: CertificateAndKey,
    ) -> crate::http::runtime::io::Result<Self> {
    ...
    pub fn local_addr(&self) -> crate::http::runtime::io::Result<SocketAddr> {
        self.socket
            .local_addr()
    ...
    /// ```no_run
    /// use edgerun_http_client::http::http3::Http3Server;
    /// use edgerun_http_client::http::{Handler, Request, Response, StatusCode, into_handler};
    /// use crate::tls::certificate_gen::generate_self_signed;
    /// use crate::rt::Runtime;
    /// use edgerun_http_client::http::runtime::sync::Arc;
    ///
    /// let rt = Runtime::new_multi_thread().enable_all().build().unwrap();
    /// rt.block_on(async {
    ///     let cert = generate_self_signed(&["localhost"]).unwrap();
    ...
    pub async fn serve(
        ...
        loop {
            ...
            match runtime::timeout(
                crate::http::runtime::time::Duration::from_millis(100),
                self.accept(),
            )
            .await
            {
