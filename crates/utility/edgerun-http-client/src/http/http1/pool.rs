...
use crate::http::http1::compression;
use crate::http::{Error, HeaderMap, Method, Request, Response, Result, StatusCode};
...
impl PooledConn {
    ...
    async fn read_line(&mut self) -> crate::http::runtime::io::Result<Option<String>> {
        match self {
            PooledConn::Plain(r) => r.read_line().await.map_err(crate::http::runtime::bare_io),
            #[cfg(feature = "tls")]
            PooledConn::Tls(r) => r.read_line().await.map_err(crate::http::runtime::bare_io),
        }
    }

    /// Read bytes from the connection.
    async fn read(&mut self, buf: &mut [u8]) -> crate::http::runtime::io::Result<usize> {
        match self {
            PooledConn::Plain(r) => r.read(buf).await.map_err(crate::http::runtime::bare_io),
            #[cfg(feature = "tls")]
            PooledConn::Tls(r) => r.read(buf).await.map_err(crate::http::runtime::bare_io),
        }
    }

    /// Write the request and flush.
    async fn write_request(&mut self, data: &[u8]) -> crate::http::runtime::io::Result<()> {
        match self {
            PooledConn::Plain(r) => {
                r.get_mut()
                    .write_all(data)
...
impl ConnectionPool {
    ...
    async fn try_pooled_requests(
        ...
        (
            Err(Error::Network(crate::http::runtime::io::Error::new(
        ...
        if is_https {
            #[cfg(not(feature = "tls"))]
            {
                ...
                return Err(Error::ProtocolError(
                    "HTTPS requires edgerun-http-client tls feature".into(),
                ));
            }
            #[cfg(feature = "tls")]
            {
    ...
    async fn connect_sock_static(
        ...
        match rt_timeout(connect_timeout, fut).await {
            ...
            Ok(Err(e)) => Err(Error::Network(crate::http::runtime::io::Error::other(e))),
    ...
    async fn read_response(
        ...
        // Read body
        let is_chunked = crate::http::chunked::has_chunked_transfer_coding(&headers);
