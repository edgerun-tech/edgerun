...
impl From<crate::http::runtime::io::Error> for Http2Error {
    fn from(err: crate::http::runtime::io::Error) -> Self {
        Http2Error::Io(err)
    }
}

impl From<Http2Error> for crate::http::runtime::io::Error {
    fn from(err: Http2Error) -> Self {
        match err {
            Http2Error::Io(err) => err,
            other => crate::http::runtime::io::Error::other(other.to_string()),
        }
    }
}
