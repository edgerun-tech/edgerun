...
//!
//! Mirrors the server-side [`crate::http::middleware`] API but wraps
//! [`crate::http::client::HttpClient::execute`] instead of [`crate::http::Handler`].
//!
//! # Example
//!
//! ```text
//! use edgerun_http_client::http::client_middleware::{Chain, ClientMiddleware, ClientNext};
//! use edgerun_http_client::http::{HttpClient, Request, Response, Result};
...
//!                         crate::rt::sleep(Duration::from_millis(100 * 2u64.pow(attempt as u32))).await;
//!                         continue;
//!                     }
//!                     Err(e) => {
//!                         last_err = Some(e);
...
//!                 }
//!             }
//!             Err(last_err.unwrap_or_else(|| crate::http::Error::ProtocolError("max retries exhausted".into())))
//!         })
//!     }
//! }
//!
//! // Function-based: add auth header
//! let auth = edgerun_http_client::http::client_middleware::client_middleware_fn(|req, next| {
//!     Box::pin(async move {
//!         let mut req = req;
...
use crate::http::{Error, Request, Response, Result};
use alloc::boxed::Box;
...
///
/// Unlike server [`crate::http::middleware::Extensions`], these live on the
...
impl ClientRequest {
    ...
    pub fn from_builder(builder: crate::http::request::RequestBuilder) -> Result<Self> {
    ...
    pub fn uri(&self) -> &crate::http::uri::Uri {
        self.request.uri()
    }
...
///
/// ```text
/// let client = edgerun_http_client::http::client_middleware::Chain::new(edgerun_http_client::http::HttpClient::new())
///     .with(logging())
///     .with(retry(3))
///     .build();
/// ```
