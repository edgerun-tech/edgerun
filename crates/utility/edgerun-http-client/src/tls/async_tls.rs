...
//! `edgerun_http_client::rt::AsyncRead + edgerun_http_client::rt::AsyncWrite + Unpin` transport.
//!
...
//! # Example (client)
//! ```rust
//! use edgerun_http_client::tls::{AsyncRead, AsyncTlsStream, AsyncWrite};
//!
//! // AsyncTlsStream works with any async read/write stream
//! async fn tls_client_example<S>(stream: S) -> edgerun_http_client::tls::Result<AsyncTlsStream<S>>
//! where
...
impl<S: AsyncRead + AsyncWrite + Unpin> AsyncTlsStream<S> {
    ...
    async fn client_inner_with_cookie(
        ...
        let record = crate::tls::record::TlsRecord {
            content_type: 22,
            version: 0x0301,
            fragment: ch,
        };
    ...
    pub fn poll_flush(&mut self, cx: &mut Context<'_>) -> Poll<crate::rt::io::Result<()>> {
        Pin::new(&mut self.stream).poll_flush(cx)
    }

    /// Shut down the underlying transport.
    pub fn poll_shutdown(&mut self, cx: &mut Context<'_>) -> Poll<crate::rt::io::Result<()>> {
...
impl<S: AsyncRead + AsyncWrite + Unpin> AsyncWrite for AsyncTlsStream<S> {
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<crate::rt::io::Result<()>> {
...
impl<S: AsyncRead + AsyncWrite + Unpin> AsyncTlsServerStream<S> {
    ...
    /// # Example
    /// ```rust
    /// use edgerun_http_client::tls::{AsyncRead, AsyncTlsServerStream, AsyncWrite};
    /// use edgerun_http_client::tls::certificate_gen::CertificateAndKey;
    ///
    /// async fn starttls<S>(tcp_stream: S, cert: CertificateAndKey) -> edgerun_http_client::tls::Result<()>
    /// where
    ///     S: AsyncRead + AsyncWrite + Unpin,
    /// {
...
async fn server_handshake_impl<S: AsyncRead + AsyncWrite + Unpin>(
    ...
    let (selected_group, client_key_share) = if let Some((group, key)) = ch.all_key_shares.first() {
        let keg = match group {
            crate::tls::cipher::NamedGroup::X25519 => KeyExchangeGroup::X25519,
            _ => KeyExchangeGroup::SECP256R1,
        };
        (keg, key.clone())
    } else if let Some(ks) = ch.client_key_share {
        let keg = match ch.client_key_share_group {
            Some(crate::tls::cipher::NamedGroup::X25519) => KeyExchangeGroup::X25519,
            _ => KeyExchangeGroup::SECP256R1,
        };
        (keg, ks)
    } else {
...
fn parse_hrr_selected_group(data: &[u8]) -> Result<crate::tls::key_exchange::KeyExchangeGroup> {
    use crate::tls::key_exchange::KeyExchangeGroup;
