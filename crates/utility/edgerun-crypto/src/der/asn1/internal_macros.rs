macro_rules! impl_any_conversions {
    ...
    ($type: ty, $($li: lifetime)?) => {
        impl<'__der: $($li),*, $($li),*> TryFrom<$crate::der::AnyRef<'__der>> for $type {
            type Error = $crate::der::Error;

            fn try_from(any: $crate::der::AnyRef<'__der>) -> Result<$type> {
                any.decode_as()
            }
        }

        #[cfg(feature = "alloc")]
        impl<'__der: $($li),*, $($li),*> TryFrom<&'__der $crate::der::Any> for $type {
            type Error = $crate::der::Error;

            fn try_from(any: &'__der $crate::der::Any) -> Result<$type> {
                any.decode_as()
            }
        }
    };
