...
#[cfg(all(feature = "elliptic_curve_pkcs8", feature = "elliptic_curve_sec1"))]
mod pkcs8;
    ...
    C: Curve,
{
    ...
    #[cfg(feature = "elliptic_curve_arithmetic")]
    pub fn random(rng: &mut impl crate::rand_core::CryptoRngCore) -> Self
    where
        C: CurveArithmetic,
    {
        Self {
    ...
    pub fn to_sec1_der(&self) -> crate::der::Result<Zeroizing<Vec<u8>>>
    where
        C: CurveArithmetic,
    ...
    FieldBytesSize<C>: ModulusSize,
{
    ...
    fn try_from(sec1_private_key: sec1::EcPrivateKey<'_>) -> crate::der::Result<Self> {
        let secret_key = Self::from_slice(sec1_private_key.private_key)
            .map_err(|_| crate::der::Tag::Sequence.value_error())?;
        ...
        if let Some(pk_bytes) = sec1_private_key.public_key {
            let pk = EncodedPoint::<C>::from_bytes(pk_bytes)
                .map_err(|_| crate::der::Tag::BitString.value_error())?;

            if C::validate_public_key(&secret_key, &pk).is_err() {
                return Err(crate::der::Tag::BitString.value_error());
            }
        }
