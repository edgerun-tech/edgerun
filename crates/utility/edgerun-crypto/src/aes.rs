//! AES encryption (software implementation).
...
impl Aes128 {
    ...
    pub fn encrypt_block(&self, block: &[u8; 16]) -> [u8; 16] {
        ...
        add_round_key(&mut state, make_key_array(&self.round_keys[0..16]));
        for round in 1..10 {
            sub_bytes(&mut state);
        ...
        sub_bytes(&mut state);
        shift_rows(&mut state);
        add_round_key(&mut state, make_key_array(&self.round_keys[160..176]));
...
#[inline]
fn make_key_array(slice: &[u8]) -> [u8; 16] {
    let mut arr = [0u8; 16];
...
impl Aes256 {
    ...
    pub fn encrypt_block(&self, block: &[u8; 16]) -> [u8; 16] {
        ...
        sub_bytes(&mut state);
        shift_rows(&mut state);
        add_round_key(&mut state, make_key_array(&self.round_keys[224..240]));
...
fn expand_key(round_keys: &mut [u8], nr: usize) {
    ...
    for i in nk..4 * (nr + 1) {
        ...
        let k = i * 4;
        for idx in 0..4 {
            round_keys[k + idx] = round_keys[j + idx] ^ temp[idx];
        }
    }
}
