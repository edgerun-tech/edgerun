...
//
// ```
// # use crate::num_bigint::{smallvec, SmallVec};
// # fn main() {
...
macro_rules! smallvec {
    ...
    (@one $x:expr) => (1usize);
    () => (
        $crate::num_bigint::SmallVec::new()
    );
    ($elem:expr; $n:expr) => ({
        $crate::num_bigint::SmallVec::from_elem($elem, $n)
    });
    ($($x:expr),+$(,)?) => ({
        let count = 0usize $(+ $crate::smallvec!(@one $x))+;
        let mut vec = $crate::num_bigint::SmallVec::new();
        if count <= vec.inline_size() {
            $(vec.push($x);)*
            vec
        } else {
            $crate::num_bigint::SmallVec::from_vec($crate::alloc::vec![$($x,)+])
        }
    });
}
...
//
// ```
// # use crate::num_bigint::{smallvec_inline, SmallVec};
// # fn main() {
...
macro_rules! smallvec_inline {
    ...
    (@one $x:expr) => (1usize);
    ($elem:expr; $n:expr) => ({
        $crate::num_bigint::SmallVec::<[_; $n]>::from_const([$elem; $n])
    });
    ($($x:expr),+ $(,)?) => ({
        const N: usize = 0usize $(+ $crate::smallvec_inline!(@one $x))*;
        $crate::num_bigint::SmallVec::<[_; N]>::from_const([$($x,)*])
    });
}
...
//
// ```rust
// use crate::num_bigint::{ExtendFromSlice, SmallVec};
//
