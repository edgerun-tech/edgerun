...
macro_rules! hf16 {
    ($s:literal) => {{
        const X: f16 = $crate::num_bigint::libm::support::hf16($s);
        X
    }};
}
...
macro_rules! hf32 {
    ($s:literal) => {{
        const X: f32 = $crate::num_bigint::libm::support::hf32($s);
        X
    }};
}
...
macro_rules! hf64 {
    ($s:literal) => {{
        const X: f64 = $crate::num_bigint::libm::support::hf64($s);
        X
    }};
}
...
macro_rules! hf128 {
    ($s:literal) => {{
        const X: f128 = $crate::num_bigint::libm::support::hf128($s);
        X
    }};
}
...
macro_rules! assert_biteq {
    ($left:expr, $right:expr, $($tt:tt)*) => {{
        ...
        let bits = $crate::num_bigint::libm::support::Int::leading_zeros(l.to_bits() - l.to_bits());
        assert!(
            $crate::num_bigint::libm::support::Float::biteq(l, r),
            "{}\nl: {l:?} ({lb:#0width$x} {lh})\nr: {r:?} ({rb:#0width$x} {rh})",
            format_args!($($tt)*),
            lb = l.to_bits(),
            lh = $crate::num_bigint::libm::support::Hexf(l),
            rb = r.to_bits(),
            rh = $crate::num_bigint::libm::support::Hexf(r),
