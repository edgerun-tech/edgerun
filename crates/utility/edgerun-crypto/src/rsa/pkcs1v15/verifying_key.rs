...
where
    D: Digest,
{
    fn verify_digest(&self, digest: D, signature: &Signature) -> crate::signature::Result<()> {
        verify(
            &self.inner,
            &self.prefix,
            &digest.finalize(),
...
where
    D: Digest,
{
    fn verify(&self, msg: &[u8], signature: &Signature) -> crate::signature::Result<()> {
        verify(
            &self.inner,
            &self.prefix.clone(),
            &D::digest(msg),
...
impl<D> TryFrom<crate::pkcs8::SubjectPublicKeyInfoRef<'_>> for VerifyingKey<D>
where
    D: Digest + AssociatedOid,
{
