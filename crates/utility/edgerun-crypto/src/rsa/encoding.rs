...
fn verify_algorithm_id(algorithm: &crate::pkcs8::AlgorithmIdentifierRef) -> crate::pkcs8::spki::Result<()> {
    ...
    if algorithm.parameters_any()? != crate::pkcs8::der::asn1::Null.into() {
        return Err(crate::pkcs8::spki::Error::KeyMalformed);
    }

    Ok(())
}

impl TryFrom<crate::pkcs8::PrivateKeyInfo<'_>> for RsaPrivateKey {
    ...
    fn try_from(spki: crate::pkcs8::SubjectPublicKeyInfoRef<'_>) -> crate::pkcs8::spki::Result<Self> {
        ...
        let pkcs1_key = pkcs1::RsaPublicKey::try_from(
            spki.subject_public_key
                .as_bytes()
                .ok_or(crate::pkcs8::spki::Error::KeyMalformed)?,
        )?;
        let n = BigUint::from_bytes_be(pkcs1_key.modulus.as_bytes());
        let e = BigUint::from_bytes_be(pkcs1_key.public_exponent.as_bytes());
        RsaPublicKey::new(n, e).map_err(|_| crate::pkcs8::spki::Error::KeyMalformed)
    }
}
...
impl EncodePrivateKey for RsaPrivateKey {
    fn to_pkcs8_der(&self) -> crate::pkcs8::Result<SecretDocument> {
        ...
        crate::pkcs8::PrivateKeyInfo::new(pkcs1::ALGORITHM_ID, private_key.as_ref()).try_into()
    }
}

impl EncodePublicKey for RsaPublicKey {
    fn to_public_key_der(&self) -> crate::pkcs8::spki::Result<Document> {
        let modulus = self.n().to_bytes_be();
        let public_exponent = self.e().to_bytes_be();
        ...
        crate::pkcs8::SubjectPublicKeyInfoRef {
            algorithm: pkcs1::ALGORITHM_ID,
            subject_public_key: crate::pkcs8::der::asn1::BitStringRef::new(
                0,
