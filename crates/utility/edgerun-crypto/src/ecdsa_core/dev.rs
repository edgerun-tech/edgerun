...
macro_rules! new_wycheproof_test {
    ($name:ident, $test_name: expr, $curve:path) => {
        use $crate::ecdsa_core::{
            crate::elliptic_curve::{bigint::Integer, sec1::EncodedPoint},
            crate::signature::Verifier,
            Signature,
        };

        #[test]
        fn $name() {
            use crate::elliptic_curve::{bigint::Encoding as _, generic_array::typenum::Unsigned};
            ...
            fn element_from_padded_slice<C: crate::elliptic_curve::Curve>(
                data: &[u8],
            ) -> crate::elliptic_curve::FieldBytes<C> {
                ...
                if data.len() >= point_len {
                    ...
                    crate::elliptic_curve::FieldBytes::<C>::clone_from_slice(&data[offset..])
                } else {
                    ...
                    crate::elliptic_curve::FieldBytes::<C>::from_exact_iter(iter).unwrap()
                }
            }
            ...
            fn run_test(
                ...
                let verifying_key =
                    $crate::ecdsa_core::VerifyingKey::<$curve>::from_encoded_point(&q_encoded)
                        .unwrap();
