...
use {
    crate::ecdsa_core::VerifyingKey, crate::elliptic_curve::PublicKey, crate::signature::KeypairRef,
};

/// ECDSA secret key used for signing. Generic over prime order elliptic curves
/// (e.g. NIST P-curves)
///
/// Requires an [`crate::elliptic_curve::CurveArithmetic`] impl on the curve, and a
    ...
    SignatureSize<C>: ArrayLength<u8>,
{
    ...
    pub fn random(rng: &mut impl CryptoRngCore) -> Self {
        let mut rng = EllipticCurveRng(rng);
        NonZeroScalar::<C>::random(&mut rng).into()
    }
...
struct EllipticCurveRng<'a, R: ?Sized>(&'a mut R);

impl<R: crate::rand_core::RngCore + ?Sized> crate::elliptic_curve::rand_core::RngCore
    for EllipticCurveRng<'_, R>
{
    ...
    fn try_fill_bytes(
        &mut self,
        dest: &mut [u8],
    ) -> core::result::Result<(), crate::elliptic_curve::rand_core::Error> {
        self.0.try_fill_bytes(dest).map_err(|err| {
            let fallback =
                NonZeroU32::new(crate::elliptic_curve::rand_core::Error::CUSTOM_START).unwrap();
            crate::elliptic_curve::rand_core::Error::from(err.code().unwrap_or(fallback))
        })
    }
}

impl<R: crate::rand_core::CryptoRng + ?Sized> crate::elliptic_curve::rand_core::CryptoRng
    for EllipticCurveRng<'_, R>
{
}
...
where
    C: PrimeCurve + CurveArithmetic + DigestPrimitive,
    D: DigestAssociatedOid + Digest + FixedOutput<OutputSize = FieldBytesSize<C>>,
    Scalar<C>: Invert<Output = CtOption<Scalar<C>>> + SignPrimitive<C>,
    SignatureSize<C>: ArrayLength<u8>,
{
...
#[cfg(feature = "p256_ecdsa_pkcs8")]
impl<C> TryFrom<crate::pkcs8::PrivateKeyInfo<'_>> for SigningKey<C>
where
