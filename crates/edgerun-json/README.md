...
`edgerun-json` is aimed at workloads where generic JSON trees leave performance on the table:
...
# Ok::<(), edgerun_json::JsonParseError>(())
```

## Example: compiled row serialization

```rust
use edgerun_json::{CompiledRowSchema, JsonValue};
