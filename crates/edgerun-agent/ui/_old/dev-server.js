 ...
 * The frontend connects directly to the Rust backend's WS on port 8080.
 *
 * Usage:
...
analyzePath ??= process.env.CODEANALYZE_PATH ?? "./samples";

console.log(`🚀 Dev server on http://localhost:${port}`);
console.log(`   Backend: managed (port 8080)`);
...
function startRust(pathToAnalyze) {
  ...
  rustProc = Bun.spawn(
    ["cargo", "run", "--bin", "codeanalyzer", "--", pathToAnalyze, "--port", "8080"],
    {
      cwd: import.meta.dir.split("/viewer/src")[0],
      stdout: "pipe",
      stderr: "pipe",
...
async function waitForRust() {
  for (let i = 0; i < 60; i++) {
    await sleep(500);
    try {
      const resp = await fetch("http://localhost:8080/", { method: "HEAD" });
