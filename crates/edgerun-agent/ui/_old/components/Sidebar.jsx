...
import { Search, Code2, MessageSquare, FileText, GitBranch, RotateCcw, Maximize2, FolderOpen, ScanSearch, Bug, Tags } from "lucide-solid";
...
export function Sidebar() {
  ...
  function getNeighbors(nodeId) {
    ...
    return Array.from(result).slice(0, neighborLimit());
  }
      ...
      <div class="flex-1 overflow-y-auto p-3">
        {focusedNode() ? (
          <div>
            ...
            {/* Neighbors */}
            {getNeighbors(focusedNode().id).length > 0 && (
              <div class="mt-3">
                <div class="flex justify-between text-xs">
                  <span class="text-text-secondary">Neighbors</span>
                  <span class="text-text-primary">{getNeighbors(focusedNode().id).length}</span>
                </div>
                <div class="mt-1 space-y-0.5">
                  <For each={getNeighbors(focusedNode().id).slice(0, neighborLimit())}>
                    {(nid) => {
                  ...
                  {getNeighbors(focusedNode().id).length > neighborLimit() && (
                    <button
                      class="text-[11px] text-text-muted px-2 py-0.5"
                      onClick={() => setNeighborLimit(neighborLimit() + 10)}
                    >
                      +{getNeighbors(focusedNode().id).length - neighborLimit()} more
                    </button>
                  )}
                </div>
              </div>
