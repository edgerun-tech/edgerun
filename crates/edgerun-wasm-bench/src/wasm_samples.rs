...
pub fn ensure_samples(dir: &str) -> Result<()> {
    ...
    let wasm_samples = [(
        ...
        vec![
            ...
            0x00, 0x00, 0x03, 0x02, 0x00, 0x00, 0x05, 0x03, 0x01, 0x02, 0x68, 0x00, 0x07, 0x03,
            0x01, 0x02, 0x72, 0x75, 0x6e, 0x00, 0x00, 0x09, 0x03, 0x01, 0x02, 0x00, 0x00, 0x00,
            0x00, 0x0b,
        ],
    )];
...
(module
  ...
  (export "run") (param i32 i32) (result i32)
    ...
    for (name, wat) in samples {
        ...
        let wasm_path = dir
            .join(name.strip_suffix(".wat").unwrap_or(name))
            .with_extension("wasm");
