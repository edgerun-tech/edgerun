...
impl From<u8> for CpuId { fn from(id: u8) -> Self { Self(id) } }
impl From<CpuId> for u8 { fn from(id: CpuId) -> Self { id.0 } }
...
pub static mut CPU_DATA: [PerCpu; MAX_CPUS] = [PerCpu { id: CpuId(0), stack_top: 0, tls_ptr: 0, run_queue_head: 0, run_queue_tail: 0, idle_count: 0 }; MAX_CPUS];
...
pub fn this() -> &'static PerCpu {
    ...
    {
        let id: usize;
        unsafe { core::arch::asm!("mov rax, fs:4", out("rax") id); }
...
pub fn cpu_init(id: CpuId, stack_top: usize) {
    unsafe { CPU_DATA[id.0 as usize].id = id; }
    unsafe { CPU_DATA[id.0 as usize].stack_top = stack_top; }
}
