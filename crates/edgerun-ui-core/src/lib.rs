...
//!
//! This crate intentionally does not implement DOM, CSS, Wayland, GTK, Qt,
//! terminal escape handling, or browser layout. It draws a small system UI kit
//! into a caller-owned XRGB8888/ARGB8888 buffer so the same UI language can be
//! used by the compositor, terminal, framebuffer, browser canvas host, and
//! remote renderer.
...
impl<'a> Painter<'a> {
    ...
    pub fn badge(&mut self, rect: Rect, label: &str, color: Color, theme: Theme) {
        self.rect(rect, color.with_alpha(80));
...
pub fn demo_dashboard(p: &mut Painter<'_>, state: DashboardState<'_>, theme: Theme) {
    ...
    let open = Rect::new(hero.x + 24, hero.y + 120, 168, 38);
    let trust = Rect::new(hero.x + 204, hero.y + 120, 168, 38);
    p.button(open, "Open node", theme, true);
    p.button(trust, "Trust root", theme, false);
...
fn glyph5x7(byte: u8) -> [u8; 7] {
    let c = if (b'a'..=b'z').contains(&byte) {
        byte - 32
    } else {
        byte
    };
