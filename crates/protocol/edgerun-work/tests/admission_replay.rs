...
#[test]
fn admission_controller_reserves_budget_when_signing_admission() {
    let controller = controller();
    let (relay_client, relay_thread) = admit_relay(&controller, 61);
    let user_key = Ed25519SigningKey::from_bytes(&[62u8; 32]);
    let user = user_public_key(&user_key);
    controller.set_user_balance(user, 7);

    assert_admission(
        controller
            .handle_packet(
                WorkPacket::WorkRequest(signed_request(&user_key, 1, [62u8; 32])),
                None,
            )
            .expect("funded request handled"),
    );
    assert_ack(
        controller
            .handle_packet(
                WorkPacket::WorkRequest(signed_request(&user_key, 2, [63u8; 32])),
                None,
            )
            .expect("second request handled"),
        402,
