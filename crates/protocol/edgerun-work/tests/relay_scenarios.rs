...
fn relay_forwards_ordered_message_to_final_node_and_gets_paid_with_delivery_proof() {
    ...
    let mut bundle_builder = RelayTransitBundleBuilder::new(
        request_hash,
        admission_hash,
        ...
        result.forwarded_packet_hash,
        8,
    )
    .expect("relay transit bundle builder");
    let appended_transit = result
        .append_to_transit_bundle(&mut bundle_builder)
        .expect("live relay result appends to bundle");
    assert_eq!(appended_transit, result.transit_hash);
    let route_commitment = bundle_builder.route_commitment();
    let transit_bundle = bundle_builder
        .finish(channel_proof_hash(&recipient_proof))
        .expect("relay transit bundle");
    assert_eq!(
        route_commitment,
        relay_transit_bundle_route_commitment(&transit_bundle)
    );
    assert_eq!(transit_bundle.hops[0], result.transit_hop_evidence(0));
    verify_relay_transit_bundle(
        &transit_bundle,
        &RelayTransitBundleContext {
            request_hash,
            admission_hash,
            controlling_admission_node_id: admission.identity.node_id,
            source_node_id: sender.identity.node_id,
            destination_node_id: receiver.identity.node_id,
            relay_path: &[relay.identity.node_id],
            packet_hash: result.forwarded_packet_hash,
