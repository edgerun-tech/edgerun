...
#[test]
fn thread_safe_settlement_wrapper_exposes_custody_requirement_verifier() {
    let ledger = ThreadSafeSettlementLedger::new();
    let valid = RelayTransitCustodyRequirement {
        expected_packed_proof_root: blake3_hash(b"runtime-custody-root"),
        required_custody_kind: RELAY_PROOF_CUSTODY_KIND_STORED,
    };
    let empty_root = RelayTransitCustodyRequirement {
        expected_packed_proof_root: [0u8; 32],
        required_custody_kind: RELAY_PROOF_CUSTODY_KIND_STORED,
    };
    let unknown_kind = RelayTransitCustodyRequirement {
        expected_packed_proof_root: blake3_hash(b"runtime-custody-root"),
        required_custody_kind: 99,
    };

    ledger
        .verify_relay_transit_custody_requirement(&valid)
        .expect("valid custody requirement");
    assert_eq!(
        ledger.verify_relay_transit_custody_requirement(&empty_root),
        Err(SettlementError::InvalidRelayProofCustodyAck)
    );
    assert_eq!(
        ledger.verify_relay_transit_custody_requirement(&unknown_kind),
        Err(SettlementError::InvalidRelayProofCustodyAck)
    );
}
