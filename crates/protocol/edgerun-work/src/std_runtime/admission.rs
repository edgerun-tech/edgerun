...
impl InMemoryAdmissionController {
    ...
    fn accept_first(
        ...
        let WorkPacket::NodeAvailable(available) = packet else {
            write_work_packet(
                stream,
                &ack(false, 400, "first packet must be NodeAvailable"),
            )?;
            return Ok(None);
        };
    ...
    fn handle_heartbeat(&self, heartbeat: NodeHeartbeat, on_connection: Option<NodeId>) -> bool {
        if !verify_node_heartbeat(&heartbeat)
            || on_connection.is_some_and(|id| id != heartbeat.node.node_id)
        {
            return false;
        }
        let mut state = self.inner.lock().expect("admission state poisoned");
    ...
    fn handle_network_message(
        ...
        if on_connection != Some(message.via_relay) {
            return Ok(ack(
                false,
                403,
                "message not received on claimed relay connection",
            ));
        }
        let mut state = self.inner.lock().expect("admission state poisoned");
    ...
    fn handle_work_request(&self, request: WorkRequest) -> io::Result<WorkPacket> {
        ...
        let Some(balance) = state.balances.get_mut(&request.user) else {
            return Ok(ack(false, 402, "insufficient work credit"));
        };
        if *balance < request.max_total_cost {
            return Ok(ack(false, 402, "insufficient work credit"));
        }
        *balance -= request.max_total_cost;
        state
            .replay
        ...
        let admission = WorkAdmission {
            ...
            admission_id: admission_id_for_request(
                self.admission_identity.node_id,
                request_hash,
                sequence,
            ),
            dao_id: self.dao_id,
        ...
        Ok(WorkPacket::WorkAdmission(sign_work_admission(
            &self.admission_key,
            admission,
        )))
    }
...
fn admission_replay_ack(error: AdmissionReplayError) -> WorkPacket {
    match error {
        ...
        AdmissionReplayError::NonIncreasingUserSequence { .. } => {
            ack(false, 409, "stale user request sequence")
        }
    }
}
