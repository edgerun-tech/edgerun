...
use crate::codec::{EdgeWire, WireCursor, WireWriter, blake3_hash, wire_bytes, wire_from_bytes};
...
const NOTARY_WORK_REPORT_DOMAIN: &[u8] = b"edgerun:v1:work:notary:work-report";
...
pub fn notary_work_report_preimage(value: &NotaryWorkReport) -> Vec<u8> {
    PreimageBuilder::domain(NOTARY_WORK_REPORT_DOMAIN)
        .node(&value.notary)
        .node_id(&value.requester)
        ...
        .hash(&value.response_hash)
        .u64(value.completed_unix_ms)
        .finish()
}

pub fn sign_notary_work_report(
    key: &Ed25519SigningKey,
    mut value: NotaryWorkReport,
) -> NotaryWorkReport {
    value.signature = sign_ed25519(key, &notary_work_report_preimage(&value));
    value
}

pub fn verify_notary_work_report(value: &NotaryWorkReport) -> bool {
    value.abi_version == WORK_WIRE_ABI_VERSION
        && value.notary.role == NODE_ROLE_NOTARY
        && matches!(
            value.work_type,
            WORK_TYPE_NOTARY_SEAL | WORK_TYPE_NOTARY_UNSEAL
        )
        && verify_signature(
            &value.notary,
            &value.signature,
            &notary_work_report_preimage(value),
        )
}

impl EdgeWire for NotaryWorkReport {
    fn encode_wire(&self, out: &mut WireWriter) {
        ...
        self.response_hash.encode_wire(out);
        self.completed_unix_ms.encode_wire(out);
    ...
    fn decode_wire(input: &mut WireCursor<'_>) -> Result<Self, WorkProtocolError> {
        Ok(Self {
            ...
            response_hash: <Hash as EdgeWire>::decode_wire(input)?,
            completed_unix_ms: <u64 as EdgeWire>::decode_wire(input)?,
