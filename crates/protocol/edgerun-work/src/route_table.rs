...
use crate::route_auth::{
    available_route_hash, route_hash, route_is_available, verify_live_route_advertisement,
};

pub(crate) type RouteMap = BTreeMap<NodeId, RouteAdvertisement>;
pub(crate) type RouteInboxMap = BTreeMap<NodeId, Vec<ChannelEnvelope>>;

pub(crate) fn insert_route(routes: &mut RouteMap, route: RouteAdvertisement) -> Hash {
...
pub(crate) fn insert_live_route(
    ...
    verify_live_route_advertisement(&route, now_unix_ms).then(|| insert_route(routes, route))
}
...
pub(crate) fn insert_live_route_with_inbox(
    ...
    verify_live_route_advertisement(&route, now_unix_ms)
