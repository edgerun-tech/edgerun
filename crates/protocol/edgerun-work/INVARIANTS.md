...
`RouteBinding` is derived runtime state recorded by admission/runtime code. It is
not signed by a capability node and must not be accepted as a work authority
without the user/admission chain.

Runtime APIs may apply a `RouteBinding` so bytes can move, but the caller must
already be operating under admission/runtime authority. Recording a binding does
...
Portable protocol code should remain `no_std` + `alloc` compatible.

Allowed in portable modules:
