//! Legacy command validation for the older edgerun stream protocol.
//!
...
//! New cross-node authority should be modeled as admitted work in
//! `edgerun-work`; this module remains for migration and existing local stream
//! validation.
//!
//! ## Protocol invariants (§5, §18.6)
//!
//! - A command is not network authority; admitted work is the canonical
//!   authority path for work crossing node, relay, capability, storage,
//!   execution, or settlement boundaries
//! - Delivery alone has no effect on node state
...
fn required_capability_for_command(command_type: i32) -> Option<(i32, &'static str)> {
    ...
    match crate::protocol::enum_from_i32::<Ct>(command_type)? {
        ...
        // Legacy enum names are kept for wire compatibility. The capability
        // action labels describe the converged run/cache model instead.
        Ct::InstallApp => Some((Ck::ExecuteWorkload as i32, "verify_cache_app")),
        Ct::UninstallApp => Some((Ck::ExecuteWorkload as i32, "remove_app_cache")),
