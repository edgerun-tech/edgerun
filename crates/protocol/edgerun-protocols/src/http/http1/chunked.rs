...
pub type Result<T> = core::result::Result<T, ChunkedBodyError>;
...
impl core::fmt::Display for ChunkedBodyError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ChunkedBodyError::InvalidBody => f.write_str("invalid chunked body"),
        }
    }
}
...
pub fn parse_chunked_body(data: &[u8]) -> Result<Vec<u8>> {
    crate::http::chunked::parse_body(data).map_err(|_| ChunkedBodyError::InvalidBody)
}
...
pub fn parse_chunked_body_with_trailers(data: &[u8]) -> Result<(Vec<u8>, HeaderMap)> {
    crate::http::chunked::parse_body_with_trailers(data).map_err(|_| ChunkedBodyError::InvalidBody)
}
