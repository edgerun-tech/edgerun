...
pub fn validate_request_headers(headers: &[(Vec<u8>, Vec<u8>)]) -> ValidationResult {
    ...
    for (name, value) in headers {
        if name.starts_with(b":") {
            ...
            match name.as_slice() {
                ...
                _ => {
                    return Err((ErrorCode::ProtocolError.to_u32(), "unknown pseudo-header"));
                }
            }
        } else {
    ...
    if method_count == 0 {
        return Err((ErrorCode::ProtocolError.to_u32(), "missing :method"));
    }
    if scheme_count == 0 {
        return Err((ErrorCode::ProtocolError.to_u32(), "missing :scheme"));
    }
    if path_count == 0 {
        return Err((ErrorCode::ProtocolError.to_u32(), "missing :path"));
    }

    // No duplicates
    if method_count > 1 {
        return Err((ErrorCode::ProtocolError.to_u32(), "duplicate :method"));
    }
    if scheme_count > 1 {
        return Err((ErrorCode::ProtocolError.to_u32(), "duplicate :scheme"));
    }
    if path_count > 1 {
        return Err((ErrorCode::ProtocolError.to_u32(), "duplicate :path"));
    }

    // :path MUST NOT be empty for non-CONNECT requests (RFC 9113 §8.3.1)
    for (name, value) in headers {
        if name == b":path" && value.is_empty() {
            return Err((ErrorCode::ProtocolError.to_u32(), "empty :path"));
        }
    }
