//! OCI distribution image reference parsing.

use alloc::format;
use alloc::string::{String, ToString};
use core::{fmt, str::FromStr};

use super::manifest::validate_digest_reference;
...
mod tests {
    ...
    #[test]
    fn defaults_docker_library_latest() {
        let image: ImageRef = "alpine".parse().unwrap();
    ...
    fn parses_registry_repository_tag() {
        let image: ImageRef = "registry.example.com/team/app:v1".parse().unwrap();
    ...
    fn parses_digest_reference() {
        ...
        let image: ImageRef = format!("example.com/team/app@{digest}").parse().unwrap();
        assert!(image.is_digest_reference());
        assert_eq!(image.to_string(), format!("example.com/team/app@{digest}"));
    }
}
