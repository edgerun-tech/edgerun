//! OCI image config and manifest JSON parsers.
...
pub fn parse_image_config(data: &[u8]) -> Result<ImageConfig, String> {
    from_json_slice(data).map_err(|err| err.to_string())
}
...
fn take_config_descriptor(
    ...
    match object.remove("config") {
        ...
        Some(other) => Err(JsonValueError::WrongType(alloc::format!(
