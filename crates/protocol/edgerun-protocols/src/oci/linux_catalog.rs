//! Shared Linux name catalogs used by OCI validation and host setup.

pub const LINUX_CAPABILITIES: &[(&str, u32)] = &[
    ("CAP_CHOWN", 0),
    ("CAP_DAC_OVERRIDE", 1),
...
pub const LINUX_RLIMITS: &[(&str, u32)] = &[
    ("RLIMIT_CPU", 0),
    ("RLIMIT_FSIZE", 1),
    ("RLIMIT_DATA", 2),
    ("RLIMIT_STACK", 3),
...
pub const SECCOMP_ARCHES: &[(&str, u32)] = &[
...
pub const LINUX_NAMESPACES: &[(&str, Option<i32>)] = &[
...
pub fn is_oci_capability_name(name: &str) -> bool {
    has_exact_name(name, LINUX_CAPABILITIES)
}

pub fn rlimit_number(name: &str) -> Option<u32> {
    named_linux_value(name, "RLIMIT_", LINUX_RLIMITS)
}

pub fn is_oci_rlimit_name(name: &str) -> bool {
    has_exact_name(name, LINUX_RLIMITS)
}

pub fn seccomp_arch_bpf(name: &str) -> Option<u32> {
    named_linux_value(name, "SCMP_ARCH_", SECCOMP_ARCHES)
}

pub fn is_seccomp_arch_name(name: &str) -> bool {
    has_exact_name(name, SECCOMP_ARCHES)
}

pub fn namespace_flag(name: &str) -> Option<i32> {
    LINUX_NAMESPACES
        .iter()
...
pub fn is_oci_namespace_name(name: &str) -> bool {
    LINUX_NAMESPACES
        .iter()
