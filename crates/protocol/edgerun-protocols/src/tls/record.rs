...
impl RecordCipher {
    ...
    pub fn new(key: &[u8], iv: &[u8]) -> Result<Self, super::TlsError> {
        if iv.len() != 12 {
            return Err(super::TlsError::Cipher(format!(
        ...
        let cipher =
            AeadCipher::new_from_key(key).map_err(|e| super::TlsError::Cipher(e.to_string()))?;
