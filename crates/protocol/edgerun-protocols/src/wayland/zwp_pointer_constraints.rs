...
pub fn locked_pointer_locked_event(locked_id: u32) -> Message {
    crate::wayland::encode::message_empty(locked_id, locked_pointer_event::LOCKED)
}

/// Build unlocked event.
pub fn locked_pointer_unlocked_event(locked_id: u32) -> Message {
    crate::wayland::encode::message_empty(locked_id, locked_pointer_event::UNLOCKED)
}

/// Build locked pointer motion event.
pub fn locked_pointer_motion_event(locked_id: u32, time: u32, dx: u32, dy: u32) -> Message {
    let mut args = Vec::new();
    push_u32_le(&mut args, 0); // time_hi (we use 32-bit time)
    push_u32_le(&mut args, time); // time_lo
    push_u32_le(&mut args, dx); // dx (fixed point)
    push_u32_le(&mut args, dy); // dy (fixed point)
    Message {
        sender_id: locked_id,
...
pub fn confined_pointer_confined_event(confined_id: u32) -> Message {
    crate::wayland::encode::message_empty(confined_id, confined_pointer_event::CONFINED)
}

/// Build unconfined event.
pub fn confined_pointer_unconfined_event(confined_id: u32) -> Message {
    crate::wayland::encode::message_empty(confined_id, confined_pointer_event::UNCONFINED)
}
