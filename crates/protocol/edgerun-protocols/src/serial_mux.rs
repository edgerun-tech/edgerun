...
impl<const N: usize> Receiver<N> {
    ...
    pub fn push(&mut self, byte: u8) -> Result<Option<ReceivedFrame<N>>, DecodeError> {
        if byte == FLAG {
...
pub fn encode_frame<F>(channel: u8, seq: u16, payload: &[u8], mut write_raw: F)
where
    F: FnMut(&[u8]),
{
    let len = payload.len() as u16;
    let mut crc = 0xffff;
    let mut writer = EncodedWriter::new(&mut write_raw);
...
impl<'a> EncodedWriter<'a> {
    fn new<F>(write_raw: &'a mut F) -> Self
    where
        F: FnMut(&[u8]),
    {
        Self {
    ...
    fn flush(&mut self) {
        ...
        (self.write_raw)(&self.buf[..self.len]);
        self.len = 0;
    }
}
...
mod tests {
    ...
    fn receiver_accepts_streamed_frame() {
        ...
        for &byte in &encoded[..len - 1] {
            assert!(rx.push(byte).unwrap().is_none());
        }
        let frame = rx.push(encoded[len - 1]).unwrap().unwrap();
    ...
    fn receiver_recovers_after_bad_frame() {
        ...
        for &byte in &encoded[..len] {
            frame = rx.push(byte).unwrap().or(frame);
        }
    ...
    #[test]
    fn encodes_frame_with_callback() {
        let mut encoded = [0u8; 64];
        let mut written = 0;
        encode_frame(CHANNEL_WIFI, 5, b"wifi", |chunk| {
            encoded[written..written + chunk.len()].copy_from_slice(chunk);
            written += chunk.len();
        });
        let mut scratch = [0u8; 64];

        let frame = decode(&encoded[..written], &mut scratch).unwrap();

        assert_eq!(frame.channel, CHANNEL_WIFI);
        assert_eq!(frame.seq, 5);
        assert_eq!(frame.payload, b"wifi");
    }
}
