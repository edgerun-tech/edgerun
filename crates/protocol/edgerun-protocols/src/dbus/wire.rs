...
impl fmt::Display for DbusWireError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnexpectedEof(message) => write!(f, "unexpected end of D-Bus data: {message}"),
            Self::InvalidData(message) => write!(f, "invalid D-Bus data: {message}"),
        }
    }
}

impl core::error::Error for DbusWireError {}

impl From<FromUtf8Error> for DbusWireError {
    fn from(value: FromUtf8Error) -> Self {
        Self::invalid(value.to_string())
    }
}

pub type DbusWireResult<T> = Result<T, DbusWireError>;

// Consume one complete type from a signature string.
pub fn consume_one(sig: &str) -> DbusWireResult<(String, &str)> {
    if sig.is_empty() {
        return Err(DbusWireError::invalid("empty sig"));
    }
    match sig.as_bytes()[0] {
        b'a' => {
            ...
            if el.starts_with('{') {
                ...
                if end == 0 {
                    return Err(DbusWireError::invalid("bad dict"));
                }
        ...
        b'(' => {
            ...
            if end == 0 {
                return Err(DbusWireError::invalid("bad struct"));
            }
        ...
        _ => Err(DbusWireError::invalid("bad type")),
    }
}
    ...
    match v {
        ...
        Val::Dict(entries) => {
            if entries.is_empty() {
                ...
                format!("a{{{}{}}}", vsig(&entries[0].0), vsig(&entries[0].1))
            }
        }
...
impl Rdr {
    ...
    fn al(&mut self, a: usize) {
        let o = self.p % a;
        if o != 0 {
            self.p += a - o;
        }
    }
    fn by(&mut self, n: usize) -> DbusWireResult<&[u8]> {
        if self.p + n > self.d.len() {
            return Err(DbusWireError::UnexpectedEof("trunc"));
        }
        let s = self.p;
        self.p += n;
        Ok(&self.d[s..s + n])
    }
    fn u8(&mut self) -> DbusWireResult<u8> {
        Ok(self.by(1)?[0])
    }
    fn u16(&mut self) -> DbusWireResult<u16> {
        let b = self.by(2)?;
        Ok(read_u16_le(b, 0))
    }
    fn u32(&mut self) -> DbusWireResult<u32> {
        let b = self.by(4)?;
        Ok(read_u32_le(b, 0))
    }
    fn i32(&mut self) -> DbusWireResult<i32> {
        let b = self.by(4)?;
        Ok(read_i32_le(b, 0))
    }
    fn u64(&mut self) -> DbusWireResult<u64> {
        let b = self.by(8)?;
        Ok(read_u64_le(b, 0))
    }
    fn sv(&mut self) -> DbusWireResult<String> {
        let l = self.u32()? as usize;
        let b: Vec<u8> = self.by(l)?.to_vec();
        if self.rem() > 0 && self.d[self.p] == 0 {
            self.p += 1;
        }
        Ok(String::from_utf8(b)?)
    }
    fn sg(&mut self) -> DbusWireResult<String> {
    ...
    pub fn val(&mut self, sig: &str) -> DbusWireResult<Val> {
        if sig.is_empty() {
            return Err(DbusWireError::invalid("empty"));
        }
        match sig.as_bytes()[0] {
            ...
            b'r' | b'(' => {
                let inner = if sig.starts_with('(') && sig.ends_with(')') {
                    &sig[1..sig.len() - 1]
                } else {
                    sig
                };
            ...
            b'v' => {
                let is = self.sg()?;
                Ok(Val::Var(Box::new(self.val(&is)?)))
            }
            _ => Err(DbusWireError::invalid("bad type")),
        }
    }
        ...
        match sig.as_bytes()[0] {
            ...
            b'a' => {
                ...
                if el.starts_with('{') && el.ends_with('}') {
                    ...
                    let td = tmp.finish();
                    self.pd(4);
                    self.u32(td.len() as u32);
                    self.d.extend_from_slice(&td);
                } else if el == "y" {
                    if let Val::Arr(items) = v {
                        let bytes: Vec<u8> = items
                            .iter()
                            .filter_map(|x| if let Val::Y(b) = x { Some(*b) } else { None })
                            .collect();
                        self.pd(4);
            ...
            b'r' | b'(' => {
                ...
                if let Val::Str(fields) = v {
                    let mut rem = inner;
                    for f in fields {
                        let (ts, r) = match consume_one(rem) {
                            Ok(r) => r,
                            Err(_) => break,
                        };
                        self.wval(&ts, f);
...
pub fn encode_msg(msg: &Msg) -> Vec<u8> {
    ...
    if !msg.bsig.is_empty() {
        ...
        hf.y(b'v');
        hf.gs("g");
        hf.wval("g", &Val::G(msg.bsig.clone()));
    }
...
pub fn decode_msg(data: &[u8]) -> DbusWireResult<Msg> {
    if data.len() < 16 {
        return Err(DbusWireError::UnexpectedEof("short"));
    }
    if data[0] != BYTE_ORDER {
        return Err(DbusWireError::invalid("order"));
    }
    let mt = match data[1] {
        1 => MType::Call,
        2 => MType::Return,
        3 => MType::Err,
        4 => MType::Signal,
        _ => return Err(DbusWireError::invalid("type")),
    };
    let fl = data[2];
    ...
    while rdr.rem() > 0 {
        let fc = rdr.u8()?;
        if rdr.u8()? != b'v' {
            return Err(DbusWireError::invalid("not var"));
        }
        let sg = rdr.sg()?;
...
mod tests {
    ...
    fn roundtrip_struct_ss() {
        let sig = "(ss)";
        let secret = Val::Str(vec![Val::S("hello".into()), Val::S("world".into())]);
        let mut w = Wtr::new();
        w.wval(sig, &secret);
        let bytes = w.finish();
        let mut r = Rdr::new(bytes);
