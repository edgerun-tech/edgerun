            ...
            DnsRecordType::AAAA => {
                if data.len() == 16 {
                    let octets: [u8; 16] = data.try_into().map_err(|_| {
                        super::io::Error::new(super::io::ErrorKind::InvalidData, "AAAA wrong size")
                    })?;
    ...
    loop {
        ...
        if (len_byte & 0xC0) == 0xC0 {
            if offset + 1 >= data.len() {
                return Err(super::io::Error::new(
                    super::io::ErrorKind::InvalidData,
...
fn decode_tsig_name(data: &[u8], offset: usize) -> Result<String, super::io::Error> {
    let mut labels = Vec::new();
    let mut pos = offset;
    loop {
        if pos >= data.len() {
