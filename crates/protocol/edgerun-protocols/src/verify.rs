//! Verification adapters for legacy `edgerun-core` protocol records.
//!
//! A valid signature here proves the older record bytes were signed. It does
//! not by itself admit network work, choose relays, or make a receipt payable;
//! those authority paths belong in `edgerun-work`.
...
/// Signed legacy protocol record families that can become durable local facts,
/// signed advisory evidence, or signed access artifacts.
///
...
/// handshakes, but they are not admitted-work authority records.
...
impl ProtocolFamily {
    ...
    /// by concrete legacy protocol encoders in edgerun-core today.
