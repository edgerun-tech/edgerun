...
import { applyXrayEdits, getXrayEditInstruction, parseXrayEdits } from "@/stores/xray-edit-store"
...
export function AssistantApp() {
  ...
  const handleApplyXrayEdits = async (messageId: string, content: string) => {
    const edits = parseXrayEdits(content)
    if (edits.length === 0 || applyingMessageId) return

    setApplyingMessageId(messageId)
    try {
      const results = await applyXrayEdits(edits)
      const ok = results.filter((result) => result.ok)
      const failed = results.filter((result) => !result.ok)
      const summary = [
        `Repo Xray edit result: ${ok.length}/${results.length} applied.`,
        ...ok.map((result) => `- applied: ${result.path}`),
        ...failed.map((result) => `- failed: ${result.path}: ${result.error || "unknown error"}`),
      ].join("\n")
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemPrompt = fs.rootHandle
        ? `${getPromptForModel()}\n\n${getXrayEditInstruction()}`
        : getPromptForModel()
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="space-y-4">
          {[...session.messages].reverse().map((msg) => {
            const xrayEdits = msg.role === "assistant" ? parseXrayEdits(msg.content) : []
            return (
              <div key={msg.id} className={cn("flex gap-3", msg.role === "user" && "flex-row-reverse")}>
                ...
                <div className={cn("flex max-w-[85%] flex-col gap-1", msg.role === "user" && "items-end")}>
                  ...
                  {xrayEdits.length > 0 && fs.rootHandle && (
                    <button
                      onClick={() => handleApplyXrayEdits(msg.id, msg.content)}
                      disabled={applyingMessageId === msg.id}
                      className="mt-2 flex w-fit items-center gap-2 rounded-md border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs text-primary hover:bg-primary/20 disabled:opacity-50"
                    >
                      {applyingMessageId === msg.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wand2 className="h-3.5 w-3.5" />}
                      Apply {xrayEdits.length} Repo Xray edit{xrayEdits.length === 1 ? "" : "s"}
                    </button>
                  )}
