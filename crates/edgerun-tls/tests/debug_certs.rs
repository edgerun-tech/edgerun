...
use edgerun_tls::certificate_gen::{generate_self_signed, generate_self_signed_pem, cert_from_pem, signing_key_from_pem};
...
fn debug_cert_validity() {
    ...
    eprintln!("duration: {} seconds, {} days", 
...
fn debug_hkdf() {
    ...
    for len in &[16, 32, 48, 64] {
        match std::panic::catch_unwind(|| {
            sha256.expand_label(&secret, "key", &[], *len)
        }) {
            Ok(result) => eprintln!("expand_label with len={} succeeded: {} bytes", len, result.len()),
