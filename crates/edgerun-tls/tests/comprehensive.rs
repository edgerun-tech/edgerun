...
use edgerun_tls::certificate_gen::{
    generate_self_signed, generate_self_signed_pem, cert_from_pem, 
...
use edgerun_tls::prf::{Hasher, Tls13KeySchedule, hmac_sha256, hmac_sha384};
...
fn test_cert_generation_multiple_hostnames() {
    let hostnames = &["localhost", "example.com", "www.example.com", "*.test.example.com"];
...
fn test_cert_validity_period() {
    ...
    assert!(duration_days >= 360 && duration_days <= 370, 
        "Certificate validity should be ~365 days, got {} days", duration_days);
}
...
fn test_alert_level_wire_roundtrip() {
    assert_eq!(AlertLevel::Warning, AlertLevel::from_wire(AlertLevel::Warning as u8).unwrap());
    assert_eq!(AlertLevel::Fatal, AlertLevel::from_wire(AlertLevel::Fatal as u8).unwrap());
